alter procedure dbo.eeisp_edi_vend_830_DixieNet as
begin
------------------------------------------------------------------------------------
-- eeisp_edi_vend_830
...
---06/05/2007 - Selects only PIONEER from Vendor List - loaded procedure as eeisp_edi_vend_830_rev1_PIONEER, all other vendors still use eeisp_edi_vend_830_rev1
--09/11/2007 - Now Selects TTI and PIONEER - Andre S. Boulnager
--03/09/2007 - Selects DixeWire - Andre S. Boulnager
------------------------------------------------------------------------------------
...
where   edi_vendor.auto_create_po = 'Y' and
	...
	(edi_vendor.trading_partner_code like '%DIXIE%' OR edi_vendor.trading_partner_code like '%AEES%' OR edi_vendor.trading_partner_code LIKE '%THERMOLINK%' )and po_header.po_number in  (select PONUmber 
																																				from ft.releaseplanraw 
																																			join		ft.releaseplans on ft.releaseplanraw.ReleasePlanID =  ft.releaseplans.id 
																																		where GeneratedDT >= dateadd(wk,-6, getdate()) and STDQty>0 and PONumber = po_header.po_number
																																		UNION
																																		Select PONumber from ft.WkNMPS where PriorDemandAccum > 0 and PONumber = po_header.po_number)
...
where	po_detail.part_number = edi_830_work.part and
		...
		po_detail.date_due = (select min(date_due) 
								from po_detail
							where	po_detail.part_number = edi_830_work.part and
									po_detail.po_number = edi_830_work.po_number and
									isNULL(alternate_price,0)>0) 
...
where	part_vendor_price_matrix.part = edi_830_work.part and
		...
		part_vendor_price_matrix.break_qty = (select min(break_qty) 
								from part_vendor_price_matrix
							where	part_vendor_price_matrix.part = edi_830_work.part and
									part_vendor_price_matrix.vendor = edi_830_work.vendor and
									isNULL(edi_830_work.price,0)=0) 
