...
/*
Create Procedure.Monitor.FT.ftsp_CommonSerialReceiveSerials.sql
*/

use Monitor
go

if	objectproperty(object_id('FT.ftsp_CommonSerialReceiveSerials'), 'IsProcedure') = 1 begin
	drop procedure FT.ftsp_CommonSerialReceiveSerials
end
go
...
from
	OpenQuery (' + FT.fn_GetLinkedServer (@OriginPlant) + ', ''
		...
		select
			*
		from
			Monitor.FT.CommonSerialShipLog
		where
			Origin = ''''' + @OriginPlant + '''''
...
/*	Any records that have already been received , mark as processed. */
if	exists
		(	select
				*
			from
