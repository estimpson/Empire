...
if	objectproperty(object_id('EEA.fn_ProgramShippingRequirements2'), 'IsTableFunction') = 1 begin
	drop function EEA.fn_ProgramShippingRequirements2
end
go

create function EEA.fn_ProgramShippingRequirements2
()
...
-- <Body>
	...
	,	InTransit = coalesce(availableMaterial.InTransit, 0)
	...
	from
		...
		(	select
			...
			,	InTransit = coalesce(sum(case when nmps1.LowLevel >= 1 then nmps1.InTransitQty end), 0)
			from
				@NetMPS nmps1
				join @NetMPS nmps2
					on nmps2.Sequence = 0
	...
	,	QtyTrans = coalesce(allocMaterial.QtyTrans, 0)
	,	QtyCurrent =
		case
	...
	from
		...
		(	select
			...
			,	QtyTrans = coalesce(sum(case when nmps1.LowLevel >= 1 then nmps1.InTransitQty end), 0)
			from
				@NetMPS nmps1
			group by
				nmps1.OrderNo
