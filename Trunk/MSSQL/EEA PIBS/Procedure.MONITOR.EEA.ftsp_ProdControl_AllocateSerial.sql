...
/*
Create Procedure.MONITOR.EEA.ftsp_ProdControl_AllocateSerial.sql
*/
...
execute
	@ProcReturn = EEA.ftsp_ProdControl_AllocateSerial 
		@Operator = @Operator
	,	@WODID = @WODID
	,	@Serial = @Serial
	,	@EnforceFIFO = 1
...
if	not exists
	...
	set	@Result = 100001
	rollback tran AllocateSerial
	RAISERROR ('Invalid serial number %d', 16, 1, @Serial)
	return	@Result
end
...
--	If enforcing FIFO, serial must be FIFO for this component.
if	@EnforceFIFO = 1 begin
	if	(	select
	  			case when fgpf.IsInFifo > '' then 1 else 0 end
	  		from
	  			dbo.object o
				cross apply EEA.fn_GetPartFIFO(o.part, 'A', 'EEA', null, 'N') fgpf
			where
				o.serial = @Serial
				and o.serial = fgpf.Serial
	  	) != 1 begin
	
		set	@Result = 100002
		rollback tran AllocateSerial
		RAISERROR ('Serial %d is not the oldest inventory for this part.  Allocate oldest inventory or override FIFO enforcement.', 10, 1, @Serial)
		return	@Result
	end      
end
