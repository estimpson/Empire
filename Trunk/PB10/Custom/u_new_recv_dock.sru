HA$PBExportHeader$u_new_recv_dock.sru
forward
...
//	Commented the above statements and included the statement below to accomodate 
//	Empire's request 3/29/02

	if (i_b_warning or i_b_error) then
		select	custom3
		into	:ls_allowoverreceipt
		from	vendor_custom
		where	code = :ls_vendorcode ;
		
		if isnull(ls_allowoverreceipt,'Y') = 'N' then 
			messagebox ( monsys.msg_title, "Sorry you can't over receive from this vendor", Information!)
			i_b_error = false
			i_b_warning = false
			return
		else	
