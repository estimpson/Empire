HA$PBExportHeader$w_schedule_grid_base.srw
forward
global type w_schedule_grid_base from Window
end type
...
if bcancel then 																										// ADD 02/24/2000 KAZ
	Close ( w_statusbar )																							// ADD 02/24/2000 KAZ
...
IF l_s_value > '' THEN
	...
	l_l_row	= Long ( Right ( l_s_value,  Len ( l_s_value ) - l_i_pos ) )
END IF

IF l_s_band > '' THEN
	l_i_pos	= Pos ( l_s_band, "~t", 1 )	
	l_s_col	= Left ( l_s_band, l_i_pos - 1 ) 
	
	IF POS ( l_s_band, 'date' ) > 0 OR POS ( l_s_band, 'part_h' ) > 0  OR POS ( l_s_band, 'past_h' ) > 0 THEN
		wmainscreen.SetMicroHelp ( "Right Click on the grid" )
		RETURN
	END IF
...
IF l_l_row > 0 AND l_s_col > '' THEN
	...
	IF l_s_col <> 'part_value' THEN
		...
		CHOOSE CASE l_s_col
			...
			CASE ELSE
		      l_i_value = Integer ( MID ( is_current_col, 6, len ( is_current_col ) - 5 ) )
...
IF l_s_band > '' THEN
	...
	IF POS ( l_s_band, 'date' ) > 0 OR POS ( l_s_band, 'part_h' ) > 0 OR POS ( l_s_band, 'past_h' ) > 0 THEN
		wmainscreen.SetMicroHelp ( "Click on the grid value.." )
		RETURN
	END IF
...
FOR l_l_row = l_l_first_row to l_l_last_row
	...
	w_statusbar.wf_setstatus ( "Part: " + a_s_part, ( l_l_row / l_l_last_row ) * 100 )
...
on selectionchanged;wmainscreen.SetMicroHelp ( "You must rebuild the crosstab..." )
