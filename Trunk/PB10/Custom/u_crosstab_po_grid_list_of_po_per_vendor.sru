HA$PBExportHeader$u_crosstab_po_grid_list_of_po_per_vendor.sru
...
IF dw_selection.rowcount() = 0 THEN
	...
	END IF
END IF
ll_fndrow = dw_selection.find("po_number = "+string(ll_po), 1, dw_selection.rowcount())

uo_release.uf_reset()

if ll_fndrow > 0 then
	dw_selection.selectrow(0,false)
	dw_selection.selectrow(ll_fndrow,true)
	dw_selection.triggerevent(clicked!)
end if 
