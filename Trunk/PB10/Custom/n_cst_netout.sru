HA$PBExportHeader$n_cst_netout.sru
forward
...
type str_crosstab from structure
	...
	long		default_po_number
	double		weeks_to_freeze
	string		auto_releases
	datetime		leadtime
	integer		ledtime
end type
...
str_crosstab	ist_Data [ ], ist_datatmp[]
...
OpenWithParm ( w_statusbar, l_str_parm )

//	Initialize by reading datawindow to structure.
w_statusbar.wf_setstatus ( "Initializing...", 0 )
...
//		w_statusbar.wf_setstatus ( "Initializing...", 100 * ll_Row / ll_RowCount )
//	end if
next

//ist_data = adw_Crosstab.object.data
w_statusbar.wf_setstatus ( "Initializing...", 100 )
...
w_statusbar.wf_setstatus ( "Finishing...", 0 )
adw_Crosstab.object.data = ist_data
w_statusbar.wf_setstatus ( "Finishing...", 100 )
...
for ll_Row = 2 to ll_Rows
	...
	if ls_Part <> ls_LastPart then
		...
		w_statusbar.wf_setstatus ( "Mapping [" + String ( ls_Part ) + "]...", 100 * ll_Row / ll_Rows )
	end if
next
...
for ll_PartRow = 1 to ll_Parts
	...
	if lds_Onhand.Retrieve ( ls_Part ) <> 0 then
		...
		if ldbl_Onhand <> 0 then
			...
			//	Process part.
			w_statusbar.wf_setstatus ( "Processing: " + ls_Part, 1 + 100 * ll_PartRow / ll_Parts )
...
//	Initialize
ll_Rows = UpperBound ( ist_Data )
w_statusbar.cb_1.enabled = false
for ll_Row = 1 to ll_Rows
// messagebox ( ist_data[ll_row].part, string( ist_data [ll_row].qnty ) +' '+ string(ist_Data [ ll_Row ].qty_assigned) +' onhand - '+string(ist_Data [ ll_Row ].onhand))	
...
//				ist_Data [ ll_Row ].qty_oh_future = ist_Data [ ll_Row ].onhand
	...
	if Int ( Mod ( 100 * ll_Row / ll_Rows, 10 ) ) = 1 then w_statusbar.wf_setstatus ( "Rewriting data to grid...", 101 * ll_Row / ll_Rows )
next
Return 0
end function
