HA$PBExportHeader$w_customerservice.srw
$PBExportComments$mcs
forward
global type w_customerservice from Window
end type
...
Choose Case a_s_item
	Case "New Customer"
		OpenSheet ( w_customer_setup_with_destinations, wmainscreen, 0, Layered! )
	...
	Case "New Quote"
		...
		OpenSheetWithParm ( w_single_quote_admin, l_str_parm1, wmainscreen, 0, Layered! )
	Case "Edit Customer"
		OpenSheetWithParm ( w_customer_setup_with_destinations, ls_customer, wmainscreen, 0, Layered! )
	Case "Edit Destination"
		ls_destination = "%%" + ls_destination
		OpenSheetWithParm ( w_customer_setup_with_destinations, ls_destination, wmainscreen, 0, Layered! )
	Case "New Sales Order"
		...
		if Message.DoubleParm = 1 then
			OpenSheetWithParm ( w_blanket_order, l_str_parm1, wMainScreen, 3, Layered! )
		elseif Message.DoubleParm = 2 then
			OpenSheetWithParm ( w_orders_detail, l_str_parm1, wMainScreen, 3, Layered! )
		end if
	Case "New RMA"
		l_str_parm1.value1 = '0'
...
// 11/20/01 added
	...
	Case "Edit Profile"
		OpenSheetWithParm ( w_orders_customer_price_matrix, ls_customer, wmainscreen, 0, Layered! )
	Case "Reset Search"
	...
	case "New Issue"
		...
		OpenSheetWithParm ( w_issue_manager, l_str_parm1, wmainscreen, 0, Layered! )
	case "New Call"
		l_str_parm1.value1 = ''
		l_str_parm1.value2 = ''
		l_str_parm1.value3 = ls_customer
...
Choose Case ls_itemtype
	...
	Case "Quotes"
		...
		OpenSheetWithParm ( w_single_quote_admin, lstr_parm, wmainscreen, 0, Layered! )
	Case "Sales Orders"
		...
		if Message.DoubleParm = 1 then
			OpenSheetWithParm ( w_blanket_order, lstr_parm, wMainScreen, 0, Layered! )
		elseif Message.DoubleParm = 2 then
			OpenSheetWithParm ( w_orders_detail, lstr_parm, wMainScreen, 0, Layered! )
		end if
	Case "Issues"
		lstr_parm.value1 = ''
		...
		OpenSheetWithParm ( w_issue_manager, lstr_parm, wmainscreen, 0, Layered! )
	Case "RMAs"
		lstr_parm.value1 = '0'
...
Choose Case uo_1.dw_1.Tag
	Case "Quotes"
		...
		l_st_Parm.form_type 			= "Quote"
		ll_row = uo_1.dw_1.GetRow ( )
		If uo_1.dw_1.IsSelected ( ll_row ) then
			l_st_Parm.document_number 	= uo_1.dw_1.GetItemNumber ( ll_row, "quote_number" )
	...
	Case "Invoices"
		...
		FOR ll_row = 1 TO uo_1.dw_1.RowCount ( )	
			If uo_1.dw_1.IsSelected ( ll_row ) Then
				l_str_parm.value1 =  String ( uo_1.dw_1.GetItemNumber ( ll_row, "invoice_number" ) )
				l_str_parm.value11 = "Invoice"
				
				IF IsValid ( w_main_screen ) THEN
					OpenSheetWithParm ( l_w_instance[UpperBound ( l_w_instance ) + 1], l_str_parm, w_main_screen, 2, Layered! )
				ELSEIF IsValid ( wmainscreen ) THEN
					OpenSheetWithParm ( l_w_instance[UpperBound ( l_w_instance ) + 1], l_str_parm, wmainscreen, 2, Layered! )
				ELSE
					OpenWithParm ( l_w_instance[UpperBound ( l_w_instance ) + 1], l_str_parm )
				END IF
			End if	
		NEXT
	Case "Issues"
...
IF Len(l_s_error_syntaxfromSQL) > 0 THEN
	// Display errors
...
ELSE
	...
	IF Len(l_s_error_create) > 0 THEN
...
if g_s_platform = 'Sybase SQL Anywhere' then
	l_s_newsyntax = SQLCA.SyntaxFromSQL("select left(rtrim(customer)+space((10-length(rtrim(customer)))),10)+' >'+name from cs_customers_vw " + a_s_where, 'Style(Type=Grid) text(border=6 background.color=12632256 font.weight=700)', l_s_error_syntaxfromSQL)
else
	l_s_newsyntax = SQLCA.SyntaxFromSQL("select left(rtrim(customer)+space((10-len(rtrim(customer)))),10)+' >'+name from cs_customers_vw " + a_s_where, 'Style(Type=Grid) text(border=6 background.color=12632256 font.weight=700)', l_s_error_syntaxfromSQL)
end if 	
...
For l_l_count = 1 to ll_custrowcount
	...
	if g_s_platform = 'Sybase SQL Anywhere' then
		l_s_newsyntax = SQLCA.SyntaxFromSQL("select left(rtrim(destination)+space((20-length(rtrim(destination)))),20)+' >'+name from destination where customer = '" + l_s_customer_temp + "'", 'Style(Type=Grid) text(border=6 background.mode=2 background.color=12632256 font.weight=700)', l_s_error_syntaxfromSQL)
	else
		l_s_newsyntax = SQLCA.SyntaxFromSQL("select left(rtrim(destination)+space((20-len(rtrim(destination)))),20)+' >'+name from destination where customer = '" + l_s_customer_temp + "'", 'Style(Type=Grid) text(border=6 background.mode=2 background.color=12632256 font.weight=700)', l_s_error_syntaxfromSQL)
	end if 	
...
//	ls_parseddata = trim(left(as_data, 10))	
end if 
...
event resize;//  Resize explorer user object
uo_1.height = height - 125
uo_1.width 	= width - 75
end event

event activate;long 	ll_rowcount
long	ll_row

//  Change menu to customer service menu if not already current
if wmainscreen.menuname <> "m_customer_service_center" then
	wmainscreen.changemenu ( m_customer_service_center )
end if
...
Choose Case a_s_tag
	Case "Quotes"
		...
		Choose Case wf_valid_quote ( l_l_quote )
			Case 1
				SetPointer ( HourGlass! )
				OpenSheetWithParm ( w_blanket_order, l_str_parm, wmainscreen, 0, Layered! )
			Case 2
				SetPointer ( HourGlass! )
				OpenSheetWithParm ( w_orders_detail, l_str_parm, wmainscreen, 0, Layered! )
			Case 3
				// cancelled
			Case else
...
// ???????
if i_i_clicked_tvrow < 1 or NOT l_tvi_item.Selected then return 1
...
Choose Case i_ds_treeview.object.itemlabel[i_i_clicked_tvrow]
	Case "Quotes"
		...
		OpenSheetWithParm ( w_single_quote_admin, l_str_parm, wmainscreen, 0, Layered! )
	Case "Sales Orders"
		l_l_data = dw_1.GetItemNumber ( row, "order_no" )
		OpenSheetWithParm ( w_decide_order, l_l_data, wmainscreen, 0, Layered! )
	Case "Ship Schedule"
		l_l_data = dw_1.GetItemNumber ( row, "id" )
		OpenSheetWithParm ( w_cs_shipping_schedule, l_l_data, wmainscreen, 0, Layered! )
	Case "Ship History"
		l_l_data = dw_1.GetItemNumber ( row, 1 )
		OpenSheetWithParm ( w_cs_ship_history, l_l_data, wmainscreen, 0, Layered! )
	Case "Issues"
		...
		OpenSheetWithParm ( w_issue_manager, l_str_parm, wmainscreen, 0, Layered! )
	Case "RMAs"
	...
	Case "Part Profile"
		...
		OpenSheetWithParm ( w_orders_customer_price_matrix, l_s_data, wmainscreen, 0, Layered! )
	Case "Notes"
	Case "Contacts"
	...
	Case "Invoices"
		...
		End if										 
		OpenSheetWithParm ( w_invoice_detail, l_str_Parm, wMainScreen, 3, layered! )
End Choose
