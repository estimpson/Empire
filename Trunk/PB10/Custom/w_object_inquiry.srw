HA$PBExportHeader$w_object_inquiry.srw
forward
...
event ue_correct;call super::ue_correct;long						l_l_serial,&
							l_l_row
...
event activate;if wMainScreen.MenuName <> "m_object_inquiry" then
	wMainScreen.ChangeMenu ( m_object_inquiry )
end if

// added the next line to set the active sheet to the instance variable of menu - mb : 1/13/99

triggerevent( "ue_activate" )
end event

event ue_add;call super::ue_add;OpenSheet ( w_manual_add_object, wMainScreen)
...
if isnull(cDeleteAuditTrail,'N') = 'Y' then 
	...
	If MessageBox ( "Audit Trail", "Do you also want to delete the audit trail for this object?", Question!, YesNo!, 2 ) = 1 Then
		...
		If SQLCA.SQLCode <> 0 Then
			RollBack ;
			wMainScreen.SetMicroHelp ( "Delete Failed! Datawindow" )
			Return
		End if
	End if
end if 
...
If Not f_create_audit_trail ( lSerial, "D",szOperator,String ( dNewOnHand ), string(dqty), string(dstdqty),"","","","","","No Commit" ) Then
	wMainScreen.SetMicroHelp ( "Delete Failed! Creating Audit Trail" )
	RollBack ;
	Return
Else
...
event dw_inquiry::retrievestart;call super::retrievestart;wMainScreen.Menuid.Dynamic mf_switch(0) // Disable buttons
end event

event dw_inquiry::retrieveend;call super::retrieveend;wMainScreen.Menuid.Dynamic mf_switch(1) // Enable buttons
end event
