HA$PBExportHeader$w_report_view.srw
forward
...
ELSE
	wmainscreen.setmicrohelp ( "Saving Datawindow..." )
END IF
end event
...
DO WHILE Len ( l_s_objects ) > 0
	Yield ( )	//Remove me after testing
	l_s_object = Left ( l_s_objects, Pos ( l_s_objects, '~t' ) - 1 )
	IF l_s_object <> "" THEN
		l_s_objects = Right ( l_s_objects, Len ( l_s_objects ) - Len ( l_s_object ) - 1 )
	ELSE
...
event activate;IF IsValid ( w_main_screen ) THEN
	IF w_main_screen.MenuName <> "m_report_view" THEN &
		w_main_screen.ChangeMenu ( m_report_view )
ELSEIF IsValid ( wMainScreen ) THEN
	IF wmainscreen.MenuName <> "m_report_view" THEN &
		wmainscreen.ChangeMenu ( m_report_view )
END IF
