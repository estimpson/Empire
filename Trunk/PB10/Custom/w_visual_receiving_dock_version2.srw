HA$PBExportHeader$w_visual_receiving_dock_version2.srw
forward
global type w_visual_receiving_dock_version2 from Window
end type
...
event inquiry;opensheet ( w_receiving_dock_inquiry, wmainscreen, 3, Layered! )
...
luo_items = CREATE n_cst_associative_array
...
if ISVALID ( u_new_recv_dock ) then
	...
	do while (	l_i_count <= u_new_recv_dock.dw_main.RowCount()	) 
		...
		l_n_TotalQty	= l_n_TotalQty + u_new_recv_dock.dw_main.object.quantity[l_i_count]
		itotalBoxes	= itotalBoxes + u_new_recv_dock.dw_main.object.boxes[l_i_count]
		l_i_count ++
	loop
	l_n_TotalQty	= l_n_TotalQty * itotalboxes															
	ntotalqty 	= l_n_TotalQty 																			
	...
	do while l_b_OK and ( l_i_row <= u_new_recv_dock.dw_main.RowCount() )
		...
		if ( nQty > 0 ) and (l_i_Boxes > 0) then
			...
			select	critical_part,
				trim(criticalpartnotes)
			into	:ls_criticalpart,
				:ls_cpnotes
			from	part_eecustom
			where	part = :szpart;
			
			if isnull(ls_criticalpart,'N') = 'Y' then
				if (Messagebox(monsys.msg_title, "Part "+szpart+" "+ls_cpnotes+" , would like to change the settings ?", Question!, YesNo!, 2)) = 1 then
			...
			if (szPartClass='P' and szqa = 'Y') then Messagebox ( monsys.msg_title, "QUALITY INSPECTION REQUIRED on Part "+szpart +", the objects being received for part " +szpart + " will be on HOLD status")
			l_i_count = 1
			...
			do while l_b_OK and ( l_i_count <= l_i_Boxes )
				...
				if l_b_OK then
					...
					l_s_SQLErrorText = SQLCA.SQLErrText																	
					Rollback ;																									
					If l_s_SQLErrorText = '' Then																																
...
IF iObjectsProcessed <= 0 Then																																			
	MessageBox ( monsys.msg_title, "No Objects Were Processed!~rTotal Processed Objects: " + String(iObjectsProcessed), Information! )	
else																																												
	MessageBox ( monsys.msg_title, "Receiving event has been completed succesfully!~rTotal Processed Objects: " + String(iObjectsProcessed), Information! )	
End IF																																											
dw_crosstab.Reset ( )
...
event ue_reverse_receipt;// Modified the event to not account for drag drop boolean checks as the drag drop 
//	funcationality has been removed.
/* 03-30-2000 KAZ Modified to update object if serial was an outside process.  Issue # 13232 */
...
if dw_recvd_objects.getselectedrow(0) > 0  then
	...
	if MessageBox ( monsys.msg_title, "Are you sure you want to delete object(s) " + l_s_List + " from inventory?", Question!, YesNo! ) = 1 then
		...
		Do while l_B_Ok and ( l_i_Count > 0 )
			...
			//			l_s_objecttype = stDelObj[itotalObjDel].object_type			// CHG 03-30-2000 KAZ
...
where	serial = :a_l_Serial and
		type   = 'R' and 
		date_stamp = ( select max(date_stamp) from audit_trail at2 where at1.serial = at2.serial and type = 'R' );
...
event activate;if wMainScreen.Menuname <> "m_receiving_dock" then &
	wMainScreen.ChangeMenu(m_receiving_dock)
...
this.Control[]={this.dw_vendors,&
this.dw_release_display,&
this.cbx_line_item,&
this.st_po_plant,&
this.st_5,&
this.cbx_retrieve,&
this.dw_objects,&
this.dw_objects_on_pallet,&
this.sle_pallet,&
this.sle_shipper,&
this.st_4,&
this.st_3,&
...
this.gb_pallet,&
this.gb_po,&
this.dw_pallet_object_info,&
this.gb_5,&
this.dw_range_weights,&
this.dw_active_po_per_vendor,&
this.st_part,&
this.dw_recvd_objects,&
this.gb_7,&
this.dw_crosstab,&
this.gb_3}
end on
...
event itemchanged;dw_active_po_per_vendor.visible = false
...
if not bPalletMode then
	...
	choose CASE dw_range_weights.Rowcount()
		...
		CASE 1
			...
			wmainscreen.SetMicroHelp ( 'Retreiving Objects...' )
			dw_recvd_objects.reset()
...
if wmainscreen.menuname = "m_receiving_dock" then
