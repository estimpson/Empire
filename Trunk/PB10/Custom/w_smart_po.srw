HA$PBExportHeader$w_smart_po.srw
forward
global type w_smart_po from Window
end type
...
If uo_print.cbx_freeze.checked then
	stParm.value4 	= 'N' 
Else
	stParm.value4	= 'Y'
End If
...
bSuccess	= FALSE		// bSuccess:Whether pass the open event successfully
OpenWithParm(w_print_preview_po, stParm)			
If bSuccess then
	w_print_preview_po.wf_get_host_window(w_smart_po)
End If
*/
end event
...
For l_l_Row = 1 to l_l_TotalRows
	...
	l_s_Temp	= Left(String(l_l_Qty) + Space(10), 10) + " $" + String(Truncate(l_n_Price,4))
	lb_price_search.AddItem(l_s_Temp)
...
l_s_date			= Left ( a_st_parm.value3, Len (a_st_parm.value3 ) - 9 ) 
l_s_time			= Right ( a_st_parm.value3, 8 )  
...
event close;If IsValid(uo_print) then CloseUserObject(uo_print)

end event

event activate;IF wMainScreen.MenuName <> 'm_smart_po' THEN &
	wMainScreen.ChangeMenu ( m_smart_po )
...
this.st_mode,&
this.dw_vendors,&
this.dw_update_future_note,&
this.dw_vendor_parts,&
this.dw_po_detail_2,&
this.dw_po_exists2,&
this.dw_price_matrix,&
this.dw_part_vendors,&
this.dw_po_row_id,&
this.dw_po_per_vendor,&
this.st_new_po,&
this.p_delete,&
this.p_receive,&
this.lb_price_search,&
this.gb_3,&
this.gb_1,&
this.dw_po_detail,&
...
event doubleclicked;OpenSheetWithParm(w_smart_price_matrix, szVendor, wMainScreen, 3, Layered!)
end event
...
If source = dw_crosstab then
	...
	If dw_po_row_id.RowCount() > 0 then 	
		...
		OpenSheetWithParm ( w_visual_receiving_dock_version2, stParm, wMainScreen, 3, Layered! )
...
If ( Row > 0) And ( li_Col > 3 ) then
	...
	Else		
		...
		OpenSheetWithParm(w_smart_po_detail_entry_version3, lst_parm, wMainScreen, 3, Layered!)
