...
char	lc_NextChar, lc_Switch
integer	li_Length, li_Ptr = 1, li_Args
string	ls_SwitchValue, ls_Arg[]
...
do while li_Length > 0
	...
	choose case lc_NextChar
		case "-", "/"
			lc_Switch = Mid ( ls_RCommand, li_Ptr + 1, 1 )
			...
			if lc_Switch = '?' then
				...
				lncst_Args.of_SetItem ( lc_Switch, ls_SwitchValue )
			end if
...
//	Add runtime to display name.
if Handle ( iapp_Object ) = 0 then
	iapp_Object.DisplayName += " -- Runtime"
end if

//	Look for connect message.
string	ls_Connect
ls_Connect = IsNull ( message.incst_Parm.of_GetItem ( "SQLCAConnect" ), "" )
if ls_Connect > "" then
...
//	Set general items.
this.of_SetHelpFile ( "" )
this.of_SetLogo ( "executor.logo.bmp" )
this.of_SetCopyright( "Copyright 2004 Fore-Thought, LLC." )
