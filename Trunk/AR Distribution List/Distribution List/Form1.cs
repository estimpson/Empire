...
namespace Distribution_List
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string[] filePaths = Directory.GetFiles(@"\\SRVDATA1\Data\ACCTNG\Accounts Receivable\Invoices\Scans\READY TO SEND", "*.pdf");

            // Make sure all invoice destination folders exist before processing invoices - CAD 08/07/2014
            if (AllDestinationFoldersExist(filePaths) == false) return;
                    ...
                    if(_values.Length == 3)
                    {
                        ...
                        if (i == "I")
                        {
                            ...
                            if (test)
                            {
                                ...
                                {
                                    File.Move(filePaths[fileCount], @"\\SRVDATA1\Data\ACCTNG\Accounts Receivable\Invoices\" + companyName + @"\" + file);
                                    freeCount++;
                                    freeSamples += file;
                                    freeSamples += "\r\n";
                                }
                                ...
                                else
                                {
                                    if (dList != null)
                                    {
                                        try
                                        {
                                            ...
                                            mail.From = new MailAddress("AccountsReceivable@empireelect.com");
                                            mail.To.Add(dList);
                                            //mail.CC.Add("apellerito@empireelect.com,rlee@empireelect.com");
                                            mail.CC.Add("AccountsReceivable@empireelect.com");
                                            Attachment data = new Attachment(@"\\SRVDATA1\Data\ACCTNG\Accounts Receivable\Invoices\" + companyName + @"\" + file);
                                            ...
                                            System.Net.NetworkCredential theCredential = new System.Net.NetworkCredential("ithelpdesk", "Pooplop99!");
                                    ...
                                    {
                                        if (File.Exists(@"\\SRVDATA1\Data\ACCTNG\Accounts Receivable\Invoices\Exceptions\" + file))
                                        {
                                            MessageBox.Show("Invoice has already been filed. " + file + " will be left in the Scans folder");
                                            duplicateCount++;
                                            duplicates += file;
                                        ...
                                        {
                                            File.Move(filePaths[fileCount], @"\\SRVDATA1\Data\ACCTNG\Accounts Receivable\Invoices\Exceptions\" + file);
                                            exceptionCount++;
                                            exceptions += file;
                                            exceptions += "\r\n";
                                        }
            ...
            try
            {
                ...
                System.Net.NetworkCredential theCredential2 = new System.Net.NetworkCredential("ithelpdesk", "Pooplop99!");
        ...
        private bool AllDestinationFoldersExist(string[] filePaths)
        {
            string file;
            string[] values;
            ...
            foreach (string filePath in filePaths)
            {
                ...
                if (file.Contains(" "))
                {
                    ...
                    if (values.Length == 3)
                    {
                        ...
                        {
                            MessageBox.Show(String.Format("An Invoice folder does not exist for {0}.  Invoice {1}.  Please correct before continuing.", companyName, file), "Alert");
                            return false;
                        }
                    }
                }
        ...
        private void btnCheckEmailAddresses_Click(object sender, EventArgs e)
        {
        ...
        private void ValidateEmailAddresses()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand(
                        "select company_name, distribution_list from ee_accounts_receivable where distribution_list != ''", con);
                cmd.Connection = con;

                rdr = cmd.ExecuteReader();
                while (rdr.Read())
                {
                    string email = rdr["distribution_list"].ToString();
                    string company = rdr["company_name"].ToString();
                    int result = ValidateEmails(email, company);
                    if (result == 0) allValid = false;
                }

                if (allValid) MessageBox.Show("All email addresses passed the initial validation check.", "Success");
            }
            catch (Exception ex)
            {
        ...
        private int ValidateEmails(string emails, string company)
        {
            if (emails.EndsWith(","))
            {
                string message = string.Format("Bad email address / list for company:  {0}.  Remove the comma at the end.", company);
                MessageBox.Show(message, "Distribution List Issue");
                return 0;
            }

            if (emails.EndsWith("."))
            {
                string message = string.Format("Bad email address / list for company:  {0}.  Remove the period at the end.", company);
                MessageBox.Show(message, "Distribution List Issue");
                return 0;
            }
            ...
            foreach (string address in addresses)
            {
                ...
                {
                    string message = string.Format("Bad email address / list for company:  {0}.", company);
                    MessageBox.Show(message, "Distribution List Issue");
                    result = 0;
                }
            }
