...
create procedure [PartTrack].[usp_GetMonitorCustomers]
as
begin
	...
	begin try
		...
		begin
			...
			,	CustomerName = c.name
			,	Address = coalesce(c.address_1, '') + coalesce( ' || ' + nullif(c.address_2, ''), '') + coalesce(' || ' + nullif(c.address_3, ''), '')
			from
				PartTrack.MONITOR_Customers c
			order by
				c.customer
		end
		--- </Body>
...
execute
	@ProcReturn = PartTrack.usp_GetMonitorCustomers
