...
create procedure [PartTrack].[usp_NewCustomerPart]
	@UserCode varchar(25)
,	@CustomerCode char(3)
--,	@BasePart char(7)
...
begin
	...
	begin try
		...
		---	<ArgumentValidation>
		/*  Customer part does not already exist (for any customer)  */
		declare @custCode char(3)
		select
			@custCode = coalesce(cp.CustomerCode, '')
		from
			PartTrack.CustomerParts cp
		where
			cp.CustomerPartNumber = @CustomerPartNumber

		if (@custCode <> '') begin
			set	@Result = 999999
			RAISERROR ('Part %s already exists for customer %s.', 16, 1, @CustomerPartNumber, @custCode)
			--return
		end
		---	</ArgumentValidation>
		...
		--- <Body>
		-- Create new base part: get the most recent part for this customer and increment it by 1, or start from 0001
		declare @NewBasePart char(7)
		select
			@NewBasePart = 
			case
				when PartList.Part <> 0 then @CustomerCode + right('0000' + (convert(varchar(4), PartList.Part + 1)), 4) 
				else @CustomerCode + '0001'
			end
		from 
			(
				select
					coalesce(max(convert(int, right(cp.BasePart, 4))), 0) as Part
				from
					PartTrack.CustomerParts cp
				where 
					left(cp.BasePart, 3) = @CustomerCode ) as PartList

			
		/*	Insert customer part. */
		set @TocMsg = 'Insert new customer part'
		begin
			--- <Insert rows="1">
