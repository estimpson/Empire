...
create procedure [PartTrack].[usp_GetCustomerParts]
@CustomerCode char(3)
as
begin
	...
	begin try
		...
		begin
			...
			,	cp.VehicleOEM
			,	cp.VehicleProgram
			,	cp.VehicleApplication
			,	cp.ModelYear
			,	cp.Description
			,	cp.ReplacedByBasePart
			,	cp.ReplacesBasePart
			,	cp.RelatedBasePart
			,	cp.CustomerRFQNumber
			,	cp.SOP
			,	cp.EAU
			from
...
execute
	@ProcReturn = PartTrack.usp_GetCustomerParts
