...
create procedure [PartTrack].[usp_GetCustomers]
as
begin
	...
	begin try
		...
		begin
			select
				c.CustomerCode
			,	c.Name
			,	MonitorCustomerCode = c2.customer
			,	MonitorCustomerName = c2.name
			,	MonitorAddress = coalesce(c2.address_1, '') + coalesce( ' || ' + nullif(c2.address_2, ''), '') + coalesce(' || ' + nullif(c2.address_3, ''), '')
			,	c.RowID
			from
...
execute
	@ProcReturn = PartTrack.usp_GetCustomers
