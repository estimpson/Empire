...
create procedure [PartTrack].[usp_SetCustomerPart]
	@UserCode varchar(25)
,	@CustomerCode char(3)
,	@BasePart char(7)
...
,	@ReplacedByBasePart char(7)
,	@ReplacesBasePart char(7)
,	@RelatedBasePart char(7)
...
begin
	...
	begin try
		...
		set @TocMsg = 'Update customer part'
		begin
			--- <Update rows="1">
			...
			,	cp.VehicleOEM = @VehicleOEM
			,	cp.VehicleProgram = @VehicleProgram
			,	cp.VehicleApplication = @VehicleApplication
			,	cp.ModelYear = @ModelYear
			,	cp.Description = @Description
			,	cp.ReplacedByBasePart = @ReplacedByBasePart
			,	cp.ReplacesBasePart = @ReplacesBasePart
			,	cp.RelatedBasePart = @RelatedBasePart
			,	cp.CustomerRFQNumber = @CustomerRFQNumber
			,	cp.SOP = @SOP
			,	cp.EAU = @EAU
			,	cp.RowModifiedDT = @TranDT
			,	cp.RowModifiedUser = @UserCode
			from
				PartTrack.CustomerParts cp
			where
				cp.CustomerCode = @CustomerCode
				and cp.BasePart = @BasePart
