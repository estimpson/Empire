...
CREATE procedure [NSA].[usp_SetLogistics]
	@QuoteNumber varchar(100)
,	@EmpireFacility varchar(20)
,	@FreightTerms varchar(100)
,	@CustomerShipTo varchar(20) 
...
begin
	...
	begin try
		...
		--- <Body>
		/*	Create new logistics entry if needed. */
		set @TocMsg = 'Create new logistics entry if needed'
		if	not exists
			(	select
					*
				from
		...
		begin
			...
			set @TocMsg = 'Update logistics entry'
			...
			update
				aql
			set
				aql.EmpireFacility = @EmpireFacility
			,	aql.FreightTerms = @FreightTerms
			from
				NSA.AwardedQuoteLogistics aql
			where
				aql.QuoteNumber = @QuoteNumber
		...
		/*	Create new ship to entry if needed. */
		set @TocMsg = 'Create new ship to entry if needed'
		if	not exists
			(	select
					*
				from
		...
		begin
			...
			/*	Update ship to entry. */
			set @TocMsg = 'Update ship to entry'
			...
			update
				aqst
			set
				aqst.CustomerShipTo = @CustomerShipTo
			from
				NSA.AwardedQuoteShipTos aqst
			where
				aqst.QuoteNumber = @QuoteNumber
...
execute
	@ProcReturn = NSA.usp_SetHardTooling
