...
CREATE FUNCTION [EDIVARROCic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIVARROCic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIVARROCic.PlanningHeaders ph
				JOIN EDIVARROCic.PlanningReleases pr
		...
		JOIN EDIVARROCic.PlanningHeaders ph
			JOIN EDIVARROCic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIVARROCic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
