...
CREATE FUNCTION [EDIMagnaTrim].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIMagnaTrim.PlanningHeaders ph
				JOIN EDIMagnaTrim.PlanningReleases pr
		...
		JOIN EDIMagnaTrim.PlanningHeaders ph
			JOIN EDIMagnaTrim.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIMagnaTrim.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
