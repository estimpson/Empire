...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [EDIFnGMex].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIFnGMex.PlanningHeaders ph
				JOIN EDIFnGMex.PlanningReleases pr
		...
		JOIN EDIFnGMex.PlanningHeaders ph
			JOIN EDIFnGMex.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIFnGMex.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
