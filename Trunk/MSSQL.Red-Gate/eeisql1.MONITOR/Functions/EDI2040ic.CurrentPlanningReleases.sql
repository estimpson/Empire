...
CREATE FUNCTION [EDI2040ic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI2040ic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI2040ic.PlanningHeaders ph
				JOIN EDI2040ic.PlanningReleases pr
		...
		JOIN EDI2040ic.PlanningHeaders ph
			JOIN EDI2040ic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI2040ic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
