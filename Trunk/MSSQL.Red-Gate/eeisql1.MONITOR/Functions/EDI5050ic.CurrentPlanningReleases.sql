...
CREATE FUNCTION [EDI5050ic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI5050ic.PlanningHeaders ph
				JOIN EDI5050ic.PlanningReleases pr
		...
		JOIN EDI5050ic.PlanningHeaders ph
			JOIN EDI5050ic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI5050ic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
