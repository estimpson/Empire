...
CREATE FUNCTION [EDICarPlastics].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDICarPlastics.PlanningHeaders ph
				JOIN EDICarPlastics.PlanningReleases pr
		...
		JOIN EDICarPlastics.PlanningHeaders ph
			JOIN EDICarPlastics.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDICarPlastics.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
