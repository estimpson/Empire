...
CREATE FUNCTION [EDIValsylv].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIValsylv.PlanningHeaders ph
				JOIN EDIValsylv.PlanningReleases pr
		...
		JOIN EDIValsylv.PlanningHeaders ph
			JOIN EDIValsylv.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIValsylv.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
