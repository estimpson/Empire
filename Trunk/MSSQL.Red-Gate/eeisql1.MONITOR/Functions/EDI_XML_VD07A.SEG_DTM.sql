...
CREATE function [EDI_XML_VD07A].[SEG_DTM]
(	@dateID varchar(5)
,	@shipDate datetime
...
--- <Body>
	...
	set	@xmlOutput =
		(	select
				EDI_XML.SEG_INFO('00D07A', 'DTM')
			,	EDI_XML_VD07A.CE('C507', convert(xml, convert(varchar(max), EDI_XML_VD07A.DE('2005', @DateID)) + convert(varchar(max), EDI_XML_VD07A.DE('2380', EDI_XML.FormatDateTime(@shipDate, @xmlDTFormat))) + convert(varchar(max), EDI_XML_VD07A.DE('2379', @dateFormat))))
