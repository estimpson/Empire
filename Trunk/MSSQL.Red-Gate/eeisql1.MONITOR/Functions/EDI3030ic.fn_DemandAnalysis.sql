...
CREATE FUNCTION [EDI3030ic].[fn_DemandAnalysis] ( )
...
SELECT 'SS', ssh.TradingPartner, css.RawDocumentGUID, css.ReleaseNo, css.ShipToCode, css.CustomerPart, css.CustomerPO, ssh.RowCreateDT
 FROM 
	EDI3030ic.CurrentShipSchedules() css
JOIN
	EDI3030ic.ShipScheduleHeaders ssh ON ssh.RawDocumentGUID = css.RawDocumentGUID
...
SELECT 'PR', ph.TradingPartner, cpr.RawDocumentGUID, cpr.ReleaseNo, cpr.ShipToCode, cpr.CustomerPart, cpr.CustomerPO, ph.RowCreateDT
 FROM 
	EDI3030ic.CurrentPlanningReleases() cpr
JOIN
	EDI3030ic.PlanningHeaders ph ON ph.RawDocumentGUID = cpr.RawDocumentGUID
	ORDER BY 6
	...
	SELECT 
		...
		JOIN
			...
			JOIN
			EDI3030ic.PlanningReleases pr ON pr.RawDocumentGUID = ls.RawDocGUID 
			AND pr.CustomerPart = ls.CustomerPart AND
					pr.CustomerPO = ls.CustomerPO AND
                    pr.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDI3030ic.PlanningAccums pa ON pa.RawDocumentGUID = ls.RawDocGUID 
		...
		JOIN
			...
			lst.SchedType = ls.SchedType
			JOIN
			EDI3030ic.ShipSchedules ss ON ss.RawDocumentGUID = ls.RawDocGUID 
			AND ss.CustomerPart = ls.CustomerPart AND
					ss.CustomerPO = ls.CustomerPO AND
                    ss.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDI3030ic.ShipScheduleAccums ssa ON ssa.RawDocumentGUID = ls.RawDocGUID 
