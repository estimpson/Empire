...
CREATE function [EDINorplas].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ssh.Status = 0 --(select dbo.udf_StatusValue('EDINorplas.ShipScheduleHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				...
				join EDINorplas.ShipSchedules ss
					on ss.RawDocumentGUID = ssh.RawDocumentGUID
			where
				ssh.Status in
				(	0 --(select dbo.udf_StatusValue('EDINorplas.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDINorplas.ShipScheduleHeaders', 'Status', 'Active'))
				)
			group by
				ss.ShipToCode
		...
		join EDINorplas.ShipScheduleHeaders ssh
			join EDINorplas.ShipSchedules ss
				on ss.RawDocumentGUID = ssh.RawDocumentGUID
