...
CREATE FUNCTION [EDI3030ic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3030ic.PlanningHeaders ph
				JOIN EDI3030ic.PlanningReleases pr
		...
		JOIN EDI3030ic.PlanningHeaders ph
			JOIN EDI3030ic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3030ic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
