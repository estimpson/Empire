...
CREATE function [EDINascote].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ssh.Status = 0 --(select dbo.udf_StatusValue('EDINascote.ShipScheduleHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				...
				join EDINascote.ShipSchedules ss
					on ss.RawDocumentGUID = ssh.RawDocumentGUID
			where
				ssh.Status in
				(	0 --(select dbo.udf_StatusValue('EDINascote.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDINascote.ShipScheduleHeaders', 'Status', 'Active'))
				)
			group by
				ss.ShipToCode
		...
		join EDINascote.ShipScheduleHeaders ssh
			join EDINascote.ShipSchedules ss
				on ss.RawDocumentGUID = ssh.RawDocumentGUID
