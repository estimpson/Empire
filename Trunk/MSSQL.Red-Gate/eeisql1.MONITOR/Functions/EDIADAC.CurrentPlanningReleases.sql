...
CREATE FUNCTION [EDIADAC].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIADAC.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIADAC.PlanningHeaders ph
				JOIN EDIADAC.PlanningReleases pr
		...
		JOIN EDIADAC.PlanningHeaders ph
			JOIN EDIADAC.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIADAC.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
