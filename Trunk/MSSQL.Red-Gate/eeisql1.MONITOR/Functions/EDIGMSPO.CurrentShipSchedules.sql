...
CREATE function [EDIGMSPO].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ssh.Status = 0 --(select dbo.udf_StatusValue('EDIGMSPO.ShipScheduleHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				...
				join EDIGMSPO.ShipSchedules ss
					on ss.RawDocumentGUID = ssh.RawDocumentGUID
			where
				ssh.Status in
				(	0 --(select dbo.udf_StatusValue('EDIGMSPO.ShipScheduleHeaders', 'Status', 'New'))
				,	1 --(select dbo.udf_StatusValue('EDIGMSPO.ShipScheduleHeaders', 'Status', 'Active'))
				)
			group by
				ss.ShipToCode
		...
		join EDIGMSPO.ShipScheduleHeaders ssh
			join EDIGMSPO.ShipSchedules ss
				on ss.RawDocumentGUID = ssh.RawDocumentGUID
