...
CREATE FUNCTION [EDIVIST].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIVIST.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIVIST.PlanningHeaders ph
				JOIN EDIVIST.PlanningReleases pr
		...
		JOIN EDIVIST.PlanningHeaders ph
			JOIN EDIVIST.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIVIST.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
