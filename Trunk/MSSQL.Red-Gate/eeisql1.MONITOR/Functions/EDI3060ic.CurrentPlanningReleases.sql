...
CREATE FUNCTION [EDI3060ic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3060ic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3060ic.PlanningHeaders ph
				JOIN EDI3060ic.PlanningReleases pr
		...
		JOIN EDI3060ic.PlanningHeaders ph
			JOIN EDI3060ic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3060ic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
