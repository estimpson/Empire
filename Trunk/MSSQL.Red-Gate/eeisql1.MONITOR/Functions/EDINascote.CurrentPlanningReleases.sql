...
CREATE function [EDINascote].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ph.Status = 0 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				EDINascote.PlanningHeaders ph
				join EDINascote.PlanningReleases pr
		...
		join EDINascote.PlanningHeaders ph
			join EDINascote.PlanningReleases pr
				on pr.RawDocumentGUID = ph.RawDocumentGUID
