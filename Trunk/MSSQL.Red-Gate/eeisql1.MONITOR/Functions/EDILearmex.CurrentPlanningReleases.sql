...
CREATE FUNCTION [EDILearmex].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDILearmex.PlanningHeaders ph
				JOIN EDILearmex.PlanningReleases pr
		...
		JOIN EDILearmex.PlanningHeaders ph
			JOIN EDILearmex.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDILearmex.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
