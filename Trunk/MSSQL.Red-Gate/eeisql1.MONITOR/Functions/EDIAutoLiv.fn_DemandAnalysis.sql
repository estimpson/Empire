...
CREATE FUNCTION [EDIAutoLiv].[fn_DemandAnalysis] ( )
...
SELECT 'SS', ssh.TradingPartner, css.RawDocumentGUID, css.ReleaseNo, css.ShipToCode, css.CustomerPart, css.CustomerPO, ssh.RowCreateDT
 FROM 
	EDIAutoliv.CurrentShipSchedules() css
JOIN
	EDIAutoliv.ShipScheduleHeaders ssh ON ssh.RawDocumentGUID = css.RawDocumentGUID
...
SELECT 'PR', ph.TradingPartner, cpr.RawDocumentGUID, cpr.ReleaseNo, cpr.ShipToCode, cpr.CustomerPart, cpr.CustomerPO, ph.RowCreateDT
 FROM 
	EDIAutoliv.CurrentPlanningReleases() cpr
JOIN
	EDIAutoliv.PlanningHeaders ph ON ph.RawDocumentGUID = cpr.RawDocumentGUID
	ORDER BY 6
	...
	SELECT 
		...
		JOIN
			...
			JOIN
			EDIAutoliv.PlanningReleases pr ON pr.RawDocumentGUID = ls.RawDocGUID 
			AND pr.CustomerPart = ls.CustomerPart AND
					pr.CustomerPO = ls.CustomerPO AND
                    pr.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDIAutoliv.PlanningAccums pa ON pa.RawDocumentGUID = ls.RawDocGUID 
		...
		JOIN
			...
			lst.SchedType = ls.SchedType
			JOIN
			EDIAutoliv.ShipSchedules ss ON ss.RawDocumentGUID = ls.RawDocGUID 
			AND ss.CustomerPart = ls.CustomerPart AND
					ss.CustomerPO = ls.CustomerPO AND
                    ss.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDIAutoliv.ShipScheduleAccums ssa ON ssa.RawDocumentGUID = ls.RawDocGUID 
