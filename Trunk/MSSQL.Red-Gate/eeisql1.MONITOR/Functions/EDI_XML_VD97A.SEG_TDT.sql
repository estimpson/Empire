...
CREATE function [EDI_XML_VD97A].[SEG_TDT]
...
--- <Body>
	...
	set	@xmlOutput =
		(	select
			...
			,	EDI_XML_TRW_ASN.CE('C040', convert(xml, convert(varchar(max), EDI_XML_VD97A.DE('3127', @carrierID)) + convert(varchar(max), EDI_XML_VD97A.DE('1131', @clQualifier)) + convert(varchar(max), EDI_XML_VD97A.DE('3055', @clResponsible)) + convert(varchar(max), EDI_XML_VD97A.DE('3128', @CarrierName)) ))
