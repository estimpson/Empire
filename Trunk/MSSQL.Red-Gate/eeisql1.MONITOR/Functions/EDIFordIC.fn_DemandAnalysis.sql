...
CREATE FUNCTION [EDIFordIC].[fn_DemandAnalysis] ( )
...
SELECT 'SS', ssh.TradingPartner, css.RawDocumentGUID, css.ReleaseNo, css.ShipToCode, css.CustomerPart, css.CustomerPO, ssh.RowCreateDT
 FROM 
	EDIFordIC.CurrentShipSchedules() css
JOIN
	EDIFordIC.ShipScheduleHeaders ssh ON ssh.RawDocumentGUID = css.RawDocumentGUID
...
SELECT 'PR', ph.TradingPartner, cpr.RawDocumentGUID, cpr.ReleaseNo, cpr.ShipToCode, cpr.CustomerPart, cpr.CustomerPO, ph.RowCreateDT
 FROM 
	EDIFordIC.CurrentPlanningReleases() cpr
JOIN
	...
	SELECT 
		...
		JOIN
			...
			JOIN
			EDIFordIC.PlanningReleases pr ON pr.RawDocumentGUID = ls.RawDocGUID 
			AND pr.CustomerPart = ls.CustomerPart AND
					pr.CustomerPO = ls.CustomerPO AND
                    pr.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDIFordIC.PlanningAccums pa ON pa.RawDocumentGUID = ls.RawDocGUID 
		...
		JOIN
			...
			lst.SchedType = ls.SchedType
			JOIN
			EDIFordIC.ShipSchedules ss ON ss.RawDocumentGUID = ls.RawDocGUID 
			AND ss.CustomerPart = ls.CustomerPart AND
					ss.CustomerPO = ls.CustomerPO AND
                    ss.ShipToCode = ls.ShipToCode
			LEFT JOIN
			EDIFordIC.ShipScheduleAccums ssa ON ssa.RawDocumentGUID = ls.RawDocGUID 
