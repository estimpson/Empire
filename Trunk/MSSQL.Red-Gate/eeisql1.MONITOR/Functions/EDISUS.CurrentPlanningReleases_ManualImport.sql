...
CREATE FUNCTION [EDISUS].[CurrentPlanningReleases_ManualImport]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDISUS.PlanningReleases_ManualImport pr
		...
		JOIN EDISUS.PlanningHeaders_ManualImport ph
			JOIN EDISUS.PlanningReleases_ManualImport pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			--LEFT JOIN
			--EDISUS.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
