...
CREATE function [EDIVSL].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ssh.Status = 0 --(select dbo.udf_StatusValue('EDIVSL.ShipScheduleHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		...
		join EDIVSL.ShipScheduleHeaders ssh
			join EDIVSL.ShipSchedules ss
				on ss.RawDocumentGUID = ssh.RawDocumentGUID
