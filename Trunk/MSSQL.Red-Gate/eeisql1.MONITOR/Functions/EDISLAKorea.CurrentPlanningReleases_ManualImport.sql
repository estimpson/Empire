...
create FUNCTION [EDISLAKorea].[CurrentPlanningReleases_ManualImport]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDISLAKorea.PlanningHeaders_ManualImport ph
				JOIN EDISLAKorea.PlanningReleases_ManualImport pr
		...
		JOIN EDISLAKorea.PlanningHeaders_ManualImport ph
			JOIN EDISLAKorea.PlanningReleases_ManualImport pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			--LEFT JOIN
			--EDISLAKorea.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
