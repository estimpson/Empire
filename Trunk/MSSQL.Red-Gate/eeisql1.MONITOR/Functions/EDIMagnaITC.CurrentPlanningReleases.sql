...
CREATE FUNCTION [EDIMagnaITC].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIMagnaITC.PlanningHeaders ph
				JOIN EDIMagnaITC.PlanningReleases pr
		...
		JOIN EDIMagnaITC.PlanningHeaders ph
			JOIN EDIMagnaITC.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIMagnaITC.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
