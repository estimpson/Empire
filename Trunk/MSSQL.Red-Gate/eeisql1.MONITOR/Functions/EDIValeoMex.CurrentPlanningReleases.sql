...
CREATE FUNCTION [EDIValeoMex].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIValeoMex.PlanningHeaders ph
				JOIN EDIValeoMex.PlanningReleases pr
		...
		JOIN EDIValeoMex.PlanningHeaders ph
			JOIN EDIValeoMex.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIValeoMex.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
