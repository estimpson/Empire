...
CREATE function [EDILiteTek].[CurrentShipSchedules]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ssh.Status = 0 --(select dbo.udf_StatusValue('EDILiteTek.ShipScheduleHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				...
				join EDILiteTek.ShipSchedules ss
		...
		join EDILiteTek.ShipScheduleHeaders ssh
			join EDILiteTek.ShipSchedules ss
				on ss.RawDocumentGUID = ssh.RawDocumentGUID
			...
			and	(	convert(char(10), ss.ReleaseNo)
					+ convert(char(20), ssh.DocumentDT, 120)
					+ convert(char(10), ssh.DocNumber)
					+ convert(char(10), ssh.ControlNumber)
					+ convert(char(20), ssh.DocumentImportDT, 120)
