...
CREATE FUNCTION [EDINAL].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDINAL.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				...
				JOIN EDINAL.PlanningReleases pr
		...
		JOIN EDINAL.PlanningHeaders ph
			JOIN EDINAL.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDINAL.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
