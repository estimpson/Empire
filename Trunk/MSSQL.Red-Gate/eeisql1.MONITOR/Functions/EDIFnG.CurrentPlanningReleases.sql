...
CREATE function [EDIFnG].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ph.Status = 0 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				EDIFnG.PlanningHeaders ph
				join EDIFnG.PlanningReleases pr
		...
		join EDIFnG.PlanningHeaders ph
			join EDIFnG.PlanningReleases pr
				on pr.RawDocumentGUID = ph.RawDocumentGUID
