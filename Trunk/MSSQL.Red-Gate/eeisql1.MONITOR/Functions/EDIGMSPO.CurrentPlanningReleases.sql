...
CREATE function [EDIGMSPO].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			case
				when ph.Status = 0 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'New'))
					then 1
				else 0
			end
	from
		(	select
			...
			from
				EDIGMSPO.PlanningHeaders ph
				join EDIGMSPO.PlanningReleases pr
		...
		join EDIGMSPO.PlanningHeaders ph
			join EDIGMSPO.PlanningReleases pr
				on pr.RawDocumentGUID = ph.RawDocumentGUID
			on 
