...
CREATE FUNCTION [EDITRWSAF].[CurrentPlanningReleases_ManualImport]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDITRWSAF.PlanningHeaders_ManualImport ph
				JOIN EDITRWSAF.PlanningReleases_ManualImport pr
		...
		JOIN EDITRWSAF.PlanningHeaders_ManualImport ph
			JOIN EDITRWSAF.PlanningReleases_ManualImport pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			--LEFT JOIN
			--EDITRWSAF.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
