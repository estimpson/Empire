...
CREATE FUNCTION [EDIPLEX].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIPLEX.PlanningHeaders ph
				JOIN EDIPLEX.PlanningReleases pr
		...
		JOIN EDIPLEX.PlanningHeaders ph
			JOIN EDIPLEX.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIPLEX.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
