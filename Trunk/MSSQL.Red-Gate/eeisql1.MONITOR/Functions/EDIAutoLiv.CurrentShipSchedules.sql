...
CREATE FUNCTION [EDIAutoLiv].[CurrentShipSchedules]
()
...
JOIN
		EDIAutoLiv.ShipSchedules Alss ON ALss.CustomerPart = CheckFirst.CustomerPart
...
LEFT JOIN
	...
	,	NewDocument =
			CASE
				WHEN ssh.Status = 0 --(select dbo.udf_StatusValue('EDIAutoLiv.ShipScheduleHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		...
		JOIN EDIAutoLiv.ShipScheduleHeaders ssh
			JOIN EDIAutoLiv.ShipSchedules ss
			...
			Select RANNumber from EDIAutoLiv.CanceledRANs
