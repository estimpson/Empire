...
CREATE FUNCTION [EDISLA].[CurrentPlanningReleases_ManualImport]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDISLA.PlanningHeaders_ManualImport ph
				JOIN EDISLA.PlanningReleases_ManualImport pr
		...
		JOIN EDISLA.PlanningHeaders_ManualImport ph
			JOIN EDISLA.PlanningReleases_ManualImport pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			--LEFT JOIN
			--EDISLA.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
