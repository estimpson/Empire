...
CREATE FUNCTION [EDI3040ic].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDI3040ic.PlanningHeaders ph
				JOIN EDI3040ic.PlanningReleases pr
		...
		JOIN EDI3040ic.PlanningHeaders ph
			JOIN EDI3040ic.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDI3040ic.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
