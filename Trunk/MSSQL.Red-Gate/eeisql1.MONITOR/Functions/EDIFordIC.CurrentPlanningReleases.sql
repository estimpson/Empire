...
CREATE FUNCTION [EDIFordIC].[CurrentPlanningReleases]
()
...
--- <Body>
	...
	,	NewDocument =
			CASE
				WHEN ph.Status = 0 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'New'))
					THEN 1
				ELSE 0
			END
	FROM
		(	SELECT
			...
			FROM
				EDIFordIC.PlanningHeaders ph
				JOIN EDIFordIC.PlanningReleases pr
		...
		JOIN EDIFordIC.PlanningHeaders ph
			JOIN EDIFordIC.PlanningReleases pr
				ON pr.RawDocumentGUID = ph.RawDocumentGUID
			ON 
			...
			LEFT JOIN
			EDIFordIC.BlanketOrders bo ON bo.EDIShipToCode = pr.ShipToCode
