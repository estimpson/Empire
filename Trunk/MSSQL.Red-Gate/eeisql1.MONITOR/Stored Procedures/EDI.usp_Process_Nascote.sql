...
CREATE procedure [EDI].[usp_Process_Nascote]
	@TranDT datetime out
,	@Result integer out
,	@Testing int = 1
--<Debug>
...
,	fr.CustomerPart
,	case
		when fr.status = 0 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'New'))
			then 1
		else 0
	end
from
	EDI.Nascote_830_Releases fr
	join EDI.Nascote_830_Headers fh
		on fh.RawDocumentGUID = fr.RawDocumentGUID
where
	fr.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'Active'))
	)
	and fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'Active'))
	)
	and fh.DocumentDT =
	(	select
			max(fh2.DocumentDT)
		from
			EDI.Nascote_830_Headers fh2
			join EDI.Nascote_830_Releases fr2
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'Replaced'))
		end
from
	EDI.Nascote_830_Releases fr
...
from
	EDI.Nascote_830_Cumulatives fr
	left join @Current830s c
		on fr.RawDocumentGUID = c.RawDocumentGUID
		and fr.ShipToCode = c.ShipToCode
		and fr.CustomerPart = c.CustomerPart
		...
		when exists
			(	select
				...
				where
					...
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'Replaced'))
	end
from
	EDI.Nascote_830_Headers fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'Active'))
...
,	Type = 2
,	ReleaseDT = [FT].[fn_ShipDate]('wk', bo.ShipDay ,dateadd(dd, coalesce(bo.OverrideTransitDays,bo.TransitDays)*-1 ,fr.ReleaseDT))
...
from
	EDI.Nascote_830_Headers fh
	join EDI.Nascote_830_Releases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI.Nascote_830_Cumulatives dcv
	...
	join NascoteEDI.BlanketOrders bo
	...
	left join @Current830s c
		...
		and fr.CustomerPart = c.CustomerPart
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDI.Nascote_830_Headers', 'Status', 'Active'))
	and fr.Status = 1--(select dbo.udf_StatusValue('EDI.Nascote_830_Releases', 'Status', 'Active'))
	and	dcv.status = 1
	...
	[Notes] [varchar](5000) NULL
		...
		if exists (	select 1 
						from 
							@Current830s F830c
						join	
							EDI.Nascote_830_Releases F830r on F830c.RawDocumentGUID = F830r.RawDocumentGUID and NewDocument = 1
						left join 
							NascoteEDI.BlanketOrders bo	on bo.EDIShipToCode in (F830r.ICCode, F830r.ShipToCode)
						and 
		...
		from (	select
						...
						join	
							EDI.Nascote_830_Releases F830r on F830c.RawDocumentGUID = F830r.RawDocumentGUID and NewDocument = 1
						left join 
							NascoteEDI.BlanketOrders bo	on bo.EDIShipToCode =  F830r.ShipToCode
						and 
							bo.CustomerPart = F830r.CustomerPart
						and (case when bo.CheckCustomerPO = 1 then bo.CustomerPO else '-1' end ) = (case when bo.CheckCustomerPO = 1 then F830r.CustomerPO else '-1' end )
						where 
...
SET @tableHTML =
    N'<H1>Nascote EDI Exceptions</H1>' +
    N'<table border="1">' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @subject = N'EDI Data Exception when processing Nascote EDI Document(s)', -- nvarchar(255)
    ...
    @importance = 'High' -- varchar(6)
		...
		-- E-Mail Alerts for Accum discrepancy
			...
			if exists (
						...
						from 
							@Current830s F830c
						join	
							EDI.Nascote_830_Cumulatives F830r on F830c.RawDocumentGUID = F830r.RawDocumentGUID and NewDocument = 1
						left join 
							NascoteEDI.BlanketOrders bo	on bo.EDIShipToCode =  F830r.ShipToCode
						and 
							bo.CustomerPart = F830r.CustomerPart
		...
		from (select
						(select max(ReleaseNo) from EDI.Nascote_830_Releases SPOR where SPOR.RawDocumentGUID = F830r.RawDocumentGUID and SPOR.CustomerPart = F830r.CustomerPart  ) ReleaseNo, 
						F830r.CustomerPart CustomerPart,
						F830r.CustomerPO CustomerPO,
						F830r.ShipToCode ShipToID,
...
SET @tableHTMLAA =
    N'<H1>Nascote Accum Discrepancy</H1>' +
    N'<table border="1">' +
    N'<tr><th>ReleaseNo</th>' +
    N'<th>CustomerPart</th><th>CustomerPO</th><th>ShipToCode</th>' +
    N'<th>Notes</th></tr>' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max)
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - Nascote EDI Document(s) Accum Dicrepancy', -- nvarchar(255)
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max)
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - Nascote EDI Document(s) Updated FX Sales Orders', -- nvarchar(255)
...
execute
	@ProcReturn = EDI.usp_Process_Nascote
	@TranDT = @TranDT out
