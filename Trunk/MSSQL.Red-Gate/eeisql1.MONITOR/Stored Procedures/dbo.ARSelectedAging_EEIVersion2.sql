...
CREATE procedure [dbo].[ARSelectedAging_EEIVersion2]
  ...
  */
      insert into [dbo].ar_customer_aging
        select ar_headers.document_type,
          ...
          as_of_date=getdate(),
          select_date=getdate(),
          isNull(ar_headers.amount,0),
          IsNull(SUM(ar_applications.applied_amount),0),
          DateDiff(day,ar_headers.due_date,getdate()),
          DateDiff(day,ar_headers.document_date,getdate()),
          ...
          and isNull(ar_headers.amount,0)<>0
  ...
  update [dbo].ar_customer_aging set
    ...
    =IsNULL((select SUM(IsNull(ar_applications.exchanged_amount*-1,0))
      ...
      and convert(char(10),ar_applications.applied_date,111)<=convert(char(10),getdate(),111)),0)
  ...
  */
  insert into [dbo].ar_customer_aging
    select 'A',
      ...
      0,
      DateDiff(day,bank_register.document_date,getdate()),
      ...
      and exists(select 1 from bank_register as br_voids
        ...
        and convert(char(10),br_voids.gl_date,111)  >convert(char(10),getdate(),111))))
      and(ar_applications.applied_date is null
      or convert(char(10),ar_applications.applied_date,111) <=convert(char(10),getdate(),111))
      and ar_applications.application_type not in('DISCNT','ADJUST','WRTOFF')
      and convert(char(10),bank_register.gl_date,111) <=convert(char(10),getdate(),111)
...
select  document_type ,
        ...
        as_of_date = getdate() ,
        select_date = getdate() ,
        ARHeadersAmount = isNull(ar_headers.amount, 0) ,
        DaysPastDue = DateDiff(day, ar_headers.due_date, getdate()) ,
        DaysPastDoc = DateDiff(day, ar_headers.document_date, getdate()) ,
        ( select    Min(ar_customers.customer_name)
          from      dbo.ar_customers
          where     customer = ar_headers.bill_customer
        ) as CutomerName ,
        ar_headers.exchanged_amount ,
        ar_headers.contract_po ,
        Sort1 = '' ,
        ...
        ar_headers.ee_overdue_reason ,
        ar_headers.ee_comment ,
        past_due_weight = null
into    #ARHeaders
from    ar_headers
where   convert(char(10), ar_headers.gl_date, 111) <= convert(char(10), getdate(), 111)
        and ar_headers.gl_date > '2005-01-01'
        and isNull(ar_headers.amount, 0) <> 0
        and ar_headers.intercompany <> 'Y'
        and intercompany != 'Y'
        and ( ar_headers.bill_unit in( '11','60') )
        and ( ar_headers.ledger_account_code in( '121011', '121060', '121260') )
/*select		* from #ARHeaders
order by 4 desc	
*/
select  document_type ,
        document ,
        SUM(offset_exchanged_amount*-1) as AAExchangedAmount,
        sum(applied_amount) as AAAppliedAmount
into    #ARApplications
from    dbo.ar_applications
where   convert(char(10), ar_applications.applied_date, 111) <= convert(char(10), getdate(), 111)
        and ar_applications.application_type <> 'OVERPY'
group by document_type ,
        document
        
--select * from #ARApplications
       
 select  document_type ,
        document ,
       	sum(ar_applications.offset_exchanged_amount)*-1 as CMOffsetExchangedAmount
into    #ARApplicationsCM
from    dbo.ar_applications
where   convert(char(10), ar_applications.applied_date, 111) <= convert(char(10), getdate(), 111)
        and ar_applications.application_type <> 'CHECK'
       and	document_type = 'C' 
group by document_type ,
 ...
 --select * from #ARApplicationsCM
 
 select  document_type ,
        document ,
		sum(ar_applications.offset_exchanged_amount)*-1 as CHKOffsetExchangedAmount
into    #ARApplicationsCHK
from    dbo.ar_applications
where   convert(char(10), ar_applications.applied_date, 111) <= convert(char(10), getdate(), 111)
        and ar_applications.application_type = 'CHECK'
       and	document_type = 'C' 
group by document_type ,
 ...
 -- select * from #ARApplicationsCHK


                                        
                                        
select  document_type = 'A' ,
        document =  LTrim(convert(char(25),bank_register.document_id3)) ,
        Bill_unit = bank_register.document_id1,
		BillCustomer = bank_register.document_id2,
		due_Date = bank_register.document_date,
        bank_register.document_date ,
        bank_register.gl_date ,
        bank_register.offset_ledger_account_code ,
        as_of_date = getdate() ,
        select_date = getdate() ,
        BankRegisterChkAmount = isNull(bank_register.application_check_amount*-1, 0) ,
        DaysPastDue = 0 ,
        DaysPastDoc = DateDiff(day, bank_register.document_date, getdate()) ,
        ( select    Min(ar_customers.customer_name)
          from      dbo.ar_customers
          where     customer = bank_register.document_id2
        ) as CutomerName ,
        BankRegisterExchangedAmount = bank_register.exchanged_amount*-1 
        
into    #ARHeadersBNK
from    dbo.bank_register
where   bank_register.document_class='AR'
      and bank_register.check_void_nsf='C'
      and(bank_register.document_type<>'N'
      or(bank_register.document_type='N'
      and exists(select 1 from bank_register as br_voids
        ...
        and convert(char(10),br_voids.gl_date,111)  >convert(char(10),getdate(),111))))
         and convert(char(10),bank_register.gl_date,111) <=convert(char(10),getdate(),111)
		and bank_register.document_id2<>'NON-AR'
		and(bank_register.document_id1 in ('11','60'))
		and(bank_register.offset_ledger_account_code in ('121011', '121060') )

-- select * from #ARHeadersBNK

 select  bill_customer ,
        check_number ,
		sum(ar_applications.offset_exchanged_amount)*-1 as CHKOffsetExchangedAmount,
		sum(ar_applications.applied_amount)*-1 as BNKAppliedAmount
into    #ARApplicationsBNK
from    dbo.ar_applications
where   (ar_applications.applied_date is null or convert(char(10),ar_applications.applied_date,111) <=convert(char(10),getdate(),111))
      and ar_applications.application_type not in ('DISCNT','ADJUST','WRTOFF')
group by bill_customer ,
        check_number 
 
-- select * from #ARApplicationsBNK


insert	dbo.ar_customer_aging
        ( document_type ,
          document ,
          unit ,
          customer ,
...
select  AH.document_type ,
        ...
        select_date ,
        ARHeadersAmount ,
		AppliedAmount =  coalesce(AAAppliedAmount ,0),
        DaysPastDue ,
        DaysPastDoc ,
        CutomerName ,
        exchanged_amount ,
		exchanged_applied_amount =  coalesce(AA.AAExchangedAmount,0) + coalesce(ACM.CMOffsetExchangedAmount,0)+coalesce(ACM.CMOffsetExchangedAmount,0),
        contract_po ,
        ledger_account_code ,
        Sort2 ,
        ee_scheduled_date ,
        ...
        past_due_weight
from    #ARHeaders AH
        left join #ARApplications AA on AH.document = AA.document
                                        and AH.document_type = AA.document_type
       left join #ARApplicationsCM ACM on AH.document = ACM.document
                                        and AH.document_type = ACM.document_type
        left join #ARApplicationsCHK ACK on AH.document = ACK.document
                                        and AH.document_type = ACK.document_type
   
--order by bill_customer
...
select  AH.document_type ,
        ...
        document_date ,
        gl_date ,
        offset_ledger_account_code ,
        as_of_date ,
        select_date ,
        BankRegisterChkAmount ,
		AppliedAmount =  coalesce(BNKAppliedAmount ,0),
        DaysPastDue ,
        DaysPastDoc ,
        CutomerName ,
        BankRegisterExchangedAmount,
		exchanged_applied_amount =  coalesce(AA. CHKOffsetExchangedAmount,0) ,
       '' ,
      offset_ledger_account_code ,
        '' ,
       '' ,
       ...
       '' ,
        Null
from    #ARHeadersBNK AH
        left join #ARApplicationsBNK AA on AH.document = AA.check_number
                                        and AH.BillCustomer = AA.bill_customer
       
 where bill_customer is not null
 
 
 
select	* from ar_customer_aging order by 6 desc, 4 asc, 2 asc
