...
,	BeginDT = getdate()
,	InArguments =
		'@shipper = ' + coalesce(convert(varchar, @shipper), '<null>')
		+ ', @serial = ' + coalesce(convert(varchar, @serial), '<null>')
		+ ', @parent_serial = ' + coalesce(convert(varchar, @parent_serial), '<null>')
		+ ', @create_sd = ' + coalesce('''' + @create_sd + '''', '<null>')
		+ ', @pkg_override = ' + coalesce('''' + @pkg_override + '''', '<null>')
