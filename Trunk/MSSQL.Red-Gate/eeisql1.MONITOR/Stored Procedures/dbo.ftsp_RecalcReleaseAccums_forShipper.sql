...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[ftsp_RecalcReleaseAccums_forShipper]
(
	@ShipperID int
)
as
...
where
	oh.order_type = 'B'
	and
		oh.customer_part + ':' + oh.destination in
		(	select
...
set	RelPrior = coalesce (
	(	select
		...
		where
			...
			and
				ShipToID = Releases.ShipToID
			and
				ReleaseDT < Releases.ReleaseDT), 0)
from
	@Releases Releases

update
	@Releases
set	RelPost = RelPrior + QtyRelease
