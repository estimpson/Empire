...
CREATE PROCEDURE [dbo].[FTsp_varroc_asn_detail]  (@shipper integer)
as

BEGIN

--[dbo].[FTsp_varroc_asn_detail] 47841
...
CREATE TABLE #atr_package_type_serials_group
   ...
   CREATE TABLE #atr_objects
	...
	SELECT 	edi_setups.prev_cum_in_asn,
			...
			customerPO =  case when customer_po like '%~%'
										then
										substring(shipper_detail.customer_po, 1, patindex('%~%', shipper_detail.customer_po)-1)
										else
										customer_po
										end ,
			CustomerPoLine = case when customer_po like '%~%'
										then
										substring(shipper_detail.customer_po, patindex('%~%', shipper_detail.customer_po)+1 ,6)
										else
										customer_po
										end ,
