...
CREATE procedure [EDIValeoMex].[usp_Process]
...
,   NewDocument
from
	EDIValeoMex.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIValeoMex.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIValeoMex.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIValeoMex.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIValeoMex.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIValeoMex.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIValeoMex.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIValeoMex.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIValeoMex.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIValeoMex.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIValeoMex.ShipScheduleHeaders fh
...
from
	EDIValeoMex.ShipScheduleHeaders fh
	join EDIValeoMex.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIValeoMex.ShipScheduleAccums fa
	...
	left join EDIValeoMex.ShipScheduleAuthAccums faa
	...
	join EDIValeoMex.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIValeoMex.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIValeoMex.PlanningHeaders fh
	join EDIValeoMex.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIValeoMex.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIValeoMex.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIValeoMex.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIValeoMex.PlanningHeaders fh
					join EDIValeoMex.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIValeoMex.ShipScheduleHeaders fh
					join EDIValeoMex.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIValeoMex.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIValeoMex.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIValeoMex.ShipSchedules b
 Join 
	EDIValeoMex.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
and not exists
( Select 1 from 
		EDIValeoMex.PlanningReleases b
 Join 
	EDIValeoMex.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
	b.ShipToCode = bo.EDIShipToCode
...
from
	@Current862s a
	 Join 
	EDIValeoMex.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIValeoMex.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIValeoMex.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIValeoMex.PlanningAccums pra on 
 	...
 	left join
		EDIValeoMex.PlanningAuthAccums praa on 
...
Select 
		...
		,	@EmailHeader nvarchar(max) = 'EDI Processing for EDIValeoMex' 
...
execute
	@ProcReturn = EDIValeoMex.usp_Process
	@TranDT = @TranDT out
