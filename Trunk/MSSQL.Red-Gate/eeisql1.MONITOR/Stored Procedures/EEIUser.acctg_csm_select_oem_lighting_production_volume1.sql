...
SET ANSI_NULLS ON
GO
CREATE procedure [EEIUser].[acctg_csm_select_oem_lighting_production_volume1] 
as
...
-- exec eeiuser.acctg_csm_select_oem_lighting_production_volume1
...
select	 b.[VP: Manufacturer Group] 
		...
		,d.base_part
		,convert(decimal(12,0),avg( d.[Empire_volume_2019] )) as Empire_Volume_2019
		,convert(decimal(12,0),sum( b.[Vehicle Volume 2019] )) as Vehicle_Volume_2019
		,convert(decimal(12,0),(select sum(c.[vehicle volume 2019]) from eeiuser.acctg_csm_forward_lighting c where c.[VP: Manufacturer Group] = b.[VP: Manufacturer Group] group by c.[VP: Manufacturer Group])) as oem_sort_index
		,convert(decimal(12,0),(select sum(a.[vehicle volume 2019]) from eeiuser.acctg_csm_forward_lighting a where a.[VP: Manufacturer Group] = b.[VP: Manufacturer Group] and a.[Headlamp Supplier] = b.[Headlamp Supplier] Group by a.[Headlamp Supplier])) as supplier_sort_index
...
where b.[Vehicle Volume 2019]>0 and base_part is not null
...
(case when isnull(vehicle_volume_2019,0) < isnull(empire_volume_2019,0) then 0 else isnull(vehicle_volume_2019,0)-isnull(empire_volume_2019,0) end) as unknown_volume,
...
where (case when isnull(vehicle_volume_2019,0) < isnull(empire_volume_2019,0) then 0 else isnull(vehicle_volume_2019,0)-isnull(empire_volume_2019,0) end) > 0
