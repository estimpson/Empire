...
CREATE 	PROCEDURE [FT].[sp_report_SchedulingWeekOverWeekDemandAnalysis]
...
--Exec [FT].[sp_report_SchedulingWeekOverWeekDemandAnalysis]  '2017-01-11', 30
--Declare variables
DECLARE
...
--select * from #DemandSnapShotFlat
 ...
 SELECT [AnalysisDT]
     ...
     ,[Week12QtyPriorWeek]
	  ...
	  , abs([TotalQtyAnalysisWeek]-[TotalQtyPriorWeek]) as TotalQtyDifference
      , case when ([TotalQtyAnalysisWeek]-[TotalQtyPriorWeek])>0 then 'Increase'
			 ...
			 Else 'NoChange'
			 END as TotalChange				 
	  , abs([FutureVariance]) as TotalAbsVariance
	   ,HIS.*
