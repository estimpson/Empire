...
,   NewDocument
from
	EDIStanley.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIStanley.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIStanley.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIStanley.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIStanley.ShipSchedules', 'Status', 'Replaced'))
		end
from
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIStanley.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'Replaced'))
		end
from
	...
	case
		when exists
			(	select
					*
				from
					EDIStanley.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders', 'Status', 'Replaced'))
	end
from
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIStanley.ShipScheduleHeaders fh
...
from
	EDIStanley.ShipScheduleHeaders fh
	join EDIStanley.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIStanley.ShipScheduleAccums fa
	...
	left join EDIStanley.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	...
	join EDIStanley.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIStanley.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIStanley.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIStanley.PlanningHeaders fh
					join EDIStanley.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIStanley.ShipScheduleHeaders fh
					join EDIStanley.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIStanley.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIStanley.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIStanley.ShipSchedules b
 Join 
	EDIStanley.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIStanley.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIStanley.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIStanley.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIStanley.PlanningAccums pra on 
 	...
 	left join
		EDIStanley.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIStanley' 
...
execute
	@ProcReturn = EDIStanley.usp_Process
	@TranDT = @TranDT out
