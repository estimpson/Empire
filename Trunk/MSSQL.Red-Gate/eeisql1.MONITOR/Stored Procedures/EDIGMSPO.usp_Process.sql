...
CREATE procedure [EDIGMSPO].[usp_Process]
...
,   NewDocument
from
	EDIGMSPO.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIGMSPO.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIGMSPO.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIGMSPO.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIGMSPO.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIGMSPO.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIGMSPO.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIGMSPO.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIGMSPO.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIGMSPO.ShipScheduleHeaders fh
...
from
	EDIGMSPO.ShipScheduleHeaders fh
	join EDIGMSPO.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIGMSPO.ShipScheduleAccums fa
	...
	left join EDIGMSPO.ShipScheduleAuthAccums faa
	...
	join EDIGMSPO.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIGMSPO.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIGMSPO.PlanningHeaders fh
	join EDIGMSPO.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIGMSPO.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIGMSPO.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIGMSPO.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIGMSPO.PlanningHeaders fh
					join EDIGMSPO.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIGMSPO.ShipScheduleHeaders fh
					join EDIGMSPO.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIGMSPO.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIGMSPO.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIGMSPO.ShipSchedules b
 Join 
	EDIGMSPO.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIGMSPO.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIGMSPO.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIGMSPO.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIGMSPO.PlanningAccums pra on 
 	...
 	left join
		EDIGMSPO.PlanningAuthAccums praa on 
...
Select 
		...
		,	@EmailHeader nvarchar(max) = 'EDI Processing for EDIGMSPO' 
	...
	,		@recipients = 'mminth@empireelect.com,@EmailAddress' -- varchar(max)
...
execute
	@ProcReturn = EDIGMSPO.usp_Process
	@TranDT = @TranDT out
