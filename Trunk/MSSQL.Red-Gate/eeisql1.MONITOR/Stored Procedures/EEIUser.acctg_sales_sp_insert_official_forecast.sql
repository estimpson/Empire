...
-- select distinct(forecast_name), time_stamp from eeiuser.sales_1 where time_stamp >= '2019-04-01' order by time_stamp desc

-- delete from eeiuser.sales_1 where forecast_name = '2018/08 OSF' and time_stamp = '2018-10-28 18:00:00'

-- exec eeiuser.acctg_sales_sp_insert_official_forecast '2019/05 OSF', '2019-04-22 18:00:00'


CREATE procedure [EEIUser].[acctg_sales_sp_insert_official_forecast]
as

declare @forecast_name varchar(50), @time_stamp datetime

select @forecast_name = (select max(release_id) from eeiuser.acctg_csm_naihs) + ' OSF'
select @time_stamp = ft.fn_truncdate('ss',getdate())

--for testing
--select @forecast_name, @time_stamp
...
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','1', sum(Jan_21_TOTALdemand), case sum(Jan_21_Sales) when 0 then 0 else Sum(Jan_21_Sales)/sum(Jan_21_TOTALdemand) end, Sum(Jan_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','2', sum(Feb_21_TOTALdemand), case sum(Feb_21_Sales) when 0 then 0 else Sum(Feb_21_Sales)/sum(Feb_21_TOTALdemand) end, Sum(Feb_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','3', sum(Mar_21_TOTALdemand), case sum(Mar_21_Sales) when 0 then 0 else Sum(Mar_21_Sales)/sum(Mar_21_TOTALdemand) end, Sum(Mar_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','4', sum(Apr_21_TOTALdemand), case sum(Apr_21_Sales) when 0 then 0 else Sum(Apr_21_Sales)/sum(Apr_21_TOTALdemand) end, Sum(Apr_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','5', sum(May_21_TOTALdemand), case sum(May_21_Sales) when 0 then 0 else Sum(May_21_Sales)/sum(May_21_TOTALdemand) end, Sum(May_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','6', sum(Jun_21_TOTALdemand), case sum(Jun_21_Sales) when 0 then 0 else Sum(Jun_21_Sales)/sum(Jun_21_TOTALdemand) end, Sum(Jun_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','7', sum(Jul_21_TOTALdemand), case sum(Jul_21_Sales) when 0 then 0 else Sum(Jul_21_Sales)/sum(Jul_21_TOTALdemand) end, Sum(Jul_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','8', sum(Aug_21_TOTALdemand), case sum(Aug_21_Sales) when 0 then 0 else Sum(Aug_21_Sales)/sum(Aug_21_TOTALdemand) end, Sum(Aug_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','9', sum(Sep_21_TOTALdemand), case sum(Sep_21_Sales) when 0 then 0 else Sum(Sep_21_Sales)/sum(Sep_21_TOTALdemand) end, Sum(Sep_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','10', sum(Oct_21_TOTALdemand), case sum(Oct_21_Sales) when 0 then 0 else Sum(Oct_21_Sales)/sum(Oct_21_TOTALdemand) end, Sum(Oct_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','11', sum(Nov_21_TOTALdemand), case sum(Nov_21_Sales) when 0 then 0 else Sum(Nov_21_Sales)/sum(Nov_21_TOTALdemand) end, Sum(Nov_21_Sales) from @b group by customer, base_part
union
select @forecast_name, @time_stamp, NULL, customer, base_part, '2021','12', sum(Dec_21_TOTALdemand), case sum(Dec_21_Sales) when 0 then 0 else Sum(Dec_21_Sales)/sum(Dec_21_TOTALdemand) end, Sum(Dec_21_Sales) from @b group by customer, base_part
