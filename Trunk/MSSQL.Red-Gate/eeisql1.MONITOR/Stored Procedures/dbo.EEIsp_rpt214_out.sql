...
CREATE procedure [dbo].[EEIsp_rpt214_out] (	@fromdate datetime, @throughdate datetime )
as
Begin
	...
	from		audit_trail
		...
		where	type in ( 'S', 'D', 'V') and status <> 'S'  and
				...
				serial not in (select serial from audit_trail where part like 'ALC%' and from_loc = 'ELPASO' and type = 'S')
