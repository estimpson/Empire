...
CREATE procedure [EEIUser].[acctg_budget_annual] (@budget_id varchar(50), @organization varchar(3), @fiscal_year varchar(5), @user_id varchar(50))
as
declare @period1 int
,@period2 int
,@period3 int
...
select distinct(ledger_account) from eeiuser.acctg_budget where budget_id = @budget_id and fiscal_year = @fiscal_year and right(ledger_account,2) in (select organization from #selected_organizations)
union
select distinct(ledger_account) from gl_cost_transactions where ledger = 'EMPIRE' and fiscal_year = @fiscal_year and update_balances = 'Y' and len(ledger_account)=6 and right(ledger_account,2) in (select organization from #selected_organizations) and left(ledger_account,4) > '5019'
end
else
begin
insert into #authorized_ledger_accounts
select ledger_account from eeiuser.acctg_budget_groups where authorized_user = @user_id and budget_id = @budget_id and len(ledger_account)=6 and right(ledger_account,2) in (select organization from #selected_organizations) and left(ledger_account,4) > '5019'
end

insert into #a (budget_line, budget_id, ledger_account, cashflow, budget_description) 
select distinct(budget_line),budget_id, ledger_account, cashflow, budget_description from eeiuser.acctg_budget where budget_id = @budget_id and fiscal_year = @fiscal_year and ledger_account in (select ledger_account from #authorized_ledger_accounts)
insert into #a (ledger_account, budget_line, budget_id, cashflow, budget_description)
select distinct(ledger_account), ledger_account+'-9999', @budget_id, '?', 'Unbudgeted' from gl_cost_transactions where ledger = 'EMPIRE' and fiscal_year = @fiscal_year and update_balances = 'Y' and ledger_account in (select ledger_account from #authorized_ledger_accounts) and ledger_account not in (select distinct(ledger_account) from #a)
