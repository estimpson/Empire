...
CREATE PROCEDURE [EDIValsylv].[usp_Process]
...
,   NewDocument
FROM
	EDIValsylv.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIValsylv.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIValsylv.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIValsylv.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIValsylv.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIValsylv.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIValsylv.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIValsylv.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIValsylv.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIValsylv.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIValsylv.ShipScheduleHeaders fh
...
from
	EDIValsylv.ShipScheduleHeaders fh
	join EDIValsylv.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIValsylv.ShipScheduleAccums fa
	...
	left join EDIValsylv.ShipScheduleAuthAccums faa
	...
	join EDIValsylv.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.ShipScheduleHeaders', 'Status', 'Active'))
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIValsylv.PlanningHeaders fh
	join EDIValsylv.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIValsylv.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIValsylv.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
	...
	and not exists
		(	select
				*
			from
				EDIValsylv.PlanningReleases ss
			where
				ss.RawDocumentGUID = fr.RawDocumentGUID
				and ss.CustomerPart = fr.CustomerPart
				and ss.ShipToCode = fr.ShipToCode
		...
		fh.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningHeaders', 'Status', 'Active'))
	and fr.Status = 1 --(select dbo.udf_StatusValue('EDIValsylv.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIValsylv.PlanningHeaders fh
					join EDIValsylv.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIValsylv.ShipScheduleHeaders fh
					join EDIValsylv.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIValsylv.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIValsylv.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIValsylv.ShipSchedules b
 Join 
	EDIValsylv.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIValsylv.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIValsylv.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIValsylv.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIValsylv.PlanningAccums pra on 
 	...
 	left join
		EDIValsylv.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for Valeo Sylvania' 
...
execute
	@ProcReturn = EDIValsylv.usp_Process
	@TranDT = @TranDT out
