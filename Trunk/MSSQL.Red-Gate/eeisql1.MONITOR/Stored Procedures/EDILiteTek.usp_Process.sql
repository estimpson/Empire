...
CREATE procedure [EDILiteTek].[usp_Process]
...
,	NewDocument
from
	EDILiteTek.CurrentShipSchedules ()
...
,	NewDocument
from
	EDILiteTek.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDILiteTek.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDILiteTek.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDILiteTek.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDILiteTek.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Active'))
	)
			...
			when exists
				(	select
					...
					where
						...
						and ss.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Active')
				) then 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningHeaders', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDILiteTek.PlanningHeaders', 'Status', 'Replaced'))
		end
from
	EDILiteTek.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDILiteTek.ShipScheduleHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDILiteTek.ShipScheduleHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDILiteTek.PlanningReleases fr
		...
		when exists
			(	select
				...
				where
					...
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDILiteTek.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDILiteTek.PlanningHeaders fh
where
	fh.Status in
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDILiteTek.ShipScheduleHeaders fh
...
from
	EDILiteTek.ShipScheduleHeaders fh
	join EDILiteTek.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDILiteTek.ShipScheduleAccums fa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.ShipScheduleHeaders', 'Status', 'Active'))
	and fr.Status = 1 
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.ShipScheduleHeaders', 'Status', 'Active'))
	and fr.Status = 1 
order by 1,2,3*/
/*		830s. */
/*union all*/
...
from
	EDILiteTek.PlanningHeaders fh
	join EDILiteTek.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
...
where
	...
	and not exists
		(	select
				*
			from
				EDILiteTek.PlanningReleases ss
			where
				ss.RawDocumentGUID = fr.RawDocumentGUID
				and ss.CustomerPart = fr.CustomerPart
				and ss.ShipToCode = fr.ShipToCode
		...
		fh.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningHeaders', 'Status', 'Active'))
	and fr.Status = 1 --(select dbo.udf_StatusValue('EDILiteTek.PlanningReleases', 'Status', 'Active'))
	and	bo.ActiveOrder = 1
order by
	1, 2, 3

	--Select * from TestDecofinmexreleasesPreAdjust from @RawReleases
...
where
	...
	--Select * from TestDecofinmexreleasesPostAdjust from @RawReleases
	...
	Notes varchar(255) NULL
		...
		if exists (Select 1 
						from
							@Current862s C830
						join
							EDILiteTek.ShipSchedules F862r on C830.RawDocumentGUID = F862r.RawDocumentGUID and C830.NewDocument = 1
						and
							C830.CustomerPart = F862r.CustomerPart
						and
							C830.ShipToCode = F862r.ShipToCode
						Join 
							EDILiteTek.BlanketOrders bo on C830.CustomerPart = bo.CustomerPart
						and
						...
						from
							@Current830s C830
						join
							EDILiteTek.PlanningReleases F862r on C830.RawDocumentGUID = F862r.RawDocumentGUID and C830.NewDocument = 1
						and
							C830.CustomerPart = F862r.CustomerPart 
						and
							C830.ShipToCode = F862r.ShipToCode
		...
		from (select	'Ship Schedule' DocumentType,
					...
					select
						...
						from
							@Current830s C830
						join
							EDILiteTek.PlanningReleases F830r on C830.RawDocumentGUID = F830r.RawDocumentGUID and C830.NewDocument = 1
						and
...
SET @tableHTML =
	N'<H1>Decofinmex EDI Exceptions</H1>' +
	N'<table border="1">' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
	...
	@subject = N'EDI Data Exception when processing Decofinmex EDI Document(s)', -- nvarchar(255)
...
insert	@EDIOrdersAlert
		...
		from (/*select	distinct
						...
						from
							(Select * From EDILiteTek.CurrentShipSchedules()) C830
						join
							EDILiteTek.ShipScheduleAccums F862r on C830.RawDocumentGUID = F862r.RawDocumentGUID and C830.NewDocument = 1
						and
							C830.CustomerPart = F862r.CustomerPart
						and
							C830.ShipToCode = F862r.ShipToCode
					...
					select
						...
						from 
							@Current830s F830c
						join	
							EDILiteTek.PlanningAccums F830r on F830c.RawDocumentGUID = F830r.RawDocumentGUID and NewDocument = 1
						left join 
							EDILiteTek.BlanketOrders bo	on bo.EDIShipToCode in (F830r.ConsigneeCode, F830r.ShipToCode)
						and 
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
	...
	@copy_recipients = 'dwest@empireelect.com', -- varchar(max)
	--@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
	@subject = N'EDI Data Alert Decofinmex EDI Document(s) Updated FX Sales Orders', -- nvarchar(255)
...
SET @tableHTMLZ =
	N'<H1>Decofinmex Accum Alert</H1>' +
	N'<table border="1">' +
	N'<tr><th>Document Type</th>' +
	N'<th>ReleaseNo</th>' +
	N'<th>CustomerPart</th><th>CustomerPO</th><th>ShipToCode</th>' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
	...
	@copy_recipients = 'dwest@empireelect.com', -- varchar(max)
	--@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
	@subject = N'EDI Data Alert Decofinmex EDI Document(s) - Verify Accums with Updated FX Sales Orders', -- nvarchar(255)
