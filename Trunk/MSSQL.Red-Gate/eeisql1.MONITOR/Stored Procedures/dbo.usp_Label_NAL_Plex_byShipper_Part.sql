...
CREATE procedure [dbo].[usp_Label_NAL_Plex_byShipper_Part] ( @ShipperID int, @Part varchar(25))

as Begin
Set nocount on
--exec dbo.usp_Label_NAL_Plex_byShipper_Part 93585, 'NAL0158'

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[NALlabelDataByShipper_Part]') AND type in (N'U'))
DROP TABLE [dbo].[NALlabelDataByShipper_Part]
...
Select *
into
 NALlabelDataByShipper_part
From
	@SerialsToPrint stp
left join
	@LabelData ld on ld.BlanketPart = stp.Part
	
	--Select * From NALLabelDataByShipper_Part
