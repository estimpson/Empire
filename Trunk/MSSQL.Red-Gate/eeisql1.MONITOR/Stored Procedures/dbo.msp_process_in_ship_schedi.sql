...
create procedure [dbo].[msp_process_in_ship_schedi]
as
---------------------------------------------------------------------------------------
...
--	20. If previous order was valid, calculate committed quantity for the previous order.
				if @prevorderno > 0
				begin -- (5B)
					exec	@returncode = msp_calculate_committed_qtyi
							@prevorderno,
							null,
							null
...
--	34. Release is valid, get the next sequence and rowid for the new release.
				select	@sequence = isnull ( (
						select	max ( sequence )
						  from	order_detail_inserted as od
