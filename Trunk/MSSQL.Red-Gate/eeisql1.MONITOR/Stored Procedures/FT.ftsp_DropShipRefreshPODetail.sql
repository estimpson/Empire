...
create procedure [FT].[ftsp_DropShipRefreshPODetail]
...
,	price =
	(	select
		...
		where
			po_header.blanket_part = part_vendor_price_matrix.part
			and po_header.vendor_code = part_vendor_price_matrix.vendor
			and part_vendor_price_matrix.break_qty <= order_detail.quantity
	)
,	row_id =
	(	select
			count(1) + 1
...
from
	...
	join po_header
		on order_detail.part_number = po_header.blanket_part
	join part
		on po_header.blanket_part = part.part
	join part_vendor
		on po_header.blanket_part = part_vendor.part
			and po_header.vendor_code = part_vendor.vendor
where
	order_detail.ship_type = 'D'
	and order_header.cs_status = 'Approved'
	and isnull(order_detail.status,'O') != 'H'
