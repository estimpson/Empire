...
CREATE procedure [EDIMagnaTrim].[usp_Process]
...
,   NewDocument
from
	EDIMagnaTrim.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIMagnaTrim.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIMagnaTrim.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIMagnaTrim.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIMagnaTrim.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIMagnaTrim.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIMagnaTrim.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIMagnaTrim.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIMagnaTrim.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIMagnaTrim.ShipScheduleHeaders fh
...
from
	EDIMagnaTrim.ShipScheduleHeaders fh
	join EDIMagnaTrim.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIMagnaTrim.ShipScheduleAccums fa
	...
	left join EDIMagnaTrim.ShipScheduleAuthAccums faa
	...
	join EDIMagnaTrim.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIMagnaTrim.PlanningHeaders fh
	join EDIMagnaTrim.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIMagnaTrim.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIMagnaTrim.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaTrim.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIMagnaTrim.PlanningHeaders fh
					join EDIMagnaTrim.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIMagnaTrim.ShipScheduleHeaders fh
					join EDIMagnaTrim.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIMagnaTrim.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIMagnaTrim.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIMagnaTrim.ShipSchedules b
 Join 
	EDIMagnaTrim.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
and not exists
( Select 1 from 
		EDIMagnaTrim.PlanningReleases b
 Join 
	EDIMagnaTrim.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
	b.ShipToCode = bo.EDIShipToCode
...
from
	@Current862s a
	 Join 
	EDIMagnaTrim.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIMagnaTrim.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIMagnaTrim.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIMagnaTrim.PlanningAccums pra on 
 	...
 	left join
		EDIMagnaTrim.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIMagnaTrim' 
...
execute
	@ProcReturn = EDIMagnaTrim.usp_Process
	@TranDT = @TranDT out
