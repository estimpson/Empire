...
CREATE procedure [EEIUser].[usp_ST_Report_Hitlist_MSF_ProgramParam]
...
,	isnull(convert(char(4), year(eop)), '') as eop
,	parent_customer
,	empire_application
...
where
	parent_customer in (	
		...
		where
			sf.SalesForecastParentCustomer is not null )
