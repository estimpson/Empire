...
CREATE procedure [EEIUser].[usp_ST_Report_Hitlist_MSF]
...
if (@SOPYear is null) begin
	...
	-- Headlamps, LED (PCB)
	with cte_head_pcb (Vehicle, Applications, BaseParts, SopYears)
	as
	(
		--select
	...
	-- Headlamps, Harness
	cte_head_harness (Vehicle, Applications, BaseParts, SopYears)
	as
	(
		select
			sfd.Vehicle as Vehicle
	...
	-- Taillamps, LED (PCB)
	with cte_tail_pcb (Vehicle, Applications, BaseParts, SopYears)
	as
	(
		select
			sfd.Vehicle as Vehicle
	...
	-- Taillamps, Harness
	cte_tail_harness (Vehicle, Applications, BaseParts, SopYears)
	as
	(
		select
			sfd.Vehicle as Vehicle
