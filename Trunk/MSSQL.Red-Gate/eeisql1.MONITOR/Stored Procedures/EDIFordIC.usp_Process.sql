...
CREATE PROCEDURE [EDIFordIC].[usp_Process]
...
,   NewDocument
FROM
	EDIFordIC.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIFordIC.CurrentPlanningReleases ()
...
END
--- <Update rows="*">
set	@TableName = 'EDIFordIC.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFordIC.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFordIC.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIFordIC.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFordIC.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIFordIC.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIFordIC.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDIFordIC.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDIFordIC.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFordIC.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIFordIC.ShipScheduleHeaders fh
...
from
	EDIFordIC.ShipScheduleHeaders fh
	join EDIFordIC.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIFordIC.ShipScheduleAccums fa
	...
	left join EDIFordIC.ShipScheduleAuthAccums faa
	...
	join EDIFordIC.BlanketOrders bo
...
and	fh.Status = 1 --(select dbo.udf_StatusValue('EDIFordIC.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIFordIC.PlanningHeaders fh
	join EDIFordIC.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIFordIC.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIFordIC.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
				c.RawDocumentGUID = fr.RawDocumentGUID
	and	fh.Status = 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIFordIC.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIFordIC.PlanningHeaders fh
					join EDIFordIC.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIFordIC.ShipScheduleHeaders fh
					join EDIFordIC.ShipScheduleSupplemental sss
	...
	--- <Update rows="*">
		...
		join
		  ...
		  join
				EDIFordIC.PlanningSupplemental prs
		on
...
update
		...
		on
				bo.BlanketOrderNo = oh.order_no
		join
				(Select * From EDIFordIC.CurrentPlanningReleases()) cpr 
			on
				cpr.CustomerPart = bo.customerpart and
				cpr.ShipToCode = bo.EDIShipToCode and
				(	bo.CheckCustomerPOPlanning = 0
		...
		JOIN
			EDIFordIC.PlanningHeaders ph ON ph.RawDocumentGUID = cpr.RawDocumentGUID
		WHERE
		...
		on
				bo.BlanketOrderNo = oh.order_no
		join
				(Select * From EDIFordIC.CurrentShipSchedules()) css 
			on
		  ...
		  join
				EDIFordIC.ShipScheduleSupplemental sss
		on
	...
	/*update
		...
		from
			...
			join 
				EDIFordIC.ShipScheduleHeaders fh
			join 
				EDIFordIC.ShipSchedules ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIFordIC.BlanketOrders bo	on bo.EDIShipToCode =  ps.ShipToCode
					and bo.CustomerPart = ps.CustomerPart
...
and not exists
( Select 1 from 
		EDIFordIC.ShipSchedules b
 Join 
	EDIFordIC.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIFordIC.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIFordIC.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIFordIC.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIFordIC.PlanningAccums pra on 
 	...
 	left join
		EDIFordIC.PlanningAuthAccums praa on 
...
execute
	@ProcReturn = EDIFordIC.usp_Process
	@TranDT = @TranDT out
