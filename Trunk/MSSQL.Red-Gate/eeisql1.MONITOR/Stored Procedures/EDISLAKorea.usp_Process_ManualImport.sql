...
CREATE procedure [EDISLAKorea].[usp_Process_ManualImport]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDISLAKorea.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDISLAKorea.CurrentPlanningReleases_ManualImport()
...
--- <Update rows="*">
set	@TableName = 'EDISLAKorea.PlanningReleases_ManualImport'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'Replaced'))
		end
from
	EDISLAKorea.PlanningReleases_ManualImport PR
...
where
	PR.Status in
	(	0 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDISLAKorea.PlanningHeaders_ManualImport'
	...
	case
		when exists
			(	select
					*
				from
					EDISLAKorea.PlanningReleases_ManualImport fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders_ManualImport', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders_ManualImport', 'Status', 'Replaced'))
	end
from
	EDISLAKorea.PlanningHeaders_ManualImport fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders_ManualImport', 'Status', 'Active'))
	)
...
from
	EDISLAKorea.PlanningHeaders_ManualImport fh
	join EDISLAKorea.PlanningReleases_ManualImport fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
		/*
	left join EDISLAKorea.PlanningAccums fa
	...
	left join EDISLAKorea.PlanningAuthAccums faa
		...
		*/
	left join EDISLAKorea.BlanketOrders bo
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningHeaders_ManualImport', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDISLAKorea.PlanningReleases_ManualImport', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	from
		@RawReleases rr
		join EDISLAKorea.BlanketOrders bo
...
and not exists
( Select 1 from 
		EDISLAKorea.PlanningReleases_ManualImport b
 Join 
	EDISLAKorea.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
	 Join 
	EDISLAKorea.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDISLAKorea.PlanningAccums pra on 
 	...
 	left join
		EDISLAKorea.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDISLAKorea' 
...
execute
	@ProcReturn = EDISLAKorea.usp_Process
	@TranDT = @TranDT out
