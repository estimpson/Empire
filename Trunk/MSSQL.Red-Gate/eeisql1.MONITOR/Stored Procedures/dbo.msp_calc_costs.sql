	...
	begin -- (3b)
		...
		if @cost_bucket='S'
			...
			begin
				...
				select @burden = (((1.0/isnull(@parts_per_hour,1.0)) + (isnull(@setup_time,0.0)/isnull(@mfg_lot_size,1))) * isnull(@standard_rate_mc,0) * isnull(@crew_size,1)) 
				if @varying_mc>0
					select @burden = isnull(@burden,0) + (((1.0/isnull(@parts_per_hour,1.0)) + (isnull(@setup_time,0.0)/isnull(@mfg_lot_size,1))) * isnull(@varying_mc,0) * isnull(@crew_size,1))
				if @indirect_mc>0
					select @burden = isnull(@burden,0) + (((1.0/isnull(@parts_per_hour,1.0)) + (isnull(@setup_time,0.0)/isnull(@mfg_lot_size,1))) * isnull(@indirect_mc,0) * isnull(@crew_size,1))
				if @sga_mc>0
					select @burden = isnull(@burden,0) + (((1.0/isnull(@parts_per_hour,1.0)) + (isnull(@setup_time,0.0)/isnull(@mfg_lot_size,1))) * isnull(@sga_mc,0) * isnull(@crew_size,1))
			end -- ()
			else
			begin
				...
				select @burden=((1.0/isnull(@parts_per_hour,1.0))* isnull(@standard_rate_mc,0) * isnull(@crew_size,1))
				if @varying_mc>0
					select @burden = isnull(@burden,0) + ((1.0/isnull(@parts_per_hour,1.0)) * isnull(@varying_mc,0) * isnull(@crew_size,1))
				if @indirect_mc>0
					select @burden = isnull(@burden,0) + ((1.0/isnull(@parts_per_hour,1.0)) * isnull(@indirect_mc,0) * isnull(@crew_size,1))
				if @sga_mc>0
					select @burden = isnull(@burden,0) + ((1.0/isnull(@parts_per_hour,1.0)) * isnull(@sga_mc,0) * isnull(@crew_size,1))
			end
	--Allow Obsolete to be rolled up as well as 'M' *******Andre S. Boulanger Empire Electronics, Inc. 2006-09-14
		if @parttype = 'M' or @parttype = 'O'
			...
			if ((select a.flow_route_window 
				from	activity_codes a
					join part_machine b on b.activity = a.code and
						b.sequence = 1
