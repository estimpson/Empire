...
CREATE procedure [EDIStanley].[usp_Process_ManualImport]
...
,   NewDocument
from
	EDIStanley.CurrentPlanningReleases_ManualImport()
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'Replaced'))
		end
from
...
where
	PR.Status in
	(	0 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'Active'))
	)
	...
	case
		when exists
			(	select
					*
				from
					EDIStanley.PlanningReleases_ManualImport fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders_ManualImport', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders_ManualImport', 'Status', 'Replaced'))
	end
from
	EDIStanley.PlanningHeaders_ManualImport fh
where
	fh.Status in
...
from
	EDIStanley.PlanningHeaders_ManualImport fh
	join EDIStanley.PlanningReleases_ManualImport fr
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningHeaders_ManualImport', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIStanley.PlanningReleases_ManualImport', 'Status', 'Active'))
...
and not exists
( Select 1 from 
		EDIStanley.PlanningReleases_ManualImport b
 Join 
