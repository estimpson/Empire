...
CREATE  procedure [dbo].[eeisp_rpt_materials_mps_finished_goods] 
       as
begin
...
POBalance = coalesce ((select sum (balance) 
					...
					date_due < EndDT)+(select sum (balance) 
					from po_detail 
					where part_number = Part and 
					date_due < BeginDT ), 0),
...
EEIDemand = coalesce ((select sum (EEIQty) 
					...
					due_date < EndDT)+(select sum (EEIQty) 
					from order_detail 
					where part_number = Part and 
					due_date < BeginDT ), 0),
...
CustomerDemand = coalesce ((select sum (Quantity) 
						...
						where part_number = Part and 
							due_date >= BeginDT and 
							due_date < EndDT)+(select sum (Quantity) 
							from order_detail 
						where part_number = Part and 
							due_date < BeginDT ), 0),
...
from	(	select	Part = part
	...
	(	select	BeginDT = EntryDT,
			EndDT = EntryDT + 7
	from	[FT].[fn_Calendar] (@ThisMondayDT, null, 'wk', 1, 25)) Buckets
...
where	part in ( select part
		   from	@mps
		group by part
		having abs(sum(RunningTotal))>10 
)
