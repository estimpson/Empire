...
SET ANSI_NULLS ON
GO
CREATE procedure [EDI3040ic].[usp_Process]
...
,   NewDocument
from
	EDI3040ic.CurrentShipSchedules ()
...
,   NewDocument
from
	EDI3040ic.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI3040ic.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3040ic.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3040ic.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI3040ic.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3040ic.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI3040ic.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI3040ic.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3040ic.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI3040ic.ShipScheduleHeaders fh
...
from
	EDI3040ic.ShipScheduleHeaders fh
	join EDI3040ic.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3040ic.ShipScheduleAccums fa
	...
	left join EDI3040ic.ShipScheduleAuthAccums faa
	...
	join EDI3040ic.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI3040ic.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI3040ic.PlanningHeaders fh
	join EDI3040ic.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3040ic.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI3040ic.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI3040ic.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI3040ic.PlanningHeaders fh
					join EDI3040ic.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI3040ic.ShipScheduleHeaders fh
					join EDI3040ic.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI3040ic.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI3040ic.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI3040ic.ShipSchedules b
 Join 
	EDI3040ic.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI3040ic.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI3040ic.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI3040ic.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDI3040ic.PlanningAccums pra on 
 	...
 	left join
		EDI3040ic.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI3040ic' 
...
execute
	@ProcReturn = EDI3040ic.usp_Process
	@TranDT = @TranDT out
