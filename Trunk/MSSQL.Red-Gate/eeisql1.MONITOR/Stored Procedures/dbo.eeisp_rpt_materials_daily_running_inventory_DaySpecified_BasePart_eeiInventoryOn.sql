...
CREATE PROCEDURE  [dbo].[eeisp_rpt_materials_daily_running_inventory_DaySpecified_BasePart_eeiInventoryOnly] (@EnteredDT datetime, @MaxAClass int, @MinAClass int, @MaxBClass int, @MinBClass int, @MaxCClass int, @MinCClass int, @POOffsetDays int)
       as
begin

--eeisp_rpt_materials_daily_running_inventory_DaySpecified_BasePart_eeiInventoryOnly '2011/04/30', 5, 3,10,5,15,10,0
...
- goes out for entire time horizon
- pooffsetdays converts the EEI delivery date to the EEH ship date
--------------------------------------------------------------------------------------------------------------------------*/
...
insert #DailyPOReq
	...
	Select		sum(Balance) as PoQty,
		...
		where	isNULL(balance,0)  >0 and
				date_due > dateadd(dd,-21, getdate()) AND
				part.type = 'F' AND
				part.class != 'I'
...
Select			LEFT(part_standard.part,7) as PSPart,
	...
	LEFT join	part_eecustom on part_standard.part = part_eecustom.part
...
WHERE  isNULL(secured_location, 'N') != 'Y'  and location not like '%LOST%' --and location not like '%TRAN%' and  location not like '%AIR%'
...
from	object 
left join	location on object.location = location.code 
--WHERE location not like '%TRAN%' and  location not like '%AIR%'
GROUP BY LEFT(part,7)
ORDER BY 1
...
--Select * from  #BasePartExtendedRunningPercentage order by 6 ASC
...
Select	substring(DailyReq.Basepart,1,3) as	CustomerCode,
		...
		avg(dailybasepartqty) dailybasepartqty,
		MAX(BPP.Scheduler) Scheduler
...
where		ENDdate = (select max(EndDate) from #DailyReq where dateStamp<=@EnteredDT)
