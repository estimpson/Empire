...
CREATE procedure [EDI].[usp_Process_TRW]
	@TranDT datetime out
,	@Result integer out
,	@Testing int = 1
--<Debug>
...
,	fr.CustomerPart
,	case
		when fr.status = 0 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'New'))
			then 1
		else 0
	end
from
	EDI.TRW_DELFOR_Releases fr
	join EDI.TRW_DELFOR_Headers fh
		on fh.RawDocumentGUID = fr.RawDocumentGUID
where
	fr.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'Active'))
	)
	and fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Headers', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Headers', 'Status', 'Active'))
	)
	and fh.DocumentDT =
	(	select
			max(fh2.DocumentDT)
		from
			EDI.TRW_DELFOR_Headers fh2
			join EDI.TRW_DELFOR_Releases fr2
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'Replaced'))
		end
from
	EDI.TRW_DELFOR_Releases fr
		...
		when exists
			(	select
				...
				where
					...
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Headers', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Headers', 'Status', 'Replaced'))
	end
from
...
,	Type = 2
,	ReleaseDT = [FT].[fn_ShipDate]('wk', bo.ShipDay ,dateadd(dd, bo.InTransitDays*-1 ,fr.ReleaseDT))
...
from
	...
	left join EDI.TRW_DELFOR_Cumulatives dcv
	...
	left join @CurrentDELFORs c
		...
		and fr.CustomerPart = c.CustomerPart
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Headers', 'Status', 'Active'))
	and fr.Status = 1--(select dbo.udf_StatusValue('EDI.TRW_DELFOR_Releases', 'Status', 'Active'))
	and	dcv.status = 1
...
Declare @EDIOrdersAlert table (
	...
	[Notes] [varchar](5000) NULL
		...
		if exists (	select 1 
						...
						join	
							EDI.TRW_DELFOR_Releases FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TRWEDI.BlanketOrders bo	on bo.EDIShipToCode in (FDELFORr.ICCode, FDELFORr.ShipToCode)
						and 
							bo.CustomerPart = FDELFORr.CustomerPart
							and (case when bo.CheckCustomerPO = 1 then bo.CustomerPO else '-1' end ) = (case when bo.CheckCustomerPO = 1 then FDELFORr.CustomerPO else '-1' end )
		...
		from (	select
						...
						join	
							EDI.TRW_DELFOR_Releases FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TRWEDI.BlanketOrders bo	on bo.EDIShipToCode =  FDELFORr.ShipToCode
						and 
							bo.CustomerPart = FDELFORr.CustomerPart
						and (case when bo.CheckCustomerPO = 1 then bo.CustomerPO else '-1' end ) = (case when bo.CheckCustomerPO = 1 then FDELFORr.CustomerPO else '-1' end )
						where 
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @subject = N'EDI Data Exception when processing TRW EDI Document(s)', -- nvarchar(255)
    ...
    @importance = 'High' -- varchar(6)
		...
		-- E-Mail Alerts for Accum discrepancy
			...
			if exists (
						...
						from 
							@CurrentDELFORs FDELFORc
						join	
							EDI.TRW_DELFOR_Cumulatives FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TRWEDI.BlanketOrders bo	on bo.EDIShipToCode =  FDELFORr.ShipToCode
						and 
							bo.CustomerPart = FDELFORr.CustomerPart
						and 
							(case when bo.CheckCustomerPO = 1 then bo.CustomerPO else '-1' end ) = (case when bo.CheckCustomerPO = 1 then FDELFORr.CustomerPO else '-1' end )
						where 
		...
		from (select
						(select max(ReleaseNo) from EDI.TRW_DELFOR_Releases SPOR where SPOR.RawDocumentGUID = FDELFORr.RawDocumentGUID and SPOR.CustomerPart = FDELFORr.CustomerPart  ) ReleaseNo, 
						FDELFORr.CustomerPart CustomerPart,
						FDELFORr.CustomerPO CustomerPO,
						FDELFORr.ShipToCode ShipToID,
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max)
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - TRW EDI Document(s) Accum Dicrepancy', -- nvarchar(255)
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - TRW EDI Document(s) Updated FX Sales Orders', -- nvarchar(255)
...
execute
	@ProcReturn = EDI.usp_Process_TRW
	@TranDT = @TranDT out
