...
CREATE procedure [dbo].[usp_CreateRma_ProcessSerialsQuantities_ByDestGl_Ala2]
...
--- <Body>
-- Get the next available shipper for RMA
select 
	@NextShipper = shipper 
from 
	dbo.parameters
...
from 
	...
	left join dbo.audit_trail at with (readuncommitted) 
...
/*  Set quality status for audit_trail records  */
declare
	@Status char(1)
...
/*  Re-create returned objects in the system  */
--- <Insert rows="1+">
Insert	object 
	(	serial,
		last_date,
...
/*  Insert RmaRtvNumber and serials into a look-up table for reporting purposes  */
--- <Insert rows="1+">
...
if	@Error != 0 begin
	set	@Result = 999999
	RAISERROR ('Error inserting into table %s in procedure %s.  RMA (U) record.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
	rollback tran @ProcName
	return
end
if	@RowCount = 0 begin
	set	@Result = 999999
	RAISERROR ('Error inserting into table %s in procedure %s.  RMA (U) record.  Rows inserted: %d.  Expected rows: >0.', 16, 1, @TableName, @ProcName, @RowCount)
	rollback tran @ProcName
	return
end
--- </Insert>
...
-- 'W2'	- RTV finished part (bulbed part) to Honduras.  (This used to be W1)
...
while ((select count(1) from @tempPartsList where Processed = 0) > 0) begin
	...
	if (charindex('-A', @AlabamaPart) > 0 and (@TransactionType = 0 or @TransactionType = 3)) begin -- part is an Alabama part and its harness will be returned to Honduras
		...
		while ((select count(1) from @tempSerialsList where Processed = 0) > 0) begin
			...
			-- Create a special RTV record (W2) for the finished good (bulbed) part.
			--- <Insert rows="1+">
			set	@TableName = 'dbo.audit_trail'			
			...
			if	@Error != 0 begin
				set	@Result = 999999
				RAISERROR ('Error inserting into table %s in procedure %s.  W1 record.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
				rollback tran @ProcName
				return
			end
			if	@RowCount = 0 begin
				set	@Result = 999999
				RAISERROR ('Error inserting into table %s in procedure %s.  W1 record.  Rows inserted: %d.  Expected rows: >0.', 16, 1, @TableName, @ProcName, @RowCount)
				rollback tran @ProcName
				return
			end
			--- </Insert>	
			...
			-- Create a special RMA record (W2 with negative quantity) using the finished good part RMA data, but changing its part number to the harness child part (the unbulbed part).
			--- <Insert rows="1+">
			set	@TableName = 'dbo.audit_trail'			
			...
			select 
				...
				,'W2'
				,@HarnessPartNumber
				,quantity = (@HarnessStdQuantity * @AlabamaPartQuantity) * -1 -- FT.BOM.StdQty * finished part quantity returned
				,remarks
				,price
				,''
				,customer
				...
				,'EA'
				,null
				,std_quantity = (@HarnessStdQuantity * @AlabamaPartQuantity) * -1 -- FT.BOM.StdQty * finished part quantity returned
			...
			if	@Error != 0 begin
				set	@Result = 999999
				RAISERROR ('Error inserting into table %s in procedure %s.  W2 record.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
				rollback tran @ProcName
				return
			end
			if	@RowCount = 0 begin
				set	@Result = 999999
				RAISERROR ('Error inserting into table %s in procedure %s.  W2 record.  Rows inserted: %d.  Expected rows: >0.', 16, 1, @TableName, @ProcName, @RowCount)
				rollback tran @ProcName
				return
			end
			--- </Insert>	
			...
			while ((select count(1) from @tempBulbPartsList where Processed = 0) > 0) begin
				...
				if	@Error != 0 begin
					set	@Result = 999999
					RAISERROR ('Error inserting into table %s in procedure %s.  Scrap record.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
					rollback tran @ProcName
					return
				end
				if	@RowCount != 1 begin
					set	@Result = 999999
					RAISERROR ('Error inserting into table %s in procedure %s.  Scrap record.  Rows inserted: %d.  Expected rows: 1.', 16, 1, @TableName, @ProcName, @RowCount)
					rollback tran @ProcName
					return
				end
				--- </Insert>
				...
				if	@Error != 0 begin
					set	@Result = 999999
					RAISERROR ('Error inserting into table %s in procedure %s.  Delete record.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
					rollback tran @ProcName
					return
				end
				if	@RowCount != 1 begin
					set	@Result = 999999
					RAISERROR ('Error inserting into table %s in procedure %s.  Delete record.  Rows inserted: %d.  Expected rows: 1.', 16, 1, @TableName, @ProcName, @RowCount)
					rollback tran @ProcName
					return
				end
				--- </Insert>
...
--- Close RMA shipper / create credit memo
declare
	@Invoice int

--- <Credit Memo>	
set			@CallProcName = 'dbo.msp_credit_memo'
execute		@ProcReturn = dbo.msp_credit_memo
				@NextShipper
			,	@OperatorCode
			,	@Invoice out
...
if (@TransactionType = 0) begin -- RMA + RTV
	...
	if	@Error != 0 begin
		set	@Result = 999999
		RAISERROR ('Error updating table %s in procedure %s.  RTV shipper.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
		rollback tran @ProcName
		return
	end
	if	@RowCount <> 1 begin
		set	@Result = 999999
		RAISERROR ('Error updating table %s in procedure %s.  RTV shipper.  Rows inserted: %d.  Expected rows: 1.', 16, 1, @TableName, @ProcName, @RowCount)
		rollback tran @ProcName
		return
	end
	--- </Update>
	...
	if	@Error != 0 begin
		set	@Result = 999999
		RAISERROR ('Error inserting into table %s in procedure %s.  RTV.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
		rollback tran @ProcName
		return
	end
	if	@RowCount != 1 begin
		set	@Result = 999999
		RAISERROR ('Error inserting into table %s in procedure %s.  RTV.  Rows inserted: %d.  Expected rows: 1.', 16, 1, @TableName, @ProcName, @RowCount)
		rollback tran @ProcName
		return
	end
	--- </Insert>
	...
	if	@RowCount = 0 begin
		set	@Result = 999999
		RAISERROR ('Error inserting into table %s in procedure %s.  RTV.  Rows inserted: %d.  Expected rows: >0.', 16, 1, @TableName, @ProcName, @RowCount)
		rollback tran @ProcName
		return
	end
	--- </Insert>	 
	...
	if	@Error != 0 begin
		set	@Result = 999999
		RAISERROR ('Error updating table %s in procedure %s.  RTV.  Error: %d', 16, 1, @TableName, @ProcName, @Error)
		rollback tran @ProcName
		return
	end
	--- </Update>
	...
	if	@RowCount <> 1 begin
		set	@Result = 999999
		RAISERROR ('Error updating table %s in procedure %s.  RTV.  Rows updated: %d.  Expected rows: 1.', 16, 1, @TableName, @ProcName, @RowCount)
		rollback tran @ProcName
		return
	end
	--- </Update>
	...
	-- Determine if an Alabama finished good was RTV'd
	if exists (
			select
				1
			from
		...
		-- Update the previously created W2 (Alabama finished good / bulbed part) RTV record(s) with the RTV shipper number.
		--- <Update rows="1">
		set	@TableName = 'dbo.audit_trail' 
		update
			dbo.audit_trail
		...
		where
			...
			and quantity > 0 -- W2 with negative quantity is the unbulbed part's RMA record
