...
CREATE PROCEDURE [EDIYazaki].[usp_Process]
...
,   NewDocument
from
	EDIYazaki.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIYazaki.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIYazaki.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIYazaki.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIYazaki.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIYazaki.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Replaced'))
	end
from
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIYazaki.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIYazaki.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIYazaki.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIYazaki.ShipScheduleHeaders fh
...
,	Type = 1
,	ReleaseDT = dateadd(dd, (bo.TransitDays*-1), ft.fn_TruncDate('dd', getdate())) 
...
from
	EDIYazaki.ShipScheduleHeaders fh
	join EDIYazaki.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIYazaki.ShipScheduleAccums fa
	...
	left join EDIYazaki.ShipScheduleAuthAccums faa
	...
	join EDIYazaki.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIYazaki.ShipScheduleHeaders', 'Status', 'Active'))
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
, bo.ReferenceAccum
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDIYazaki.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIYazaki.PlanningHeaders fh
	join EDIYazaki.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	join
		EDIYazaki.CurrentPlanningReleases() cpr
	...
	left join EDIYazaki.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIYazaki.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(cpr.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
		fh.Status = 1 --(select dbo.udf_StatusValue('EDIYazaki.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 
order by
	1, 2, 3
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIYazaki.PlanningHeaders fh
					join EDIYazaki.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIYazaki.ShipScheduleHeaders fh
					join EDIYazaki.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			...
			join 
				EDIYazaki.PlanningHeaders fh
			join 
				EDIYazaki.PlanningSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIYazaki.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
		...
		from
			...
			join 
				EDIYazaki.ShipScheduleHeaders fh
			join 
				EDIYazaki.ShipScheduleSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIYazaki.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
					and	(	bo.CheckCustomerPOPlanning = 0
...
and not exists
( Select 1 from 
		EDIYazaki.ShipSchedules b
 Join 
	EDIYazaki.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIYazaki.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIYazaki.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIYazaki.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIYazaki.PlanningAccums pra on 
 	...
 	left join
		EDIYazaki.PlanningAuthAccums praa on 
...
Select 
		...
		,	@EmailHeader nvarchar(max) = 'EDI Processing for EDIYazaki' 
...
execute
	@ProcReturn = EDIYazaki.usp_Process
	@TranDT = @TranDT out
