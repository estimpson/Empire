...
Create procedure [dbo].[msp_process_in_release_plan_meridian]
as
-------------------------------------------------------------------------------------
...
--	8. Remove processed inbound data and Update EEI Qty on Refreshed Release Plan.
	...
	select	@fiscalyearbegin = fiscal_year_begin
	  from	parameters
		...
		Select  *  into #orderdetailcopy
				from		order_detail
				where	type = 'P' and customer_part in (Select customer_part from m_in_release_plan)
...
--	14. Find blanket order.
		exec	@returncode = msp_find_blanket_order_meridian
				@customerpart,
				@shipto,
				@customerpo,
				@modelyear,
...
--	18. Create Copy of Order Detail for Update of EEI Qty and Deleting release plan.
				delete	order_detail
