...
--dbo.EDI_GMDESADV_Detail 1047940
--- Andre S.Boulanger Fore-Thought,LLC
---Using TLWForm GM2_DESADV_D_Dr97A_GM MGO_120303
...
Select	CONVERT( char(80), '') AS PartialComplete,
		...
		CONVERT( char(80), COALESCE(edi_setups.trading_partner_code,'MGOEDIFACT')) AS TradingPArtnerCode, 
	...
	INSERT	#DESADVFlatFileLines (LineData)
	Select	('01'+ LEFT(ShipperID,55)+ LEFT(MESSAGEFUNCTION,1) + LEFT(ASNDate,12)  ) FROM #DESADVHeaderRaw
	INSERT	#DESADVFlatFileLines (LineData)
	Select	('02'+ LEFT(DateShipped,12) + '132'+ LEFT(ArrivalDate,12) + '203'  ) FROM #DESADVHeaderRaw	
	INSERT	#DESADVFlatFileLines (LineData)
	...
	INSERT	#DESADVFlatFileLines (LineData)
	select	('05'+ convert(char(3),'MI')+ LEFT(MaterialIssuer,35) + convert(char(3),'92') ) from #DESADVHeaderRaw
	INSERT	#DESADVFlatFileLines (LineData)
	select	('05'+ convert(char(3),'ST')+ LEFT(Destination,35) + convert(char(3),'92') + LEFT(ShippingDock,25) ) from #DESADVHeaderRaw
	INSERT	#DESADVFlatFileLines (LineData)
	select	('05'+ convert(char(3),'SU')+  LEFT(SupplierCode,35) + convert(char(3),'16') ) from #DESADVHeaderRaw	
	INSERT	#DESADVFlatFileLines (LineData)
...
while
	...
	Insert	#DESADVFlatFileLines (LineData)
	Select	DISTINCT '09' + convert(char(03), PackagingIndicator) /*+ CONVERT(CHAR(30), PalletPackCode)*/ FROM @PartPackTypeSerials where Customerpart = @part
	
	Insert	#DESADVFlatFileLines (LineData)
	Select	'10' + convert(char(10), (COUNT(distinct ParentSerial))) + CONVERT(CHAR(17), LEFT(PalletPackCode,8)) FROM @PartPackTypeSerials where CustomerPart = @part and ParentSerial>0 group by PalletPackCode
	UNION
	Select	'10' + convert(char(10), COUNT(ObjectSerial)) + CONVERT(CHAR(17), LEFT(ObjectPackCode,8)) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY ObjectPackCode
	
	Insert	#DESADVFlatFileLines (LineData)
	Select	'14' + convert(char(35), CustomerPart) + CONVERT(CHAR(35), MAX(ModelYear)) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPart
	
	
	Insert	#DESADVFlatFileLines (LineData)
	Select	'16' + convert(char(3), '12') + CONVERT(CHAR(14), sum(ObjectQuantity)) +  CONVERT(CHAR(3), MAX(UM)) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPart
	
	Insert	#DESADVFlatFileLines (LineData)
	Select	'16' + convert(char(3), '3') + convert(char(14), MAX(AccumShipped))   + CONVERT(CHAR(3), MAX(UM))  FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPart
	
		
	Insert	#DESADVFlatFileLines (LineData)
	Select	'17' + space(3) + CONVERT(CHAR(35),CustomerPO ) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPO
