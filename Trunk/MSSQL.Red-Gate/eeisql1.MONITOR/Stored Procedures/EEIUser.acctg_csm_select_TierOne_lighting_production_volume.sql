...
CREATE procedure [EEIUser].[acctg_csm_select_TierOne_lighting_production_volume] (@HeadlampSupplier varchar(100))
as
...
-- exec eeiuser.acctg_csm_select_tierone_lighting_production_volume @headlampsupplier = 'Valeo'
...
select	 b.[VP: Manufacturer Group] 
		...
		,(case when d.base_part = 'Unknown' then 'Gainsboro' else 'SeaGreen' end) as color
...
where	b.[Headlamp Supplier] = @HeadlampSupplier
		and b.[Vehicle Volume 2019]>0 
		and base_part is not null
group by b.[Headlamp Supplier],
		b.[VP: Manufacturer Group], 
		b.[VP: Production Nameplate],
		d.[base_part]

union
select b.manufacturer, b.headlamp_supplier, b.nameplate, 'Unknown' as base_part, 
...
where	b.Headlamp_Supplier = @HeadlampSupplier
