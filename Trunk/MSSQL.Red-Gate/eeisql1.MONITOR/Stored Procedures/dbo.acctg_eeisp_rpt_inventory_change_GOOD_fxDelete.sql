...
CREATE procedure [dbo].[acctg_eeisp_rpt_inventory_change_GOOD_fxDelete]
...
-- Populate the table
	...
	Insert			#SelectedEmpowerTransactions (EmpowerSerial, EmpowerPart, EmpowerTransactionDate, EmpowerTransactionType, EmpowerExtCost)
		...
		where		ledger = 'EMPIRE' and
					fiscal_year = 2007 and
					period = 8 and
					ledger_account = '154111' and
					update_balances = 'Y'
...
-- Populate the table
	...
	Insert			#SelectedMonitorTransactions
		...
		select		audit_trail.serial,
					...
					part_standard_historical.material_cum,
					(audit_trail.quantity*part_standard_historical.material_cum),
					audit_trail.user_defined_status

		from		audit_trail

		left outer join	part_historical on audit_trail.part = part_copy_historical.part
		left outer join	part_standard_historical on audit_trail.part = part_standard_historical.part

		where		audit_trail.date_stamp >= '2007-08-01 00:00:00' and
					audit_trail.date_stamp < '2007-09-01 00:00:00' and
					...
					part_historical.type not in ('R','W','')
                    and part_historical.period = 8 and part_historical.fiscal_year = '2007'
                    and part_standard_historical.period = 8 and part_standard_historical.fiscal_year = '2007'
...
-- Add product_line
	...
	Update			Acctg_ActivityComparison_2006ytd
		...
		set			ProductLine = part_historical.product_line
			
		from		part_historical, Acctg_ActivityComparison_2006ytd

		where		Acctg_ActivityComparison_2006ytd.part = part_historical.part
                    and part_historical.fiscal_year = '2007' and part_historical.period = 8
