...
,   NewDocument
from
	EDIAutoLiv.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIAutoLiv.CurrentPlanningReleases ()
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIAutoLiv.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIAutoLiv.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIAutoLiv.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIAutoLiv.ShipScheduleHeaders ssh
where
	ssh.Status in
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIAutoLiv.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIAutoLiv.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningHeaders', 'Status', 'Replaced'))
	end
from
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIAutoLiv.ShipScheduleHeaders fh
...
from
	EDIAutoLiv.ShipScheduleHeaders fh
	join EDIAutoLiv.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIAutoLiv.ShipScheduleAccums fa
	...
	left join EDIAutoLiv.ShipScheduleAuthAccums faa
...
--and			fh.Status = 1 --(select dbo.udf_StatusValue('EDIAutoLiv.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIAutoLiv.PlanningHeaders fh
	join EDIAutoLiv.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIAutoLiv.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIAutoLiv.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIAutoLiv.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIAutoLiv.PlanningHeaders fh
					join EDIAutoLiv.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIAutoLiv.ShipScheduleHeaders fh
					join EDIAutoLiv.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIAutoLiv.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIAutoLiv.ShipScheduleSupplemental sss
		on
...
from
	@Current862s a
	 Join 
	EDIAutoLiv.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIAutoLiv.ShipScheduleAccums ssa on 
 	...
 	LEFT JOIN
		EDIAutoLiv.ShipScheduleAuthAccums ssaa ON 
...
(	bo.CheckModelYearPlanning = 0
	OR bo.ModelYear830 = a.CustomerModelYear)
	LEFT JOIN
		EDIAutoLiv.PlanningAccums pra ON 
 	...
 	LEFT JOIN
		EDIAutoLiv.PlanningAuthAccums praa ON 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIAutoLiv' 
...
execute
	@ProcReturn = EDIAutoLiv.usp_Process
	@TranDT = @TranDT out
