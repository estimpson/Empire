...
CREATE procedure [dbo].[eeisp_rpt_rank_vendor_spend_2010and2011] as
begin
...
select 	Base_Part,
		...
		(sum(total_2010)+sum(total_2011))/2
from		[EEIUser].[acctg_csm_vw_select_total_demand]
group by	Base_Part
having	(sum(total_2010)+sum(total_2011))>50
...
Select	TopPart,
		left(TopPart,7),
		-- left(TopPart,7) + ' ('+Substring(convert(varchar(1000),DemandY0 ),1, patindex( '%[.]%',convert(varchar(1000),DemandY0 ))-1 )+')' + ' ('+Substring(convert(varchar(1000),DemandY1 ),1, patindex( '%[.]%',convert(varchar(1000),DemandY1 ))-1 )+')' + ' ('+Substring(convert(varchar(1000),DemandY2 ),1, patindex( '%[.]%',convert(varchar(1000),DemandY2 ))-1 )+')',
...
Select	BOM.PrimaryVendor,
		...
		-- sum(DemandY0*QtyPer) as QtyY0,
		-- sum(DemandY0*DollarsPer) as SpendY0,
