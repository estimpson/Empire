...
CREATE procedure [EEIUser].[eeisp_rpt_web_sales_2_ForecastYear] (@FY char(4), @FN1 varchar(50), @FN2 varchar(50))


as
delete from sales_3

--exec '2008 OSF - 2008Q3 Update Using 2008-09 CSM','2008 OSF - 2008Q1 Update Using 2008-01 CSM'

--populate with distinct parts from actual and forecast
insert into sales_3 (base_part, forecast_year, forecast_period)
select distinct(base_part),@FY, forecast_period from sales_1 where forecast_year=@FY  and forecast_name=@FN1

insert into sales_3 (base_part, forecast_year, forecast_period)
select distinct(base_part), @FY, forecast_period from sales_1 where forecast_year=@FY  and forecast_name=@FN2
and base_part not in (select distinct(base_part) from sales_3)
...
update sales_3
set program_manager = (select max(team) from eeiuser.teamtobasepart where sales_3.base_part = teamtobasepart.basepart)

update sales_3
set program_manager = (select description from sales_manager_code where sales_3.program_manager = sales_manager_code.code)


--populate forecast columns
update sales_3
set 
sales_3.forecast_units = ISNULL(sales_1.forecast_units,0),
sales_3.forecast_sales = ISNULL(sales_1.forecast_sales,0)
from sales_1, sales_3
where sales_1.base_part = sales_3.base_part
and sales_1.forecast_year = @FY
and sales_1.forecast_period = sales_3.forecast_period 
...
update sales_3
set 
sales_3.forecast_units_2 = ISNULL(sales_1.forecast_units,0),
sales_3.forecast_sales_2 = ISNULL(sales_1.forecast_sales,0)
from sales_1, sales_3
where sales_1.base_part = sales_3.base_part
and sales_1.forecast_year = @FY
and sales_1.forecast_period = sales_3.forecast_period
