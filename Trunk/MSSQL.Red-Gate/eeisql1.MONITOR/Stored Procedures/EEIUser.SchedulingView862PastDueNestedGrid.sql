...
CREATE procedure [EEIUser].[SchedulingView862PastDueNestedGrid] @PastDueReleaseNo varchar(30)

-- exec eeiuser.SchedulingView862PastDueNestedGrid 'KR00000082450626'
...
select		s.id, s.status, s.date_shipped, s.bill_of_lading_number, s.customer, s.destination, 
			...
			sd.release_no, sd.part_original, sd.customer_part, ran.qty, ran.rannumber as PastDueReleaseNo
...
where	ran.RanNumber = @PastDueReleaseNo
