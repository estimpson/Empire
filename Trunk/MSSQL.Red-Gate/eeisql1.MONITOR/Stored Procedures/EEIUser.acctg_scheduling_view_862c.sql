...
CREATE procedure [EEIUser].[acctg_scheduling_view_862c] @releasecreatedDT datetime, @ShipToCode varchar(20), @CustomerPart varchar(30)

-- exec eeiuser.acctg_scheduling_view_862c '2/25/2013 9:41:55 AM', 'ALABAMA', '938 714-13' 
...
declare @a table (	id int, status varchar(1), date_shipped datetime, bill_of_lading_number int, customer varchar(25), destination varchar(20), ShipToCode varchar(20),
					plant varchar(10), qty_required numeric(20,6), qty_packed numeric(20,6), boxes_staged int, picklist_printed varchar(1), order_no numeric(8,0), customer_po varchar(25), 
...
select		s.id, s.status, s.date_shipped, s.bill_of_lading_number, s.customer, s.destination, 
			...
			s.plant, sd.qty_required, sd.qty_packed, sd.boxes_staged, s.picklist_printed, sd.order_no, sd.customer_po, 
...
select plant, ShipToCode, CustomerPart, part_original, release_no as shipper_ran, rannumber as nal_ran, qty, qty_required, qty_packed, id, Status, picklist_printed from @a 
