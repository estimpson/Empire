...
CREATE procedure	[dbo].[eeisp_rpt_copper_pricepershipment_ALC] 
...
Select part,
			...
			(Select max(part.name) from part  where	substring(part.part,1,7) = alc_quote_date.part),
			(Select max(shipper_detail_eei.alternate_price) from shipper_detail_eei where date_shipped = (Select max(date_shipped) from shipper_detail_eei where substring(part_original,1,7) = alc_quote_date.part and part_original not like '%PT%' and alternate_price >0) and substring(part_original,1,7) = alc_quote_date.part and part_original not like '%PT%' and alternate_price >0),
			(Select	max(prod_start) from part_eecustom where substring(part_eecustom.part,1,7) = alc_quote_date.part),
			(Select	max(prod_end) from part_eecustom where substring(part_eecustom.part,1,7) = alc_quote_date.part),
			quote_date
from		alc_quote_date
...
---get BOM Info for ALC Parts 			
 	...
 	create table #copperBOM(
 				...
 				insert		#copperBOM
					...
					and			finishedpart in (Select max(finishedpart) from vweeiBOM where substring(Finishedpart,1,7) in (select basepart from #ALCPart) and FinishedPart not like '%RW%' and finishedPart not like '%PT%')
 	...
 	Select				#ALCCopperBOM.FinPart,
							...
							#ALCpart.quote_date,
 								...
 								where	#apitemsmetalsurcharge.inv_cm_date>= dateadd(dd,-30,#ALCCopperBOM.firstnonWireInvDate ) and 
 											#apitemsmetalsurcharge.inv_cm_date<=dateadd(dd,30,#ALCCopperBOM.firstnonWireInvDate ) and
 											#ALCCopperBOM.CopperPart= #apitemsmetalsurcharge.Item ),0) as 	initalnonWireCost,							
