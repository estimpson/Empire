...
CREATE procedure [EDINascote].[usp_Process]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDINascote.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDINascote.CurrentShipSchedules ()
...
,   NewDocument
from
	EDINascote.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDINascote.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDINascote.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDINascote.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDINascote.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDINascote.ShipScheduleHeaders'
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDINascote.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDINascote.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDINascote.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDINascote.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDINascote.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDINascote.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDINascote.ShipScheduleHeaders fh
...
,	QtyRelease = 0
,	StdQtyRelease = 0
,	ReferenceAccum = min(coalesce(convert(int,fa.LastAccumQty),bo.AccumShipped,0))
...
from
	EDINascote.ShipScheduleHeaders fh
	join EDINascote.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDINascote.ShipScheduleAccums fa
	...
	left join EDINascote.ShipScheduleAuthAccums faa
	...
	join EDINascote.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDINascote.ShipScheduleHeaders', 'Status', 'Active'))
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
...
,	ReferenceAccum = coalesce(convert(int,fa.LastAccumQty),bo.AccumShipped,0)
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDINascote.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDINascote.PlanningHeaders fh
	join EDINascote.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDINascote.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDINascote.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
	...
	and not exists
		(	select
				*
			from
				EDINascote.PlanningReleases ss
			where
				ss.RawDocumentGUID = fr.RawDocumentGUID
				and ss.CustomerPart = fr.CustomerPart
				and ss.ShipToCode = fr.ShipToCode
		...
		fh.Status = 1 --(select dbo.udf_StatusValue('EDINascote.PlanningHeaders', 'Status', 'Active'))
	and fr.Status = 1 --(select dbo.udf_StatusValue('EDINascote.PlanningReleases', 'Status', 'Active'))
  ...
  ,@Result3 output
	...
	if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDINascote.PlanningHeaders fh
					join EDINascote.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDINascote.ShipScheduleHeaders fh
					join EDINascote.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			...
			join 
				EDINascote.PlanningHeaders fh
			join 
				EDINascote.PlanningSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDINascote.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
		...
		from
			...
			join 
				EDINascote.ShipScheduleHeaders fh
			join 
				EDINascote.ShipSceduleSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDINascote.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
					and	(	bo.CheckCustomerPOPlanning = 0
...
from
	@Current862s a
join
	EDINascote.ShipSchedules b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
...
Left Join 
	EDINascote.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
left join
	EDINascote.PlanningReleases b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
from
	@Current862s a
join
	EDINascote.ShipScheduleAccums b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
join
	EDINascote.ShipScheduleAuthAccums c on a.RawDocumentGUID = c.RawDocumentGUID and a.NewDocument = 1
and
...
from
	@Current830s a
join
	EDINascote.PlanningAccums b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
join
	EDINascote.PlanningAuthAccums c on a.RawDocumentGUID = c.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = c.CustomerPart
and
	a.ShipToCode = c.ShipToCode
...
Select 
		...
		,	@EmailHeader nvarchar(max) = 'EDINascote Alert' 
...
execute
	@ProcReturn = EDINascote.usp_Process
	@TranDT = @TranDT out
