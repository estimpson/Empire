...
CREATE procedure [EDIPLEX].[usp_Process_new]
...
,   NewDocument
from
	EDIPLEX.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIPLEX.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIPLEX.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIPLEX.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIPLEX.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIPLEX.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIPLEX.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIPLEX.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIPLEX.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIPLEX.ShipScheduleHeaders fh
...
,	QtyRelease = 0
,	StdQtyRelease = 0
,	ReferenceAccum = case bo.ShipScheduleReferenceAccum 
												When 'N' 
...
,	CustomerAccum = case bo.ShipScheduleAdjustmentAccum 
												When 'N' 
												then min(coalesce(convert(int,bo.AccumShipped),0))
												When 'P' 
...
from
	EDIPLEX.ShipScheduleHeaders fh
	join EDIPLEX.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIPLEX.ShipScheduleAccums fa
	...
	left join EDIPLEX.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIPLEX.ShipScheduleHeaders', 'Status', 'Active'))
	and		c.RawDocumentGUID = fr.RawDocumentGUID
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
, bo.ShipScheduleReferenceAccum
, bo.ShipScheduleAdjustmentAccum 
having
					case						bo.ShipScheduleAdjustmentAccum 
												When 'N' 
												then  min(coalesce(convert(int,bo.AccumShipped),0))
												When 'P' 
												then min(coalesce(convert(int,faa.PriorCUM),0))
												...
												case bo.ShipScheduleReferenceAccum 
												When 'N' 
												then min(coalesce(convert(int,bo.AccumShipped),0))
												When 'C' 
												then min(coalesce(convert(int,fa.LastAccumQty),0))
...
,	QtyRelease = fr.ReleaseQty
,	StdQtyRelease = fr.ReleaseQty
,	ReferenceAccum = case bo.PlanningReleaseReferenceAccum 
												When 'N' 
												then coalesce(convert(int,bo.AccumShipped),0)
												When 'C' 
												then coalesce(convert(int,fa.LastAccumQty),0)
...
,	CustomerAccum = case bo.PlanningReleaseAdjustmentAccum 
												When 'N' 
												then coalesce(convert(int,bo.AccumShipped),0)
												When 'P' 
												then coalesce(convert(int,faa.PriorCUM),0)
...
from
	EDIPLEX.PlanningHeaders fh
	join EDIPLEX.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIPLEX.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIPLEX.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIPLEX.PlanningReleases', 'Status', 'Active'))
...
from
	@RawReleases bor
where
	bor.QtyRelease >=0 -- added 07/6/2016 after analysis of NAL releases
group by
		bor.OrderNo
,		bor.ReleaseNo
,		bor.ReleaseDT
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIPLEX.PlanningHeaders fh
					join EDIPLEX.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIPLEX.ShipScheduleHeaders fh
					join EDIPLEX.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIPLEX.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIPLEX.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIPLEX.ShipSchedules b
 Join 
	EDIPLEX.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIPLEX.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIPLEX.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIPLEX.ShipScheduleAuthAccums ssaa on 
	...
	Where
		...
		(bo.ShipScheduleAdjustmentAccum != 'N' or bo.ShipScheduleReferenceAccum != 'N')
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDIPLEX.PlanningAuthAccums praa on 
	...
	Where
		coalesce(a.newDocument,0) = 1 and
		coalesce(bo.AccumShipped,0) != coalesce(pra.LastAccumQty,0)	and
		(bo.PlanningReleaseAdjustmentAccum != 'N' or bo.PlanningReleaseReferenceAccum != 'N')
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIPLEX' 
...
execute
	@ProcReturn = EDIPLEX.usp_Process
	@TranDT = @TranDT out
