...
--select  @PartType = 'F';
--select @begdate = '2012-09-30 00:00:00';
--select @enddate = '2012-10-31 00:00:00';
--select @account = '153011';
--select @productline = 'WIRE HARN - EEH';


select  @rundate = getdate();
select  @Ledger = 'EMPIRE';
...
where	ohd1.time_stamp = @adjbegdate
	...
	and phd1.product_line = @ProductLine
...
where	ohd2.time_stamp = @adjenddate 
	...
	and phd2.product_line = @ProductLine
...
-- Populate the temp table with current month inventory where no prior month inventory
insert	#Acctg_InvChange
...
-- Populate the temp table for instances of current month activity but no current month or prior month ending inventory
insert	#Acctg_InvChange
...
FULL OUTER JOIN 
	...
	(SELECT		p.product_line
				...
				,case when at.type = 'W2' and at.remarks = 'Ret Vendor' then at.quantity else (case when at.type in ('R','U','A','J') then at.quantity else -1*at.quantity end)end as 'quantity'
				,ps.material_cum
				,case when at.type = 'W2' and at.remarks = 'Ret Vendor' then round(at.quantity*ps.material_cum,2) else (case when at.type in ('R','U','A','J') then round(at.quantity*ps.material_cum,2) else round(-1*at.quantity*ps.material_cum,2) end)end as 'amount'
				,at.operator
				,at.parent_serial
				,at.posted
