...
CREATE	Procedure	[dbo].[eeisp_rpt_materials_customer_delivery_performance_report] (@FromDate datetime, @ThroughDate datetime)

as
begin

-- eeisp_rpt_materials_customer_delivery_performance_report '2008-09-01', '2008-09-30'
...
WHERE	shipper.id = shipper_detail.shipper AND 
		...
		isNULL(shipper.type, 'X')  not in ('R', 'V') and
		part_original not in (Select part from eeivw_MG)
