...
,   NewDocument
FROM
	EDIEDIFACT04A.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIEDIFACT04A.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIEDIFACT04A.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIEDIFACT04A.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIEDIFACT04A.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIEDIFACT04A.ShipScheduleHeaders ssh
where
	ssh.Status in
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIEDIFACT04A.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIEDIFACT04A.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIEDIFACT04A.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIEDIFACT04A.ShipScheduleHeaders fh
...
from
	EDIEDIFACT04A.ShipScheduleHeaders fh
	join EDIEDIFACT04A.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIEDIFACT04A.ShipScheduleAccums fa
	...
	left join EDIEDIFACT04A.ShipScheduleAuthAccums faa
	...
	join EDIEDIFACT04A.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIEDIFACT04A.PlanningHeaders fh
	join EDIEDIFACT04A.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIEDIFACT04A.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIEDIFACT04A.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIEDIFACT04A.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIEDIFACT04A.PlanningHeaders fh
					join EDIEDIFACT04A.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIEDIFACT04A.ShipScheduleHeaders fh
					join EDIEDIFACT04A.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			dbo.order_header oh
		join
				EDIEDIFACT04A.blanketOrders bo
		on
		...
		join
		  ...
		  join
				EDIEDIFACT04A.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIEDIFACT04A.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIEDIFACT04A.ShipSchedules b
 Join 
	EDIEDIFACT04A.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
and not exists
( Select 1 from 
		EDIEDIFACT04A.PlanningReleases b
 Join 
	EDIEDIFACT04A.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
	b.ShipToCode = bo.EDIShipToCode
...
from
	@Current862s a
	 Join 
	EDIEDIFACT04A.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIEDIFACT04A.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIEDIFACT04A.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIEDIFACT04A.PlanningAccums pra on 
 	...
 	left join
		EDIEDIFACT04A.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDIEDIFACT04A Alert' 
...
FROM
	@Current830s a
JOIN
		EDIEDIFACT04A.PlanningReleases c
ON			c.RawDocumentGUID = a.RawDocumentGUID AND
				a.CustomerPart = c.CustomerPart AND
				a.ShipToCode =c.ShipToCode AND
				COALESCE(a.customerPO,'') = COALESCE(c.CustomerPO,'') AND
...
execute
	@ProcReturn = EDIEDIFACT04A.usp_Process
	@TranDT = @TranDT out
