...
CREATE procedure [EDILearmex].[usp_Process]
...
,   NewDocument
from
	EDILearmex.CurrentShipSchedules ()
...
,   NewDocument
from
	EDILearmex.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDILearmex.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDILearmex.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDILearmex.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDILearmex.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDILearmex.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDILearmex.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDILearmex.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDILearmex.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDILearmex.ShipScheduleHeaders fh
...
from
	EDILearmex.ShipScheduleHeaders fh
	join EDILearmex.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDILearmex.ShipScheduleAccums fa
	...
	left join EDILearmex.ShipScheduleAuthAccums faa
	...
	join EDILearmex.BlanketOrders bo
...
and	fh.Status = 1 --(select dbo.udf_StatusValue('EDILearmex.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDILearmex.PlanningHeaders fh
	join EDILearmex.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDILearmex.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDILearmex.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
				c.RawDocumentGUID = fr.RawDocumentGUID
	and	fh.Status = 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDILearmex.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDILearmex.PlanningHeaders fh
					join EDILearmex.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDILearmex.ShipScheduleHeaders fh
					join EDILearmex.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDILearmex.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDILearmex.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDILearmex.ShipSchedules b
 Join 
	EDILearmex.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDILearmex.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDILearmex.ShipScheduleAccums ssa on 
 	...
 	left join
		EDILearmex.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDILearmex.PlanningAccums pra on 
 	...
 	left join
		EDILearmex.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for Lear Mexico' 
...
execute
	@ProcReturn = EDILearmex.usp_Process
	@TranDT = @TranDT out
