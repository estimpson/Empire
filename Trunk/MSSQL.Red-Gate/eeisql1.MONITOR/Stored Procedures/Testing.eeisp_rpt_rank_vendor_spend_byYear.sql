...
CREATE  procedure [Testing].[eeisp_rpt_rank_vendor_spend_byYear] as
begin

--exec [testing].[eeisp_rpt_rank_vendor_spend_byYear]
...
group by	TopPart,
		ChildPart
having ChildPart =  '175180-1'

select	*, (select sum(order_detail.quantity) from order_detail where part_number  = #BOM.TopPart) as Demand

from	#BOM
		
Create table	#CurrentBOM (
			TopPart varchar(25),	
			CurrentRevLevel char(1),
			BasePart varchar(25),
