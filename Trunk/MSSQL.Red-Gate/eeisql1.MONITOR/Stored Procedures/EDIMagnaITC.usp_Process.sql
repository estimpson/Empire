...
CREATE PROCEDURE [EDIMagnaITC].[usp_Process]
...
,   NewDocument
from
	EDIMagnaITC.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIMagnaITC.CurrentPlanningReleases ()
...
END
--- <Update rows="*">
set	@TableName = 'EDIMagnaITC.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIMagnaITC.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIMagnaITC.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIMagnaITC.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIMagnaITC.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIMagnaITC.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIMagnaITC.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIMagnaITC.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIMagnaITC.ShipScheduleHeaders fh
...
from
	EDIMagnaITC.ShipScheduleHeaders fh
	join EDIMagnaITC.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIMagnaITC.ShipScheduleAccums fa
	...
	left join EDIMagnaITC.ShipScheduleAuthAccums faa
	...
	join EDIMagnaITC.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaITC.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIMagnaITC.PlanningHeaders fh
	join EDIMagnaITC.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIMagnaITC.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIMagnaITC.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
,	CustomerAccum = case bo.AdjustmentAccum 
												...
												When 'P' 
												then coalesce(convert(int,bo.AccumShipped),0)
												else coalesce(convert(int,bo.AccumShipped),0) -- get last Accum from shipSchedule 1st
												end
,	NewDocument =
		(	select
				min(c.NewDocument)
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIMagnaITC.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIMagnaITC.PlanningHeaders fh
					join EDIMagnaITC.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIMagnaITC.ShipScheduleHeaders fh
					join EDIMagnaITC.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIMagnaITC.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIMagnaITC.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIMagnaITC.ShipSchedules b
 Join 
	EDIMagnaITC.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
and not exists
( Select 1 from 
		EDIMagnaITC.PlanningReleases b
 Join 
	EDIMagnaITC.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
	b.ShipToCode = bo.EDIShipToCode
...
from
	@Current862s a
	 Join 
	EDIMagnaITC.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIMagnaITC.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIMagnaITC.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIMagnaITC.PlanningAccums pra on 
 	...
 	left join
		EDIMagnaITC.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIMagnaITC' 
...
execute
	@ProcReturn = EDIMagnaITC.usp_Process
	@TranDT = @TranDT out
