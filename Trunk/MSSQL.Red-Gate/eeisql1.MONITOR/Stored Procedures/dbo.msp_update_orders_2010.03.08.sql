...
CREATE procedure [dbo].[msp_update_orders_2010.03.08] (
	@shipper integer )
as
---------------------------------------------------------------------------------------
...
CREATE	TABLE #ActiveRevOrders (
				ActiveRevOrderNo int,
				ShippedOrderNo int,
				QtyShipped numeric(20,6))

INSERT	#ActiveRevOrders
        (	ActiveRevOrderNo,
			ShippedOrderNo,
			QtyShipped
			)
...
SELECT	oh.order_no,
				CurrentShippers.order_no,
				CurrentShippers.alternative_qty
		
FROM	dbo.order_header oh
JOIN	(SELECT	order_header.order_no,
				order_header.customer_part,
				destination,
				alternative_qty
		 FROM	dbo.order_header
		 JOIN	shipper_detail ON Order_header.order_no = shipper_detail.order_no
		 WHERE	shipper_detail.shipper = @shipper AND
				order_header.order_type = 'B'
			) CurrentShippers ON oh.customer_part = CurrentShippers.customer_part AND oh.destination = CurrentShippers.destination
JOIN	part_eecustom ON oh.blanket_part = part_eecustom.part
WHERE	ISNULL(NULLIF(CurrentRevLevel,''),'N') = 'Y' AND
		oh.order_type = 'B' AND
		oh.order_no != CurrentShippers.order_no
		
UPDATE	dbo.order_header
SET		our_cum = our_cum+TotalShipped
FROM	dbo.order_header
JOIN	(Select sum(QtyShipped) as TotalShipped,
				ActiveRevOrderNo
				from #ActiveRevOrders
			Group By ActiveRevOrderNo) ActiveRev ON Order_header.order_no = ActiveRev.ActiveRevOrderNo
