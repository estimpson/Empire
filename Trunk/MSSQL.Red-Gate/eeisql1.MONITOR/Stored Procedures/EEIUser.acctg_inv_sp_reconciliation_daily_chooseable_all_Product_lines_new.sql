...
CREATE procedure [EEIUser].[acctg_inv_sp_reconciliation_daily_chooseable_all_Product_lines_new]
(	@BegDate datetime,
	@EndDate datetime,
	@Account varchar (10),
...
as
/*
execute	[EEIUser].[acctg_inv_sp_reconciliation_daily_chooseable_all_Product_lines_new]
	@BegDate = '2009-02-01',
	@EndDate = '2009-02-28',
	@Account = '154011',
	@Product_line = 'Wire Harn - EEH',
	@PartType = 'F'
...
from	(	select	Part = PriorMonthCost.part,
	...
	(	select	Part = PriorMonthPart.part,
		...
		where	PriorMonthPart.time_stamp = @AdjBeginDT and
				PriorMonthPart.type = @PartType and
				PriorMonthPart.Product_line = @Product_line) PriorMonthPart on PriorMonthCost.Part = PriorMonthPart.Part
...
from	(	select	Part = CurrentMonthCost.part,
	...
	(	select	Part = CurrentMonthPart.part,
		...
		where	CurrentMonthPart.time_stamp = @AdjEndDT and
				CurrentMonthPart.type = @PartType and
				CurrentMonthPart.Product_line = @Product_line) CurrentMonthPart on CurrentMonthCost.Part = CurrentMonthPart.Part
...
where	at9.date_stamp >= @AdjBeginDT and
		at9.date_stamp < @AdjEndDT and
		part.type = @partType
...
(	MonitorSerial varchar(50),
	...
	MonitorRemarks varchar(10) null,
...
left join	#PartSetupDaily PartSetupDaily on at4.part = PartSetupDaily.Part
join		part_standard on at4.part = part_standard.part
and		at4.date_stamp >= PartSetupDaily.FromDT and at4.date_stamp < PartSetupDaily.ToDT and
...
where	coalesce (nullif(Acctg_InvChange.CurrentProductLine,''), Acctg_ActivityComparison.ProductLine) = @Product_Line
order by
	1, 2
GO
