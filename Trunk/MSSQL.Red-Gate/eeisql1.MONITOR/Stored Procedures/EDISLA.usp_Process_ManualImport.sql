...
CREATE procedure [EDISLA].[usp_Process_ManualImport]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDISLA.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDISLA.CurrentPlanningReleases_ManualImport()
...
--- <Update rows="*">
set	@TableName = 'EDISLA.PlanningReleases_ManualImport'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'Replaced'))
		end
from
	EDISLA.PlanningReleases_ManualImport PR
...
where
	PR.Status in
	(	0 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDISLA.PlanningHeaders_ManualImport'
	...
	case
		when exists
			(	select
					*
				from
					EDISLA.PlanningReleases_ManualImport fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders_ManualImport', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders_ManualImport', 'Status', 'Replaced'))
	end
from
	EDISLA.PlanningHeaders_ManualImport fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders_ManualImport', 'Status', 'Active'))
	)
...
from
	EDISLA.PlanningHeaders_ManualImport fh
	join EDISLA.PlanningReleases_ManualImport fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
		/*
	left join EDISLA.PlanningAccums fa
	...
	left join EDISLA.PlanningAuthAccums faa
		...
		*/
	left join EDISLA.BlanketOrders bo
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDISLA.PlanningHeaders_ManualImport', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDISLA.PlanningReleases_ManualImport', 'Status', 'Active'))
...
and not exists
( Select 1 from 
		EDISLA.PlanningReleases_ManualImport b
 Join 
	EDISLA.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
	 Join 
	EDISLA.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDISLA.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDISLA' 
