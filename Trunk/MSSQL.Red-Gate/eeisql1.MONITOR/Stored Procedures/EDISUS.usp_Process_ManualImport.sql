...
CREATE procedure [EDISUS].[usp_Process_ManualImport]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDISUS.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDISUS.CurrentPlanningReleases_ManualImport()
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'Replaced'))
		end
from
...
where
	PR.Status in
	(	0 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'Active'))
	)
	...
	case
		when exists
			(	select
					*
				from
					EDISUS.PlanningReleases_ManualImport fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders_ManualImport', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders_ManualImport', 'Status', 'Replaced'))
	end
from
	EDISUS.PlanningHeaders_ManualImport fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders_ManualImport', 'Status', 'Active'))
	)
...
--,	CustomerPOLine = fr.CustomerPOLine
...
from
	EDISUS.PlanningHeaders_ManualImport fh
	join EDISUS.PlanningReleases_ManualImport fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
		/*
	left join EDISUS.PlanningAccums fa
	...
	left join EDISUS.PlanningAuthAccums faa
		...
		*/
	left join EDISUS.BlanketOrders bo
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDISUS.PlanningHeaders_ManualImport', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDISUS.PlanningReleases_ManualImport', 'Status', 'Active'))
...
and not exists
( Select 1 from 
		EDISUS.PlanningReleases_ManualImport b
 Join 
	EDISUS.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
	 Join 
	EDISUS.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearPlanning = 0
	...
	left join
 	...
 	left join
		EDISUS.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDISUS' 
