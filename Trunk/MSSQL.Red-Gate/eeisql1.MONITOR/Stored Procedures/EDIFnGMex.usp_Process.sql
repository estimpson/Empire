...
SET ANSI_NULLS ON
GO
CREATE procedure [EDIFnGMex].[usp_Process]
...
,   NewDocument
from
	EDIFnGMex.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIFnGMex.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIFnGMex.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFnGMex.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFnGMex.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIFnGMex.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFnGMex.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIFnGMex.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDIFnGMex.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFnGMex.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIFnGMex.ShipScheduleHeaders fh
...
from
	EDIFnGMex.ShipScheduleHeaders fh
	join EDIFnGMex.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIFnGMex.ShipScheduleAccums fa
	...
	left join EDIFnGMex.ShipScheduleAuthAccums faa
	...
	join EDIFnGMex.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIFnGMex.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIFnGMex.PlanningHeaders fh
	join EDIFnGMex.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIFnGMex.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIFnGMex.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDIFnGMex.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIFnGMex.PlanningHeaders fh
					join EDIFnGMex.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIFnGMex.ShipScheduleHeaders fh
					join EDIFnGMex.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDIFnGMex.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDIFnGMex.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDIFnGMex.ShipSchedules b
 Join 
	EDIFnGMex.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDIFnGMex.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDIFnGMex.ShipScheduleAccums ssa on 
 	...
 	left join
		EDIFnGMex.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDIFnGMex.PlanningAccums pra on 
 	...
 	left join
		EDIFnGMex.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDIFnGMex' 
...
execute
	@ProcReturn = EDIFnGMex.usp_Process
	@TranDT = @TranDT out
