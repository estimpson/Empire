...
create procedure [dbo].[csp_RF_CheckQuantity_fxDelete] 
(	@PONumber integer,
	@PartNumber varchar(25),
	@Quantity numeric(20,6),
	@Objects integer,
	@NextDueAmount numeric(20,6) output,
	@NextDueDate varchar(20) output,
...
Author:
Chris Rogers
Copyright © 2005 Fore-Thought, LLC
...
if @Quantity * @Objects >
	IsNull ( (	select	Sum ( IsNull(balance, 0 ))
			...
			where	po_number = @PONumber and
				...
				date_due between dateadd(dd,-7,(getdate() - datepart (dw,getdate()) + 1)) and dateadd(dd,7,(getdate() + ( 7 - datepart (dw,getdate()) )) )), 0 )
begin
...
--		B.	Check receipt against the authorized accum for the week.
	if @Quantity + 
		...
		IsNull ( (	select	max(PostAccum)
				from	ft.releaseplanraw rpr
					left outer join FT.DTGlobals dtg on dtg.name = 'BaseWeek'
					join po_header poh on poh.po_number = @PONumber
					join part_vendor pv on pv.part = rpr.part and
						pv.vendor = poh.vendor_code
				where	rpr.ponumber = @PONumber and
					rpr.Part = @PartNumber and
					rpr.DueDT between dateadd(dd,-7,(getdate() - datepart (dw,getdate()) + 1)) - ( convert(integer,( pv.ReceiptAuthDays / 7 ) ) ) and dateadd(dd,7,(getdate() + ( 7 - datepart (dw,getdate()) )) ) and
					rpr.releaseplanid in
					(	select	id
						from	ft.releaseplans rp
