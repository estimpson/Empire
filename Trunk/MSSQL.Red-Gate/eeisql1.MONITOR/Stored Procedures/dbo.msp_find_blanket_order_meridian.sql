...
CREATE procedure [dbo].[msp_find_blanket_order_meridian] (
...
select	@ordercount = isnull ( (
		select	count ( 1 )
		  from	order_header oh
			join	part_eecustom on oh.blanket_part = part_eecustom.part and isNULL(nullif(part_eecustom.currentRevLevel,''),'N') = 'Y'
...
--	4. If order count is equal to 0, set orderno to null and return 100 for "order not found."
if @ordercount = 0
begin -- (1aB)
	select	@orderno = null
	return	100
...
--	5. If order count is greater than 1, set orderno to null and return -1 for "error occurred."
...
--	7. order count is equal to 1, set orderno to the order found and return 0 for "success."
select	@orderno = order_no
  from	order_header oh
	join	part_eecustom on oh.blanket_part = part_eecustom.part and isNULL(part_eecustom.currentRevLevel,'N') = 'Y'
	join edi_setups edi on edi.destination = @shipto
 where	customer_part = @customerpart and
	oh.destination = @shipto and
	( customer_po = @customerpo or
