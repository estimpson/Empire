...
CREATE procedure [EDIFnG].[usp_Process]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDIFnG.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDIFnG.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIFnG.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFnG.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFnG.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIFnG.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIFnG.ShipScheduleHeaders'
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFnG.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIFnG.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIFnG.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDIFnG.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDIFnG.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIFnG.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Active'))
...
from
	EDIFnG.ShipScheduleHeaders fh
	join EDIFnG.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIFnG.ShipScheduleAccums fa
	...
	left join EDIFnG.ShipScheduleAuthAccums faa
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIFnG.ShipScheduleHeaders', 'Status', 'Active'))
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
, bo.ReferenceAccum
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDIFnG.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIFnG.PlanningHeaders fh
	join EDIFnG.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	join
		EDIFnG.CurrentPlanningReleases() cpr
	...
	left join EDIFnG.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIFnG.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(cpr.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
		fh.Status = 1 --(select dbo.udf_StatusValue('EDIFnG.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 
order by
	1, 2, 3
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIFnG.PlanningHeaders fh
					join EDIFnG.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIFnG.ShipScheduleHeaders fh
					join EDIFnG.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			...
			join 
				EDIFnG.PlanningHeaders fh
			join 
				EDIFnG.PlanningSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIFnG.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
		...
		from
			...
			join 
				EDIFnG.ShipScheduleHeaders fh
			join 
				EDIFnG.ShipScheduleSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIFnG.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
					and	(	bo.CheckCustomerPOPlanning = 0
...
from
	@Current862s a
join
	EDIFnG.ShipSchedules b on a.RawDocumentGUID = b.RawDocumentGUID 
and
...
Left Join 
	EDIFnG.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
Where
bo.BlanketOrderNo is NULL and COALESCE(a.NewDocument,0)= 1
union
Select 
	TradingPartner = Coalesce((Select max(TradingPartner) from fxEDI.EDI.EDIDocuments where GUID = a.RawDocumentGUID) ,'')
,	DocumentType = 'PR'
...
from
	@Current830s a
left join
	EDIFnG.PlanningReleases b on a.RawDocumentGUID = b.RawDocumentGUID 
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
Where
bo.BlanketOrderNo is not NULL and COALESCE(a.NewDocument,0) = 1
union
Select 
	TradingPartner = Coalesce((Select max(TradingPartner) from fxEDI.EDI.EDIDocuments where GUID = a.RawDocumentGUID) ,'')
,	DocumentType = 'PR'
...
from
	@Current862s a
join
	EDIFnG.ShipScheduleAccums b on a.RawDocumentGUID = b.RawDocumentGUID 
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
join
	EDIFnG.ShipScheduleAuthAccums c on a.RawDocumentGUID = c.RawDocumentGUID 
and
...
AND
(	bo.CheckModelYearPlanning = 0
	OR bo.ModelYear = b.CustomerModelYear AND COALESCE(a.NewDocument,0) = 1
)
WHERE
...
FROM
	@Current830s a
JOIN
	EDIFnG.PlanningAccums b ON a.RawDocumentGUID = b.RawDocumentGUID 
AND
	a.CustomerPart = b.CustomerPart
AND
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
JOIN
	EDIFnG.PlanningAuthAccums c ON a.RawDocumentGUID = c.RawDocumentGUID 
AND
	a.CustomerPart = c.CustomerPart
AND
	a.ShipToCode = c.ShipToCode
...
WHERE
	...
	COALESCE(bo.AccumShipped,0) != COALESCE(b.LastAccumQty,0) AND COALESCE(a.NewDocument,0) = 1
...
BEGIN
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDIFnG Alert' 
	...
	,	@copy_recipients = 'vgarcia@empireelect.com' -- varchar(max)
	, 	@subject = @EmailHeader
	,  	@body = @EmailBody
	,  	@body_format = 'HTML'
