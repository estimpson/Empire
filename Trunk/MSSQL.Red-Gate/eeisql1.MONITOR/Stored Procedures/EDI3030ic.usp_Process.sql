...
SET ANSI_NULLS ON
GO
CREATE procedure [EDI3030ic].[usp_Process]
...
,   NewDocument
from
	EDI3030ic.CurrentShipSchedules ()
...
,   NewDocument
from
	EDI3030ic.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI3030ic.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3030ic.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3030ic.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI3030ic.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3030ic.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI3030ic.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI3030ic.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI3030ic.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI3030ic.ShipScheduleHeaders fh
...
from
	EDI3030ic.ShipScheduleHeaders fh
	join EDI3030ic.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3030ic.ShipScheduleAccums fa
	...
	left join EDI3030ic.ShipScheduleAuthAccums faa
	...
	join EDI3030ic.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI3030ic.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI3030ic.PlanningHeaders fh
	join EDI3030ic.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI3030ic.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI3030ic.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI3030ic.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI3030ic.PlanningHeaders fh
					join EDI3030ic.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI3030ic.ShipScheduleHeaders fh
					join EDI3030ic.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI3030ic.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI3030ic.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI3030ic.ShipSchedules b
 Join 
	EDI3030ic.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI3030ic.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI3030ic.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI3030ic.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDI3030ic.PlanningAccums pra on 
 	...
 	left join
		EDI3030ic.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI3030ic' 
...
execute
	@ProcReturn = EDI3030ic.usp_Process
	@TranDT = @TranDT out
