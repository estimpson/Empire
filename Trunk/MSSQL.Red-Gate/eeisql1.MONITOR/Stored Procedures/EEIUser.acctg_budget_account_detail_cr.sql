...
CREATE procedure [EEIUser].[acctg_budget_account_detail_cr]
...
union
select distinct(gct.ledger_account) from gl_cost_transactions gct where gct.ledger = 'EMPIRE' and gct.fiscal_year = @fiscal_year and gct.update_balances = 'Y' and len(gct.ledger_account)=6 and right(gct.ledger_account,2) in (select organization from #selected_organizations) and left(gct.ledger_account,4) > '5025'
end
else
begin
insert into #authorized_ledger_accounts
...
delete from #authorized_ledger_accounts where ledger_account in ('506012','506212','506712','507212','507312','507412','507512','507812','507912','509012','509112','509212')
...
from	(	select	isnull (a1.ledger_account, a2.ledger_account) as mtd_ledger_account,
		...
		from	(	select gct.ledger_account as ledger_account,
				...
				where gct.fiscal_year = @Fiscal_Year and
                    ...
                    right(gct.ledger_account,2) in ('11','12','21','60') and
					left(gct.ledger_account,4) >= '5025' and
			...
			(	select	ab.ledger_account,
				...
				where	ab.fiscal_year = @fiscal_year
						...
						and right(ab.ledger_account,2) in ('11','12','21','60')
						and left(ab.ledger_account,4) >= '5025'
...
order by f1.ledger_account, budget_line
GO
