...
CREATE procedure [EDICarPlastics].[usp_Process]
...
,   NewDocument
from
	EDICarPlastics.CurrentShipSchedules ()
...
,   NewDocument
from
	EDICarPlastics.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDICarPlastics.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDICarPlastics.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDICarPlastics.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDICarPlastics.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDICarPlastics.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDICarPlastics.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDICarPlastics.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDICarPlastics.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDICarPlastics.ShipScheduleHeaders fh
...
from
	EDICarPlastics.ShipScheduleHeaders fh
	join EDICarPlastics.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDICarPlastics.ShipScheduleAccums fa
	...
	left join EDICarPlastics.ShipScheduleAuthAccums faa
	...
	join EDICarPlastics.BlanketOrders bo
...
and	fh.Status = 1 --(select dbo.udf_StatusValue('EDICarPlastics.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDICarPlastics.PlanningHeaders fh
	join EDICarPlastics.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDICarPlastics.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDICarPlastics.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
				c.RawDocumentGUID = fr.RawDocumentGUID
	and	fh.Status = 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDICarPlastics.PlanningReleases', 'Status', 'Active'))
  ...
  ,@Result3 output
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDICarPlastics.PlanningHeaders fh
					join EDICarPlastics.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDICarPlastics.ShipScheduleHeaders fh
					join EDICarPlastics.ShipScheduleSupplemental sss
		...
		*/
		/*
		insert
			EDICarPlastics.LabelInfoHeader
		(	SystemDT
		)
		select
			@TranDT
	...
	--- <Update rows="*">
		...
		from
			dbo.order_header oh
		join
				EDICarPlastics.blanketOrders bo
		on
		...
		join
		  ...
		  join
				EDICarPlastics.PlanningSupplemental prs
		on
...
update
		...
		JOIN
			EDICarPlastics.PlanningHeaders ph ON ph.RawDocumentGUID = cpr.RawDocumentGUID
		WHERE
		...
		join
		  ...
		  join
				EDICarPlastics.ShipScheduleSupplemental sss
		on
	...
	/*update
		...
		from
			...
			join 
				EDICarPlastics.ShipScheduleHeaders fh
			join 
				EDICarPlastics.ShipSchedules ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDICarPlastics.BlanketOrders bo	on bo.EDIShipToCode =  ps.ShipToCode
					and bo.CustomerPart = ps.CustomerPart
...
( Select 1 from 
		EDICarPlastics.BlanketOrders bo 
	Where
		 a.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDICarPlastics.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
	a.ShipToCode = bo.EDIShipToCode
and
(	bo.CheckCustomerPOShipSchedule = 0
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDICarPlastics.ShipScheduleAccums ssa on 
 	...
 	left join
		EDICarPlastics.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDICarPlastics.PlanningAccums pra on 
 	...
 	left join
		EDICarPlastics.PlanningAuthAccums praa on 
...
Begin
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDICarPlastics' 
...
execute
	@ProcReturn = EDICarPlastics.usp_Process
	@TranDT = @TranDT out
