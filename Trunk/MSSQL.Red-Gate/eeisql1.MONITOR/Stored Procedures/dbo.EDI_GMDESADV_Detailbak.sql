...
CREATE PROCEDURE [dbo].[EDI_GMDESADV_Detailbak] (@Shipper INT)
...
SELECT	at.serial,
		...
		(CASE WHEN (pm.type IS NULL OR atp.package_type IS NULL OR at.package_type IS NULL) THEN '4' WHEN ((pm.type = 'B' AND pm.returnable = 'N') OR (ppm.type = 'P' AND ppm.returnable = 'N')) THEN '4' ELSE '1' END ),
...
while
	...
	Insert	#DESADVFlatFileLines (LineData)
	Select	'10' + convert(char(10), 2*(COUNT(ObjectSerial))) + CONVERT(CHAR(17), LEFT(PalletPackCode,8)) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY PalletPackCode
	UNION
	...
	Insert	#DESADVFlatFileLines (LineData)
	Select	'16' + SPACE(3) + convert(char(16), MAX(AccumShipped)) + CONVERT(CHAR(3), MAX(UM)) + CONVERT(CHAR(16), sum(ObjectQuantity)) + CONVERT(CHAR(3), MAX(UM))  FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPart
	
	Insert	#DESADVFlatFileLines (LineData)
	Select	'17' + CONVERT(CHAR(35),CustomerPO ) FROM @PartPackTypeSerials where CustomerPart = @part GROUP BY CustomerPO
