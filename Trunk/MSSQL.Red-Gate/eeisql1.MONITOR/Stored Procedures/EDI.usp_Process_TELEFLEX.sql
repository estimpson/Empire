...
CREATE procedure [EDI].[usp_Process_TELEFLEX]
	@TranDT datetime out
,	@Result integer out
,	@Testing int = 1
--<Debug>
...
,	fr.CustomerPart
,	case
		when fr.status = 0 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'New'))
			then 1
		else 0
	end
from
	EDI.TELEFLEX_DELFOR_Releases fr
	join EDI.TELEFLEX_DELFOR_Headers fh
		on fh.RawDocumentGUID = fr.RawDocumentGUID
where
	fr.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'Active'))
	)
	and fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'Active'))
	)
	and fh.DocumentDT =
	(	select
			max(fh2.DocumentDT)
		from
			EDI.TELEFLEX_DELFOR_Headers fh2
			join EDI.TELEFLEX_DELFOR_Releases fr2
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'Replaced'))
		end
from
	EDI.TELEFLEX_DELFOR_Releases fr
...
from
	EDI.TELEFLEX_DELFOR_Cumulatives fr
	left join @CurrentDELFORs c
		on fr.RawDocumentGUID = c.RawDocumentGUID
		and fr.ShipToCode = c.ShipToCode
		and fr.CustomerPart = c.CustomerPart
		...
		when exists
			(	select
				...
				where
					...
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'Replaced'))
	end
from
	EDI.TELEFLEX_DELFOR_Headers fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'Active'))
...
if @Debug & 1 = 1 begin
	...
	print	'	Calculate raw releases from active  DELFORs.'
	set	@StartDT = GetDate ()
end
--</Debug>
/*	Write new releases. */
...
from
	EDI.TELEFLEX_DELFOR_Headers fh
	join EDI.TELEFLEX_DELFOR_Releases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI.TELEFLEX_DELFOR_Cumulatives dcv
	...
	left join TELEFLEXEDI.BlanketOrders bo
	...
	left join @CurrentDELFORs c
		...
		and fr.CustomerPart = c.CustomerPart
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Headers', 'Status', 'Active'))
	and fr.Status = 1--(select dbo.udf_StatusValue('EDI.TELEFLEX_DELFOR_Releases', 'Status', 'Active'))
	and	dcv.status = 1
...
EXECUTE @RC3 = [MONITOR].[dbo].[usp_SaveBlanketOrderDistributedReleases] 
  ...
  ,@Result3 output
	...
	from
		@RawReleases rr
		join TELEFLEXEDI.BlanketOrders bo
...
Declare @EDIOrdersAlert table (
	...
	[Notes] [varchar](5000) NULL
		...
		if exists (	select 1 
						...
						join	
							EDI.TELEFLEX_DELFOR_Releases FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TELEFLEXEDI.BlanketOrders bo	on bo.EDIShipToCode in (FDELFORr.ICCode, FDELFORr.ShipToCode)
						and 
		...
		from (select
						...
						join	
							EDI.TELEFLEX_DELFOR_Releases FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TELEFLEXEDI.BlanketOrders bo	on bo.EDIShipToCode =  FDELFORr.ShipToCode
						and 
							bo.CustomerPart = FDELFORr.CustomerPart
						where 
							bo.BlanketOrderNo is null
					
				) TELEFLEXEDIExceptions
...
SET @tableHTML =
    N'<H1>TELEFLEX EDI Exceptions</H1>' +
    N'<table border="1">' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @subject = N'EDI Data Exception when processing TELEFLEX EDI Document(s)', -- nvarchar(255)
    ...
    @importance = 'High' -- varchar(6)
		...
		-- E-Mail Alerts for Accum discrepancy
			...
			if exists (
						...
						from 
							@CurrentDELFORs FDELFORc
						join	
							EDI.TELEFLEX_DELFOR_Cumulatives FDELFORr on FDELFORc.RawDocumentGUID = FDELFORr.RawDocumentGUID and NewDocument = 1
						left join 
							TELEFLEXEDI.BlanketOrders bo	on bo.EDIShipToCode =  FDELFORr.ShipToCode
						and 
							bo.CustomerPart = FDELFORr.CustomerPart
		...
		from (select
						(select max(ReleaseNo) from EDI.TELEFLEX_DELFOR_Releases SPOR where SPOR.RawDocumentGUID = FDELFORr.RawDocumentGUID and SPOR.CustomerPart = FDELFORr.CustomerPart  ) ReleaseNo, 
						FDELFORr.CustomerPart CustomerPart,
						FDELFORr.CustomerPO CustomerPO,
						FDELFORr.ShipToCode ShipToID,
...
SET @tableHTMLAA =
    N'<H1>TELEFLEX Accum Discrepancy</H1>' +
    N'<table border="1">' +
    N'<tr><th>ReleaseNo</th>' +
    N'<th>CustomerPart</th><th>CustomerPO</th><th>ShipToCode</th>' +
    N'<th>Notes</th></tr>' +
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max)
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - TELEFLEX EDI Document(s) Accum Dicrepancy', -- nvarchar(255)
...
exec msdb.dbo.sp_send_dbmail @profile_name = 'DBMail', -- sysname
    ...
    @copy_recipients = 'dwest@empireelect.com', -- varchar(max)
    --@blind_copy_recipients = 'aboulanger@fore-thought.com;estimpson@fore-thought.com', -- varchar(max)
    @subject = N'EDI Data Alert - TELEFLEX EDI Document(s) Updated FX Sales Orders', -- nvarchar(255)
...
execute
	@ProcReturn = EDI.usp_Process_TELEFLEX
	@TranDT = @TranDT out
