...
--exec ftsp_EDI_ASN_Invoice_Modatec 89973
...
select
		...
		@1REF01TL = 'TaxExemption', 
		@1REF01VXQualifier ='VX' , --GST Registration Number Qual
		@1REF01VX = 'GSTRegNumber', --GST Registration Number 
...
Select @1TDS01InvoiceAmount = substring(convert(varchar(max),round(@1TDS01InvoiceAmountNumeric ,2)),1,patindex('%.%', convert(varchar(max),round(@1TDS01InvoiceAmountNumeric,2)))-1 ) +
				substring(convert(varchar(max),round(@1TDS01InvoiceAmountNumeric ,2)),patindex('%.%', convert(varchar(max),round(@1TDS01InvoiceAmountNumeric ,2)))+1, 2)
				--TXI
Select		@1TXI01GS = 'GS'
