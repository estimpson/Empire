...
CREATE 	PROCEDURE [FT].[sp_report_SchedulingWeekOverWeekDemandAnalysis_BasePart]
...
--Exec [FT].[sp_report_SchedulingWeekOverWeekDemandAnalysis_BasePart]  '2017-02-22', 30
--Declare variables
/* 
Change Log
...
SELECT @PwFirstSnapDT = ( SELECT		min(GeneratedDT) FROM dbo.CustomerReleasePlans WHERE GeneratedWeekNo = @PriorWeekSnapWeekNo)
SELECT @AwLastSnapDT =	( SELECT		max(GeneratedDT) FROM dbo.CustomerReleasePlans WHERE GeneratedWeekNo = @CurrentWeekSnapWeekNo)
...
Update f
set 
	f.QtyShippedBetweenSnapShots = coalesce(( Select sum(qty_packed) from shipper_detail sd where sd.order_no = f.SalesOrderNo and sd.date_shipped>= coalesce(f.PriorWeekSnapShotDate,@PwFirstSnapDT)  and sd.date_shipped< coalesce(f.AnalysisWeekSnapShotDate,@AwLastsnapDT) ),0),
...
--select * from #DemandSnapShotFlat
		...
		TRUNCATE TABLE   Ft.CustomerDemandAnalysisWeekOverWeekBasePart
		INSERT into Ft.CustomerDemandAnalysisWeekOverWeekBasePart
		SELECT 
 ...
 SELECT [AnalysisDT]
	  ...
	  ,[FT].[fn_ReturnScheduler]([Destination]) as Scheduler
      ,[Customer]
      ,[Destination]
      ,wow.[BasePart]
	  ,wow.part
...
,Week4QtyPriorWeekGrpOrder as Wk4GrpOrderPW
  ...
  FROM [FT].[CustomerDemandAnalysisWeekOverWeekbasepArt] wow
  ...
  outer apply ( Select	Sum(week4QtyPriorWeek) Week4QtyPriorWeekGrpOrder,  
						...
						from [FT].[CustomerDemandAnalysisWeekOverWeekBasePart] wowgrpO where wowgrpO.basepart = wow.basepart and  wowgrpO.destination = wow.destination ) GroupedByOrder
	 ...
	 outer apply ( Select	Sum(week4QtyPriorWeek) Week4QtyPriorWeekGrpBP,  
						...
						from [FT].[CustomerDemandAnalysisWeekOverWeekbasepart] wowgrpO where wowgrpO.basepart = wow.basepart  ) GroupedByBasePart
