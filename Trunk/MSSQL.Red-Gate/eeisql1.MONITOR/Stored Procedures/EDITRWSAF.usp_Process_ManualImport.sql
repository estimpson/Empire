...
create procedure [EDITRWSAF].[usp_Process_ManualImport]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDITRWSAF.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDITRWSAF.CurrentPlanningReleases_ManualImport()
...
--- <Update rows="*">
set	@TableName = 'EDITRWSAF.PlanningReleases_ManualImport'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'Replaced'))
		end
from
	EDITRWSAF.PlanningReleases_ManualImport PR
...
where
	PR.Status in
	(	0 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDITRWSAF.PlanningHeaders_ManualImport'
	...
	case
		when exists
			(	select
					*
				from
					EDITRWSAF.PlanningReleases_ManualImport fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders_ManualImport', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders_ManualImport', 'Status', 'Replaced'))
	end
from
	EDITRWSAF.PlanningHeaders_ManualImport fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders_ManualImport', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders_ManualImport', 'Status', 'Active'))
	)
...
from
	EDITRWSAF.PlanningHeaders_ManualImport fh
	join EDITRWSAF.PlanningReleases_ManualImport fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
		/*
	left join EDITRWSAF.PlanningAccums fa
	...
	left join EDITRWSAF.PlanningAuthAccums faa
		...
		*/
	left join EDITRWSAF.BlanketOrders bo
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningHeaders_ManualImport', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDITRWSAF.PlanningReleases_ManualImport', 'Status', 'Active'))
...
if	@Testing = 0 begin
	...
	from
		@RawReleases rr
		join EDITRWSAF.BlanketOrders bo
...
and not exists
( Select 1 from 
		EDITRWSAF.PlanningReleases_ManualImport b
 Join 
	EDITRWSAF.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
	 Join 
	EDITRWSAF.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDITRWSAF.PlanningAccums pra on 
 	...
 	left join
		EDITRWSAF.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDITRWSAF' 
...
execute
	@ProcReturn = EDITRWSAF.usp_Process
	@TranDT = @TranDT out
