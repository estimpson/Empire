...
CREATE procedure [EDIPolycon].[usp_Process]
...
set	@ProcName = user_name(objectproperty(@@procid, 'OwnerId')) + '.' + object_name(@@procid)  -- e.g. EDIPolycon.usp_Test
--- </Error Handling>
...
,   NewDocument
from
	EDIPolycon.CurrentShipSchedules ()
...
,   NewDocument
from
	EDIPolycon.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDIPolycon.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIPolycon.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIPolycon.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDIPolycon.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'Active'))
	)
...
--- <Update rows="*">
set	@TableName = 'EDIPolycon.ShipScheduleHeaders'
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIPolycon.ShipScheduleHeaders ssh
where
	ssh.Status in
...
--- <Update rows="*">
set	@TableName = 'EDIPolycon.PlanningReleases'
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDIPolycon.PlanningReleases PR
...
--- <Update rows="*">
set	@TableName = 'EDIPolycon.PlanningHeaders'
	...
	case
		when exists
			(	select
					*
				from
					EDIPolycon.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDIPolycon.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDIPolycon.ShipScheduleHeaders fh
...
from
	EDIPolycon.ShipScheduleHeaders fh
	join EDIPolycon.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDIPolycon.ShipScheduleAccums fa
	...
	left join EDIPolycon.ShipScheduleAuthAccums faa
	...
	join EDIPolycon.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDIPolycon.ShipScheduleHeaders', 'Status', 'Active'))
group by
	bo.BlanketOrderNo
,	fh.RawDocumentGUID
, bo.ReferenceAccum
...
where
	fh.Status = 1 --(select dbo.udf_StatusValue('EDIPolycon.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDIPolycon.PlanningHeaders fh
	join EDIPolycon.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	join
		ediPolycon.CurrentPlanningReleases() cpr
	...
	left join EDIPolycon.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDIPolycon.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(cpr.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where
		fh.Status = 1 --(select dbo.udf_StatusValue('EDIPolycon.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 
order by
	1, 2, 3
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDIPolycon.PlanningHeaders fh
					join EDIPolycon.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDIPolycon.ShipScheduleHeaders fh
					join EDIPolycon.ShipScheduleSupplemental sss
		...
		*/
		...
		from
			...
			join 
				EDIPolycon.PlanningHeaders fh
			join 
				EDIPolycon.PlanningSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIPolycon.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
		...
		from
			...
			join 
				EDIPolycon.ShipScheduleHeaders fh
			join 
				EDIPolycon.ShipScheduleSupplemental ps on ps.RawDocumentGUID = fh.RawDocumentGUID
			join 
				EDIPolycon.BlanketOrders bo	on bo.EDIShipToCode = coalesce (nullif(ps.ConsigneeCode,''), ps.ShipToCode)
					and bo.CustomerPart = ps.CustomerPart
					and	(	bo.CheckCustomerPOPlanning = 0
...
from
	@Current862s a
join
	EDIPolycon.ShipSchedules b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
...
Left Join 
	EDIPolycon.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
from
	@Current830s a
left join
	EDIPolycon.PlanningReleases b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
from
	@Current862s a
join
	EDIPolycon.ShipScheduleAccums b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
join
	EDIPolycon.ShipScheduleAuthAccums c on a.RawDocumentGUID = c.RawDocumentGUID and a.NewDocument = 1
and
...
from
	@Current830s a
join
	EDIPolycon.PlanningAccums b on a.RawDocumentGUID = b.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = b.CustomerPart
and
	a.ShipToCode = b.ShipToCode
...
--	coalesce(a.ShipFromCode, '') = coalesce(b.ShipFromCode,'')
join
	EDIPolycon.PlanningAuthAccums c on a.RawDocumentGUID = c.RawDocumentGUID and a.NewDocument = 1
and
	a.CustomerPart = c.CustomerPart
and
	a.ShipToCode = c.ShipToCode
...
Select 
		...
		,	@EmailHeader nvarchar(max) = 'EDIPloycon Alert' 
...
execute
	@ProcReturn = EDIPolycon.usp_Process
	@TranDT = @TranDT out
