...
CREATE  PROCEDURE [HN].[HCSP_Inv_Material_Issue_New]( 
		@Operator varchar(5),
		@Serial int, 
		@Machine varchar(10),
...
execute	@ProcReturn = HN.HCSP_Material_Issue
	@Operator = 'MON',
	@Serial = @Serial,
	@Machine = 'PISO11-04',
	@Quantity =  NULL,
	@Custom4 = NULL,
	@Note = 'Test of Material Issue Proc',
...
if	@Excess > 0 and  @Object_Quantity < @Quantity  begin
		...
		(	serial, date_stamp, type, part, quantity, remarks, price,
			...
			user_defined_status, part_name, start_date)
		select	serial = object.serial, date_stamp = @TranDT, 
			...
			user_defined_status = object.user_defined_status, part_name = object.name, object = start_date
		from	object
		...
		insert	Defects
			(	defect_date,	--En lugar de transaction Dt
				Machine, Part, reason, quantity,	--Reason en lugar de defectcode, quantity en lugar de QtyScrapped
				Operator,  Shift,work_order/*, DefectSerial,Comments, AuditTrailid*/)	--work_order en lugar de WODID, Defect_Serial comentado, Comments comentado, AuditTrailId comentado
		select	    TransactionDT = @TranDT, Machine = location,
				Part = object.part, DefectCode = 'Qty Excess',
				QtyScrapped = -(@Quantity  - quantity), Operator = @Operator,
				Shift =  'A', WODID = null/*,
				DefectSerial = serial, Note = 'Excess of the Material Issue',
				@Audit_trailID*/
		from	object
		where	serial = @Serial
...
--		B.	Write to audit_trail.
	...
	(	serial,	date_stamp,
		...
		std_cost,
		user_defined_status,
		tare_weight, Custom4, start_date,
		custom3  )
