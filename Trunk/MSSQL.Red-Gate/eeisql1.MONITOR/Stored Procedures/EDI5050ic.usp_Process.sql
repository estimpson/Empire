...
SET ANSI_NULLS ON
GO
CREATE procedure [EDI5050ic].[usp_Process]
...
,   NewDocument
from
	EDI5050ic.CurrentShipSchedules ()
...
,   NewDocument
from
	EDI5050ic.CurrentPlanningReleases ()
...
--- <Update rows="*">
set	@TableName = 'EDI5050ic.SchipSchedules'
	...
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI5050ic.ShipSchedules', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI5050ic.ShipSchedules', 'Status', 'Replaced'))
		end
from
	EDI5050ic.ShipSchedules ss
...
where
	ss.Status in
	(	0 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Active'))
	)
		...
		when exists
			(	select
				...
				where
					...
					and ss.Status = 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI5050ic.ShipScheduleHeaders ssh
where
	ssh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Active'))
	)
...
set
	Status =
		case
			when c.RawDocumentGUID is not null
				then 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Active'))
			else 2 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Replaced'))
		end
from
	EDI5050ic.PlanningReleases PR
	...
	case
		when exists
			(	select
					*
				from
					EDI5050ic.PlanningReleases fr
				where
					fr.RawDocumentGUID = fh.RawDocumentGUID
					and fr.Status = 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Active')
			) then 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Active'))
		else 2 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Replaced'))
	end
from
	EDI5050ic.PlanningHeaders fh
where
	fh.Status in
	(	0 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'New'))
	,	1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Active'))
...
if	@Testing > 1 begin
	...
	select
		*
	from
		EDI5050ic.ShipScheduleHeaders fh
...
from
	EDI5050ic.ShipScheduleHeaders fh
	join EDI5050ic.ShipSchedules fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI5050ic.ShipScheduleAccums fa
	...
	left join EDI5050ic.ShipScheduleAuthAccums faa
	...
	join EDI5050ic.BlanketOrders bo
...
where
	...
	and fh.Status = 1 --(select dbo.udf_StatusValue('EDI5050ic.ShipScheduleHeaders', 'Status', 'Active'))
...
from
	EDI5050ic.PlanningHeaders fh
	join EDI5050ic.PlanningReleases fr
		on fr.RawDocumentGUID = fh.RawDocumentGUID
	left join EDI5050ic.PlanningAccums fa
		on fa.RawDocumentGUID = fh.RawDocumentGUID
		and fa.CustomerPart = fr.CustomerPart
		and	fa.ShipToCode = fr.ShipToCode
		and	coalesce(fa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
		and	coalesce(fa.CustomerModelYear,'') = coalesce(fr.CustomerModelYear,'')
	left join EDI5050ic.PlanningAuthAccums faa
		on faa.RawDocumentGUID = fh.RawDocumentGUID
		and faa.CustomerPart = fr.CustomerPart
		and	faa.ShipToCode = fr.ShipToCode
		and	coalesce(faa.CustomerPO,'') = coalesce(fr.CustomerPO,'')
...
where		c.RawDocumentGUID = fr.RawDocumentGUID
	and		fh.Status = 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningHeaders', 'Status', 'Active'))
	and		fr.Status = 1 --(select dbo.udf_StatusValue('EDI5050ic.PlanningReleases', 'Status', 'Active'))
...
where
	...
	/*if  exists
		(	select
			...
			from
				dbo.order_header oh
				join EDI5050ic.PlanningHeaders fh
					join EDI5050ic.PlanningSupplemental ps
		...
		(	select
			...
			from
				dbo.order_header oh
				join EDI5050ic.ShipScheduleHeaders fh
					join EDI5050ic.ShipScheduleSupplemental sss
		...
		*/
		...
		join
		  ...
		  join
				EDI5050ic.PlanningSupplemental prs
		on
		...
		join
		  ...
		  join
				EDI5050ic.ShipScheduleSupplemental sss
		on
...
and not exists
( Select 1 from 
		EDI5050ic.ShipSchedules b
 Join 
	EDI5050ic.BlanketOrders bo on b.CustomerPart = bo.CustomerPart
and
...
from
	@Current862s a
	 Join 
	EDI5050ic.BlanketOrders bo on a.CustomerPart = bo.CustomerPart
and
...
(	bo.CheckModelYearShipSchedule = 0
	or bo.ModelYear862 = a.CustomerModelYear)
	left join
		EDI5050ic.ShipScheduleAccums ssa on 
 	...
 	left join
		EDI5050ic.ShipScheduleAuthAccums ssaa on 
...
(	bo.CheckModelYearPlanning = 0
	or bo.ModelYear830 = a.CustomerModelYear)
	left join
		EDI5050ic.PlanningAccums pra on 
 	...
 	left join
		EDI5050ic.PlanningAuthAccums praa on 
...
SELECT 
		...
		,	@EmailHeader NVARCHAR(MAX) = 'EDI Processing for EDI5050ic' 
...
execute
	@ProcReturn = EDI5050ic.usp_Process
	@TranDT = @TranDT out
