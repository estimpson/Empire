...
CREATE procedure [dbo].[eeisp_rpt_sales_forecast_weekly] 
       as
begin
		...
		@firstdayofpriorweek		datetime,
			...
			@firstdayofweek19 			datetime,
       		@firstdayofweek20 			datetime,
       		@firstdayofweek21 			datetime,
       		@firstdayofweek22 			datetime,
			@firstdayofweek23 			datetime,
		...
		Select	@firstdayofweek20 = Dateadd(wk, 20, ft.fn_truncDate('wk', @currentdate))
       	Select	@firstdayofweek21 = Dateadd(wk, 21, ft.fn_truncDate('wk', @currentdate))
        Select	@firstdayofweek22 = Dateadd(wk, 22, ft.fn_truncDate('wk', @currentdate))
	...
	select	Company,
		...
		Week19Sales = coalesce (sum (case when SalesHistoryAndForecast.type = 'Forecast' and SalesHistoryAndForecast.ShipDT >= @firstdayofweek19 and SalesHistoryAndForecast.ShipDT < @firstdayofweek20 then SalesHistoryAndForecast.Amount end), 0),
		Week20Sales = coalesce (sum (case when SalesHistoryAndForecast.type = 'Forecast' and SalesHistoryAndForecast.ShipDT >= @firstdayofweek20 and SalesHistoryAndForecast.ShipDT < @firstdayofweek21 then SalesHistoryAndForecast.Amount end), 0),
		Week21Sales = coalesce (sum (case when SalesHistoryAndForecast.type = 'Forecast' and SalesHistoryAndForecast.ShipDT >= @firstdayofweek21 and SalesHistoryAndForecast.ShipDT < @firstdayofweek22 then SalesHistoryAndForecast.Amount end), 0),
		Week22Sales = coalesce (sum (case when SalesHistoryAndForecast.type = 'Forecast' and SalesHistoryAndForecast.ShipDT >= @firstdayofweek22 and SalesHistoryAndForecast.ShipDT < @firstdayofweek23 then SalesHistoryAndForecast.Amount end), 0),
	...
	from	(	select	Team = team, Type = 'Forecast', Customer = left (BasePart, 3), Part = basepart, ShipDT = date_due, Qty = qty_projected, Amount = extended, Company = Company
			from	vw_eei_sales_forecast
			union all
			select	Team = team, Type = 'History', Customer = left (BasePart, 3), Part = basepart, ShipDT = date_shipped, Qty = qty_shipped, Amount = extended, Company=Company
