...
SET ANSI_NULLS ON
GO
Create procedure [dbo].[eeisp_EDI_process_CAMI_DELJIT]
...
JOIN		edi_CAMIDELJIT_Dock on	#orderstoupdate.customerPart = RTRIM(edi_CAMIDELJIT_Dock.CamiPart) 
...
insert  #DELJITSchedue(			customerpart	,
	...
	select 	rtrim(edi_CAMIDELJIT_Detail.CAMIPart) as custpart,
			...
			'',
			rtrim(edi_CAMIDELJIT_Detail.Relno)+'*'+rtrim(edi_CAMIDELJIT_rff.RFFItem),
			'N',
	...
	left join	edi_CAMIDELjit_address on edi_CAMIDELJit_Detail.Relno = edi_CAMIDELjit_address.Relno and edi_CAMIDELjit_Detail.RelProcId = edi_CAMIDELjit_address.RelProcId and edi_CAMIDELjit_address.RelProcId =  @CurrentID and rtrim(edi_CAMIDELjit_address.addressType) = 'ST'
	join		edi_CAMIDELjit_rff on edi_CAMIDELjit_Detail.CAMIPart = edi_CAMIDELjit_rff.CAMIPart   and  rtrim(edi_CAMIDELjit_Detail.Relno) = @LastSchedule and edi_CAMIDELjit_Detail.RelProcId =  @CurrentID and edi_CAMIDELJIT_rff.RelProcId =  @CurrentID and  edi_CAMIDELjit_rff.RelProcId =  @CurrentID and rtrim(edi_CAMIDELjit_rff.rffType) = 'ON'
	left join	edi_setups  ON  rtrim(edi_CAMIDELjit_address.AddressID) = COALESCE(edi_setups.parent_destination, edi_setups.destination) and rtrim(edi_CAMIDELjit_address.AddressType) = 'ST'
