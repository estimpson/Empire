...
CREATE trigger [EEIUser].[tr_QT_EngineeringMaterialsInitials_RowModified] on [EEIUser].[QT_EngineeringMaterialsInitials] after update
as
declare
	@TranDT datetime
,	@Result int
...
begin try
	...
	update
		ql
	set
		ql.EngineeringMaterialsInitials = i.Initials
	from
		EEIUser.QT_QuoteLog ql
		join deleted d
			on d.Initials = ql.EngineeringMaterialsInitials
		join inserted i
			on i.ID = d.ID
...
*/
GO
ALTER TABLE [EEIUser].[QT_EngineeringMaterialsInitials] ADD CONSTRAINT [PK__QT_Engin__3214EC270EE38B9F] PRIMARY KEY CLUSTERED  ([ID]) ON [PRIMARY]
GO
