...
CREATE trigger [EEIUser].[tr_QT_QuoteReviewInitials_RowModified] on [EEIUser].[QT_QuoteReviewInitials] after update
as
declare
	@TranDT datetime
,	@Result int
...
begin try
	...
	update
		ql
	set
		ql.QuoteReviewInitials = i.Initials
	from
		EEIUser.QT_QuoteLog ql
		join deleted d
			on d.Initials = ql.QuoteReviewInitials
		join inserted i
			on i.ID = d.ID
...
*/
GO
ALTER TABLE [EEIUser].[QT_QuoteReviewInitials] ADD CONSTRAINT [PK__QT_Quote__3214EC2701004BF3] PRIMARY KEY CLUSTERED  ([ID]) ON [PRIMARY]
GO
