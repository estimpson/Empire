...
[LastUser] [sys].[sysname] NOT NULL CONSTRAINT [DF__bill_of_m__LastU__6D630406] DEFAULT (suser_sname()),
...
begin
	...
	(	select	1
		from	bill_of_material_ec
			join inserted on bill_of_material_ec.ID = inserted.ID
		where	bill_of_material_ec.LastUser != system_user) begin
		update	bill_of_material_ec
		set	LastUser = system_user
		from	bill_of_material_ec
			join inserted on bill_of_material_ec.ID = inserted.ID
		where	bill_of_material_ec.LastUser != system_user
	end
...
GO
ALTER TABLE [dbo].[bill_of_material_ec] ADD CONSTRAINT [PK__bill_of_material__6B7ABB94] PRIMARY KEY CLUSTERED  ([ID]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bill_of_material_ec] ADD CONSTRAINT [UQ__bill_of_material__6C6EDFCD] UNIQUE NONCLUSTERED  ([parent_part], [part], [start_datetime]) ON [PRIMARY]
GO
