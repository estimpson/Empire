...
CREATE VIEW [dbo].[eei_vw_projectedreceipts4weeks_all]
AS
SELECT		'MON' as POtype,
					po_detail.vendor_code,
					po_header.terms as systemterms,
					venderterms.terms as kensterms,
					(CASE WHEN venderterms.terms like '%30%' then 30 when venderterms.terms like '%45%' then 45 when venderterms.terms like '%COD%' then 0 WHEN venderterms.terms like '%15%' then 15 WHEN venderterms.terms like '%1' then 1 else 30 end) as netdays,
...
SELECT		'EMP' as POtype,
					...
					po_headers.terms as systemterms,
					po_headers.terms as kensterms,
					...
					4 as FirmWeeks,
					po_headers.purchase_order as PONumber
