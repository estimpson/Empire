...
CREATE view [dbo].[vw_eei_inventory_onhold_nonService]
...
from	object
	...
	(	select	serial,
		...
		from	audit_trail
		where	type = 'U' and
			date_stamp <= DateAdd (day, -30, getdate())
		group by
			audit_trail.serial
		union
		select	serial,
		...
		from	audit_trail
		where	type = 'Q' and
			date_stamp <= DateAdd (day, -30, getdate()) and
...
where	object.status <> 'A' and object.part not in (select part from eeiVW_MG)  and isNULL(location.secured_location,'N') <> 'Y' 
