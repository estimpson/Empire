...
FROM (
		Select	Location= code,
				...
				, Date_to_Cycle = Datediff(Day,(	CASE WHEN LastycleDT < (	CASE WHEN plant = 'EEI' THEN '2016-11-29' 
													...
													END ),GETDATE())
		 from	location
			left join (Select	Location = replace(from_loc,'-FIS',''), LastycleDT =  max(date_stamp)
						from	audit_trail
						where	from_loc like '%fis'
		...
		where location.code not like '%-fis' and group_no like '%Warehouse'
	) Data 
