...
CREATE view [dbo].[vw_eei_inventory_idle_nonService]
as
...
from	object
	...
	LEFT join location on object.location = location.code --AND location.plant in ( 'EEI' ,'EEP' ,'EEH', 'ES3')
	join
...
Where	not exists (Select 1 from audit_trail where audit_trail.serial = object.serial and audit_trail.type in ('U','J','A','B','R')) and
			object.quantity > 0  and location not like '%PREOBJECT%'
