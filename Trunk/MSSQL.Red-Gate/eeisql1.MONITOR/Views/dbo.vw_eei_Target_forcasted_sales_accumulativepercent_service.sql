...
CREATE view [dbo].[vw_eei_Target_forcasted_sales_accumulativepercent_service]
as
Select	BasePartSales,
		...
		Sales,
		PercentofTotalSales,
		(Select sum(percentofTotalSales) from vw_eei_Target_forcasted_sales_percent_service sp2 where sp2.PercentofTotalSales>= vw_eei_Target_forcasted_sales_percent_service.percentofTotalSales)  as AccumulativePercent,
		...
		isNULL((select max(cost_cum) from part_standard join part_eecustom on part_standard.part = part_eecustom.part and isNULL(currentRevLevel, 'N') = 'Y' where part_standard.part like '%-%' and substring(part_standard.part, 1, ISNULL((PATINDEX( '%-%',part_standard.part)),0)-1)= BasePart),1) as UnitCostAccum,
...
from	vw_eei_Target_forcasted_sales_percent_service
GO
