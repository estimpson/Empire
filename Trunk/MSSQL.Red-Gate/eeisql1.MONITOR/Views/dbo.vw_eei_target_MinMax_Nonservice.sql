...
CREATE view [dbo].[vw_eei_target_MinMax_Nonservice]
as
select	BasePartSales,
		Ceiling((BasePartQty/12.9)/StdPack)*StdPack as AvgWeeklyDemand,
		ceiling(Inventory/(CASE WHEN (Ceiling((BasePartQty/90)/StdPack)*StdPack)= 0 THEN 1 ELSE (Ceiling((BasePartQty/90)/StdPack)*StdPack) END )) as DaysOnHand,
		BasePart,
		Sales,
		PercentofTotalSales,
		AccumulativePercent,
...
from	vw_eei_Target_forcasted_sales_accumulativepercent_nonservice
GO
