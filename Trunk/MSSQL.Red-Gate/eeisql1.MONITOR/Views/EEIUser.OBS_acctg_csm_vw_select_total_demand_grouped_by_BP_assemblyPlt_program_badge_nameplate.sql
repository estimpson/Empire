...
CREATE VIEW [EEIUser].[OBS_acctg_csm_vw_select_total_demand_grouped_by_BP_assemblyPlt_program_badge_nameplate]
as
select
f.sales_parent,
f.base_part as base_part,
isNull(Assembly_Plant, 'No CSM Program Assigned') as AssemblyPlant,
isNull(program, 'No CSM Program Assigned') as Program,
isNull(Badge, 'No CSM Program Assigned') as Badge,
isNull(NamePlate, 'No CSM Program Assigned') as NamePlate,
...
sum(f.total_2009)+isNull(sum(adj_2009),0) as total_2009,
...
sum(f.total_2010)+isNull(sum(adj_2010),0) as total_2010,
...
sum(f.total_2011)+isNull(sum(adj_2011),0) as total_2011,
isNull(sum(adj_2009),0) as CSMAdj2009,
isNull(sum(adj_2010),0) as CSMAdj2010,
isNull(sum(adj_2011),0) as CSMAdj2011
from 
(select 
a.base_part, a.empire_market_segment, assembly_plant, program, badge, nameplate, a.Empire_EOP, a.Empire_SOP, b.sales_parent,
...
(case b.version when 'Empire' then ISNULL(b.dec_11,0) else a.qty_per*a.take_rate*a.family_allocation*b.dec_11 end) as dec_11, 
 a.qty_per*a.take_rate*a.family_allocation*adj_2009 as adj_2009,
 a.qty_per*a.take_rate*a.family_allocation*adj_2010 as adj_2010,
 a.qty_per*a.take_rate*a.family_allocation*adj_2011 as adj_2011,
...
left outer join 
(select * from eeiuser.acctg_csm_NACSM 
		left join eeiuser.CSMAdjustments on eeiuser.acctg_csm_NACSM.Program = adj_program and
				eeiuser.acctg_csm_NACSM.Assembly_plant = adj_assembly_plant and
				eeiuser.acctg_csm_NACSM.badge = adj_badge and
				eeiuser.acctg_csm_NACSM.nameplate = adj_nameplate and
				eeiuser.acctg_csm_NACSM.release_id = adj_release_id
where  release_id = (Select	[dbo].[fn_ReturnLatestCSMRelease] ('CSM') ) and version = 'CSM' and (ISNULL( jan_09,0)+ISNULL( feb_09,0)+ISNULL( mar_09,0)+ISNULL( apr_09,0)+ISNULL( may_09,0)+ISNULL( jun_09,0)+ISNULL( jul_09,0)+ISNULL( aug_09,0)+ISNULL( sep_09,0)+ISNULL( oct_09,0)+ISNULL( nov_09,0)+ISNULL( dec_09,0)+ISNULL( jan_10,0)+ISNULL( feb_10,0)+ISNULL( mar_10,0)+ISNULL( apr_10,0)+ISNULL( may_10,0)+ISNULL( jun_10,0)+ISNULL( jul_10,0)+ISNULL( aug_10,0)+ISNULL( sep_10,0)+ISNULL( oct_10,0)+ISNULL( nov_10,0)+ISNULL( dec_10,0))>0) b
...
group by sales_parent, base_part, isNull(program, 'No CSM Program Assigned'),isNull(Assembly_Plant, 'No CSM Program Assigned') ,
isNull(program, 'No CSM Program Assigned'),
isNull(Badge, 'No CSM Program Assigned') ,
isNull(NamePlate, 'No CSM Program Assigned') 
having (abs(sum(f.total_2009)) +abs(sum(f.total_2010))+ abs(sum(f.total_2011)))>0
