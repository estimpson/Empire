...
CREATE VIEW [EEIUser].[OBS_acctg_csm_vw_select_total_demand_grouped_by_BP_program_badge]
as
select
f.sales_parent,
...
from 
(select 
a.base_part, a.empire_market_segment, program, badge, a.Empire_EOP, b.sales_parent,
...
group by sales_parent, base_part, isNull(program, 'No CSM Program Assigned'),isNull((Badge), 'No CSM Program Assigned') 
having (abs(sum(f.total_2009)) +abs(sum(f.total_2010)))>0
GO
