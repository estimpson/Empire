...
-- Select * From  [dbo].[Label_FinishedGoodsData] where Shipper = 90151
SELECT
	*
...
FROM
	(	SELECT
		...
		,	ShippedDate = CONVERT(VARCHAR(10), GETDATE(), 101)
...
--		,	Customer = oh.customer
		,	DockCode =  Coalesce(nullif(oh.dock_code,''), s.shipping_dock)
		...
		, SLALot =  CASE DATEPART(YYYY, o.last_date)
							WHEN 2013 THEN 'I'
							WHEN 2014 THEN 'J'
							WHEN 2015 THEN 'K'
							WHEN 2016 THEN 'L'
							...
							WHEN 2019 THEN 'O'
							END
						+   CASE DATEPART(MONTH, o.last_date)
							WHEN 1 THEN 'A'
							WHEN 2 THEN 'B'
							WHEN 3 THEN 'C'
							WHEN 4 THEN 'D'
							...
							WHEN 31 THEN '5'
							END
						,SerialSLA =  RIGHT(('0000000000' + CONVERT(VARCHAR, o.serial)), 10) +
								 CASE DATEPART(YYYY, o.last_date)
							WHEN 2013 THEN 'I'
							WHEN 2014 THEN 'J'
							WHEN 2015 THEN 'K'
							WHEN 2016 THEN 'L'
		...
		--	Fields on every label    
			Serial = at.serial
		,	Quantity = CONVERT (INT, at.quantity)
		,	CustomerPart = oh.customer_part
		,	CompanyName = param.company_name
		,	CompanyAddress1 = param.address_1
		,	CompanyAddress2 = param.address_2
		...
		,	CompanyPhoneNumber = param.phone_number
		--	Fields on some labels ...
		,	LicensePlate = 'UN' + es.supplier_code + '' + CONVERT(VARCHAR, at.serial) 
		,	SerialCooper =  RIGHT(('000000000' + CONVERT(VARCHAR, at.serial)), 9)
		,	SerialPaddedNineNines =  RIGHT(('999999999' + CONVERT(VARCHAR, at.serial)), 9)
		,	EDISetupsparentDestination = es.parent_destination
		,	EDISetupsMaterialIssuer = es.material_issuer
		,	InterBoxLicPlate= COALESCE(sd.Customer_part, oh.Customer_part) +'S' + RIGHT(('999999999' + CONVERT(VARCHAR, at.serial)), 9) + 'Q' + CONVERT(VARCHAR(15), CONVERT( INT, at.quantity ))
		,	TELFBoxLicPlate = 'ZZ' + es.supplier_code + RIGHT(('0000000000' + CONVERT(VARCHAR(15), at.serial)), 10)
		,	PartNumber = at.part
		,	UnitOfMeasure =  at.unit
		,	Location = at.to_loc
		,	Lot = at.lot
		,	Operator = at.operator
		,	MfgDate = CONVERT(VARCHAR(10), at.date_stamp, 101)
		,	MfgTime = CONVERT(VARCHAR(5), at.date_stamp, 108)
		,	ShippedDate = CONVERT(VARCHAR(10), GETDATE(), 101)
		--,	MfgDateMM = case when rl.name in ('Ford_Part Container') then convert(varchar(6), at.date_stamp, 12) end
		,	MfgDateMMM = UPPER(REPLACE(CONVERT(VARCHAR, at.date_stamp, 106), ' ', ''))
		,	MfgDateMMMDashes = UPPER(REPLACE(CONVERT(VARCHAR, at.date_stamp, 106), ' ', '-'))
		,	GrossWeight = CONVERT(NUMERIC(10,2), ROUND(s.gross_weight,2))
		,	GrossWeightKilograms = CONVERT(NUMERIC(10,0),((at.weight + at.tare_weight) / 2.2))
		,	NetWeightKilograms = CONVERT(NUMERIC(10,0),((at.weight) / 2.2))
		,	NetWeight = CONVERT(NUMERIC(10,2), ROUND(at.weight,2))
		,	TareWeight = CONVERT(NUMERIC(10,2), ROUND(at.tare_weight,2))
		,	StagedObjects = s.staged_objs
--		,	Origin = at.origin
...
--		,	Customer = oh.customer
		...
		,	DestinationAddress3 = d.address_3
		,	DestinationAddress4 = d.address_4
		,	ObjectKANBAN = COALESCE(at.kanban_number, '') 
		,	ObjectCustom5 = COALESCE(at.custom5, '')
		,	ShipToID = COALESCE(es.ediShipToID, es.parent_destination)
		,	PO_or_Release =  
				CASE
				WHEN NULLIF(sd.customer_po,'') IS NULL
		...
		, ReleaseNumber =  sd.release_no
		, ShipperDateStamp = s.date_stamp
		, DecoPlasBarcode = 'A'+oh.customer_part +'  '+'00'+CONVERT(VARCHAR(15), CONVERT(INT, at.quantity))+'.000'+'EA'+'A'+CONVERT(VARCHAR(15),at.serial)
		, OHCustom01 = oh.custom01
		, OHcustom02 = oh.custom02
		, OHCustom03 = oh.custom03
		--,	DateTimeZoneDue = CASE WHEN rl.NAME IN ('MITSUBISHI_RAN') THEN mran.date_time_zone_due END
		...
		--	Make sure we printed the correct label format.
		,	BoxLabelFormat = rl.NAME
		, SLALot =  CASE DATEPART(YYYY, at.date_stamp)
							WHEN 2013 THEN 'I'
							WHEN 2014 THEN 'J'
							WHEN 2015 THEN 'K'
							WHEN 2016 THEN 'L'
							...
							WHEN 2019 THEN 'O'
							END
						+   CASE DATEPART(MONTH, at.date_stamp)
							WHEN 1 THEN 'A'
							WHEN 2 THEN 'B'
							WHEN 3 THEN 'C'
							WHEN 4 THEN 'D'
							...
							WHEN 12 THEN 'L'
							END +
							   CASE DATEPART(DAY, at.date_stamp)
							WHEN 1 THEN 'A'
							WHEN 2 THEN 'B'
							WHEN 3 THEN 'C'
							WHEN 4 THEN 'D'
							...
							WHEN 31 THEN '5'
							END
						,SerialSLA =  RIGHT(('0000000000' + CONVERT(VARCHAR, at.serial)), 10) +
								 CASE DATEPART(YYYY, at.date_stamp)
							WHEN 2013 THEN 'I'
							WHEN 2014 THEN 'J'
							WHEN 2015 THEN 'K'
							WHEN 2016 THEN 'L'
		...
		FROM
			dbo.audit_trail at
			LEFT JOIN (SELECT MAX(engineering_level) AS engineering_level, part AS ecn_part FROM dbo.effective_change_notice GROUP BY part) ecn ON
				ecn.ecn_part = at.part
			LEFT JOIN dbo.shipper s
				JOIN dbo.shipper_detail sd
					ON sd.shipper = s.id
				ON convert(varchar(25), s.id )= COALESCE(at.shipper, CASE WHEN at.origin NOT LIKE '%[^0-9]%' AND LEN(at.origin) < 10 THEN CONVERT (INT, at.origin) END)
				AND sd.part_original = at.part
			LEFT JOIN dbo.order_header oh ON
				oh.order_no = COALESCE(sd.order_no, CASE WHEN at.origin NOT LIKE '%[^0-9]%' AND LEN(at.origin) < 10 THEN CONVERT(INT, at.origin) END)
				AND oh.blanket_part = at.part
			LEFT JOIN dbo.destination d ON
				d.destination = COALESCE(s.destination, oh.destination, at.destination)
			LEFT JOIN dbo.edi_setups es ON
				es.destination = COALESCE(s.destination, oh.destination, at.destination)
			--LEFT JOIN (SELECT MAX(CONVERT(VARCHAR(10),PickUpDT,105)) AS date_time_zone_due, RAN FROM EDIMitsubishi.RanDetails GROUP BY RAN) mran ON
			--	mran.RAN = o.custom1
			JOIN dbo.part p ON
				p.part = at.part
			...
			JOIN dbo.report_library rl ON
				rl.name = COALESCE(oh.box_label, pi.label_format)			
			CROSS JOIN dbo.parameters param
			Where at.type= 'S' and not exists ( select 1 from object o2 where o2.serial = at.serial)
	) rawLabelData
