...
as
		...
		Fis_amount = case when (CurrentLocation like '%f' OR CurrentLocation like '%f' )
								and TypeH=0 
							then ps.cost_cum*a.quantity else 0 end,

		TotalFis_amount = case when (CurrentLocation like '%f' OR CurrentLocation like '%f' or CurrentLocation like 'lost%')
								and TypeH=0 
							then ps.cost_cum*a.quantity else 0 end,


		Fis_amountLostCycle = case when ( CurrentLocation like 'lost%')
								and TypeH=0 
							then ps.cost_cum*a.quantity else 0 end,
		Good_Amount = case when  (typeh=1 or  TypeShipout=1) or
								(typeh=0 and (CurrentLocation not like '%f'
		...
		Scrap_Amount = case when TypeScrap=1 
							then ps.cost_cum*a.quantity else 0 end,
		Lost_Amount = (case when (CurrentLocation like '%f' or CurrentLocation like 'lost%' OR CurrentLocation like '%f')
								and TypeH=0 
