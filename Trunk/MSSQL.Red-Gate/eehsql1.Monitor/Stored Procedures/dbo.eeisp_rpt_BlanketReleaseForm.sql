...
GO
create procedure
[dbo].[eeisp_rpt_BlanketReleaseForm]
/*
Arguments:
None
...
while @@fetch_status=0
  begin
    select @FirmDays=(case when isnull(FABAuthdays,0)>0 then FABAuthDays else 14 end),
      ...
      and part_vendor.part=*blanket_part
    ...
    /*	</Debug>*/
    select @PlanWeeks=(case when isnull(ceiling(Lead_time/7),0)>0 then Lead_time else @firmWeeks end)
      from part_vendor
      where vendor
      =(select vendor_code
        from po_header
...
select BR.PONumber,
  BR.Part,
  BR.VendorPart,
  BR.Quantity,convert(timestamp,'20'
  +BR.DueDT),
