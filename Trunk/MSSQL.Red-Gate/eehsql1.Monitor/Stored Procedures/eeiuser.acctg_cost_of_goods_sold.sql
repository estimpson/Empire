...
CREATE proc [eeiuser].[acctg_cost_of_goods_sold] (@period int, @fiscal_year varchar(4), @beg_date datetime, @end_date datetime)
...
insert into @eeh_costs
 ...
 FROM		HistoricalData.dbo.part_standard_historical eeh_new_part_standard
 ...
 FROM       MONITOR.dbo.shipper_detail shipper_detail 
	...
	LEFT OUTER JOIN HISTORICALDATA.dbo.part_historical eei_part ON shipper_detail.part_original=eei_part.part and eei_part.fiscal_year = @fiscal_year and eei_part.period = @period and eei_part.reason = 'MONTH END'
	LEFT OUTER JOIN HISTORICALDATA.dbo.part_standard_historical eei_new_part_standard ON shipper_detail.part_original=eei_new_part_standard.part and eei_new_part_standard.fiscal_year = @fiscal_year and eei_new_part_standard.period = @period and eei_new_part_standard.reason = 'MONTH END'
	LEFT OUTER JOIN @eeh_costs eeh_new_part_standard on shipper_detail.part_original = eeh_new_part_standard.part
--	LEFT OUTER JOIN EEISQL1.Monitor.FT.Ftsp_BOMPerPart_incremental IC on ic.toppart = shipper_detail.part_original
