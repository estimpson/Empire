...
create procedure [dbo].[eeisp_rpt_object_historical_2017] (@FiscalYear int, @Period int, @Reason varchar(15), @PartType char(1), @spi_labor_percentage decimal(18,6), @spi_burden_percentage decimal(18,6) )
...
select	@eeh_time_stamp = (	select	max(time_stamp) 
							from	[EEHDATA4].[ArchiveData].dbo.object_historical -- [HistoricalData].dbo.object_historical 
							where	fiscal_year = @fiscalyear
								and period = @period
						  )
...
FROM		[EEHDATA4].ArchiveData.dbo.object_historical ohd -- HistoricalData.dbo.object_historical ohd
LEFT JOIN	[EEHDATA4].ArchiveData.dbo.part_historical phd ON ohd.part = phd.part and phd.time_stamp = @eeh_time_stamp -- HistoricalData.dbo.part_historical phd ON ohd.part = phd.part and phd.time_stamp = @eeh_time_stamp
LEFT JOIN	[EEHDATA4].ArchiveData.dbo.part_standard_historical pshd  ON  ohd.part = pshd.part and pshd.time_stamp = @eeh_time_stamp -- HistoricalData.dbo.part_standard_historical pshd ON  ohd.part = pshd.part and pshd.time_stamp = @eeh_time_stamp
