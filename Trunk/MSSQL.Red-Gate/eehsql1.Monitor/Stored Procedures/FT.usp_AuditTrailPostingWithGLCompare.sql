...
,	@ProductLineGL varchar(1000) = null
as
set nocount on
...
into
	#resultsAT
from
	eeh.dbo.audit_trail at with (readuncommitted, index = idx_audit_trail_date_type_part_std_quantity)
		...
		or	at.date_stamp in
			(	select
				...
				where
					...
					and bfd.SerialConsumed = at.serial
			)
	)
		...
		or	gct.transaction_date in
			(	select
				...
				where
					...
					and bfd.SerialConsumed = convert(int, gct.document_id1)
			)
	)
group by
	gct.document_id1
...
,	@Parts = [''Part1'', .... ''Partn'']
,	@ProductLineGL = [''80'', .... ''ProductLineGLn'']
