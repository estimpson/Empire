...
CREATE PROCEDURE [dbo].[eeisp_insert_EEH_RMA_from_EEI_RTV_withLocation]
...
--		Check that inTran Location exists:
IF	NOT EXISTS
	(	SELECT	1
		FROM	dbo.location
		WHERE	code = @LocationCode) BEGIN

	RAISERROR ('Invalid Location Code', 16, 1, @LocationCode)
	RETURN	-1
END
