...
SET ANSI_NULLS ON
GO
CREATE procedure	[dbo].[eeisp_rpt_copper_pricepershipment2] (@startdate datetime, @enddate datetime, @lookbackmonthspan integer, @intialshipmentlookback integer)
...
where				ap_items.invoice_cm = Vw_eei_spread_metal_surcharges.invoice_cm and
	...
	isNULL((select max(inv_cm_date) from #apitemsmetalsurcharge
 									...
 									where	#apitemsmetalsurcharge.inv_cm_date<= dateadd(mm, -1, #finshipped.firstshipdate) and
 												#apitemsmetalsurcharge.Item = #copperWire.part and
