...
create procedure [FTP].[usp_ReceiveCustomerEDI_InboundErrorFolderNotEmpty_Notification]
...
from
	RAWEDIDATA_FS.udf_DIR_Summary('\CustomerEDI\Inbound\Error') uds
...
into
	#ErrorFolderContents
from
	RAWEDIDATA_FS.udf_DIR('\CustomerEDI\Inbound\Error',1) ud
where
	ud.Name not in ('.', '..')
...
,   @EmailHeader nvarchar(max) = '(' + convert(varchar, @errorFileCount) + ') Files/Folders in CustomerEDI\Inbound\Error folder' 
...
execute
	@ProcReturn = FTP.usp_ReceiveCustomerEDI_InboundErrorFolderNotEmpty_Notification
	@TranDT = @TranDT out
