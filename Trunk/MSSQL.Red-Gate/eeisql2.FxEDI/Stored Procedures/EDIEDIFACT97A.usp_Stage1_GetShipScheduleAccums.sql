...
CREATE procedure [EDIEDIFACT97A].[usp_Stage1_GetShipScheduleAccums]
...
select
	...
	,	ReceivedAccum = max(case when QuantityQualifier in ('70' ) then coalesce(nullif(Quantity,''),0) when QuantityQualifier = '3' and QuantityType = '0' then coalesce(nullif(Quantity,''),0) else 0  end)
	,	ReceivedAccumBeginDT = ''
	,	ReceivedAccumEndDT = max(
					case 
					when datalength(DateDT) = 8 and  QuantityQualifier in ('70') 
					then edi.udf_GetDT('CCYYMMDD', coalesce(nullif(DateDT, '00000000'), '20990101')) 
					when datalength(DateDT) = 12 and  QuantityQualifier in ('70') 
					then edi.udf_GetDT('CCYYMMDDHHMM', coalesce(nullif(DateDT, '00000000'), '209901010001')) 
